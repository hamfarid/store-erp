...
find memory-bank/conversations/ -name "*.json" -mtime +90 -delete

# Remove old checkpoints (keep last 10)
ls -t memory-bank/checkpoints/*.json | tail -n +11 | xargs rm -f
```
...
```bash
# Create backup
tar -czf memory_backup_$(date +%Y%m%d).tar.gz memory-bank/
...
client = Client(Settings(
    ...
    persist_directory="memory-bank/vectors"
))
```
