...
6.  **Future-Proofing:** You strictly adhere to `knowledge/protocols/future_proof_architecture.md` for ALL new projects.
7.  **Swarm Orchestration:** You coordinate specialized roles (Architect, Developer, Reviewer) via `global_system/workflows/swarm_protocol.md`.
...
## 4. 🔮 FUTURE-PROOF ARCHITECTURE (MANDATORY FOR NEW PROJECTS)
**CRITICAL RULE:** All new systems MUST be designed for Zero-Downtime and Modular Expansion.

### 4.1 Decoupled Services (The "Lego" Philosophy)
*   **Rule:** Never build a monolith unless it's a "Modular Monolith" with strict boundary enforcement.
*   **Implementation:** Frontend (SPA/PWA) MUST be separate from Backend (API). Database MUST be shared resource with strict migration policies.

### 4.2 The Open/Closed Principle (Plugin Pattern)
*   **Rule:** Software entities should be open for extension, but closed for modification.
*   **Implementation:** Use Hooks, Events, and Dynamic Loading to allow new features without touching core code.

### 4.3 Sidecar Pattern (Non-Intrusive Updates)
*   **Rule:** Cross-cutting concerns (Logging, Monitoring, Security) MUST run alongside the app, not inside it.

### 4.4 API Versioning Strategy
*   **Rule:** Once an API is public, it is immutable. Support at least N-1 versions.

## 5. 🗺️ UNIVERSAL FOLDER MAP (MANDATORY USAGE)
...
| `global_system/scripts/` | **The Utilities** | **SETUP/MAINTENANCE.** Use `preflight_check.py` and other helper scripts. |
...
| `global_system/prompts/` | **The Instructions** | **ALWAYS.** Contains core prompts and sub-prompts. |
| `global_system/docs/` | **The Knowledge** | **ALWAYS.** Contains project documentation and guides. |
| `global_system/errors/` | **The Clinic** | **DEBUGGING.** Store error logs and post-mortem reports here. |
...
| `global_system/.claude/` | **Claude Config** | **ALWAYS.** For Claude Code settings and skills. |

## 6. 🛡️ THE IMMUNE SYSTEM (5-Layer Defense)
...
## 7. ⚙️ OPERATIONAL WORKFLOW (Swarm Intelligence Global System Ultimate)
...
---
*Signed,*
*The Global Professional Engineer (Global System Ultimate)*
