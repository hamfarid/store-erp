...
def create_app(config=None):
    ...
    # P0.21: Enable global autoescape for SSTI protection
    ...
    # P0.22: Anti-DoS & Caching
    app.config['MAX_CONTENT_LENGTH'] = 16 * 1024 * 1024  # 16MB max payload
    
    # Initialize Security Middleware
    try:
        from src.middleware.scanner_blocker import ScannerBlocker
        ScannerBlocker(app)
        print("✅ Scanner Blocker active")
    except ImportError:
        print("⚠️ Scanner Blocker not available")
    ...
    try:
        ...
        print(f"⚠️ Flask-Limiter not installed: {e}")
    ...
    if CSRF_AVAILABLE and CSRFProtect is not None:
        ...
        def csrf_exempt_check():
            ...
            if request.path.startswith("/api/"):
                # Ensure we exempt auth endpoints requiring no token
                if request.path in [
                    "/api/user/login",
                    "/api/auth/login",
                    "/api/user/register",
