...
class PurchaseReceipt(db.Model):
    ...
    # Relationships - disabled user relationships to avoid mapper conflicts
    ...
    @property
    def receiver(self):
        """Get the receiver - lazy query to avoid mapper conflicts"""
        if self.received_by:
            from src.models.user import User
            return User.query.get(self.received_by)
        return None
    
    @property
    def creator(self):
        """Get the creator - lazy query to avoid mapper conflicts"""
        if self.created_by:
            from src.models.user import User
