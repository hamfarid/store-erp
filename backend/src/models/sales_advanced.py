...
class SalesInvoice(db.Model):
    ...
    if SQLALCHEMY_AVAILABLE:
        customer = relationship(
            "Customer", backref=backref("sales_invoices", lazy="dynamic")
        )  # type: ignore[name-defined]
        sales_engineer = relationship(
            "SalesEngineer", backref=backref("assigned_invoices", lazy="dynamic")
        )  # type: ignore[name-defined]
        items = relationship(
...
class CustomerPayment(db.Model):
    ...
    if SQLALCHEMY_AVAILABLE:
        ...
        customer = relationship(
            "Customer", backref=backref("customer_payments", lazy="dynamic")
        )  # type: ignore[name-defined]
...
class SalesStats:
    ...
    def get_sales_engineer_stats(
        ...
        query = db.session.query(
            sa_func.count(SalesInvoice.id).label("total_invoices"),
