    ...
    def remove_hardcoded_passwords(self):
        ...
        for file_path in files_to_check:
            if os.path.exists(file_path):
                ...
                patterns = [
                    (r"password='admin123'", "password=os.getenv('ADMIN_PASSWORD', 'change_me')"),
                    (r'password === "admin123"', 'password === process.env.ADMIN_PASSWORD'),
                    (r"password.*=.*'admin123'", "password=os.getenv('ADMIN_PASSWORD', 'change_me')")
                ]
                ...
                if modified:
                    with open(file_path, 'w', encoding='utf-8') as f:
                        f.write(content)
                    self.fixes_applied.append(f"✅ إزالة كلمات مرور ثابتة من {file_path}")
                else:
                    self.fixes_applied.append(f"ℹ️ لا توجد كلمات مرور ثابتة في {file_path}")
        ...
        for pattern in test_patterns:
            for file_path in glob.glob(pattern):
                if os.path.isfile(file_path):
                    ...
                    moved_count += 1
                    self.fixes_applied.append(f"📦 نقل ملف اختبار: {file_path} -> {dest_path}")
        
        if moved_count > 0:
            self.fixes_applied.append(f"✅ تم نقل {moved_count} ملف اختبار إلى {test_archive}")
        else:
    ...
    def fix_duplicate_files(self):
        ...
        for duplicate, keep in duplicate_files:
            if os.path.exists(duplicate) and os.path.exists(keep):
                ...
                os.remove(duplicate)
                self.fixes_applied.append(f"✅ حذف ملف مكرر: {duplicate} (الاحتفاظ بـ {keep})")
            elif os.path.exists(duplicate):
                self.fixes_applied.append(f"ℹ️ ملف {duplicate} موجود ولكن {keep} غير موجود")
