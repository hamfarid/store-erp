...
class PerformanceOptimizer:
        ...
        if os.path.exists(frontend_path):
            ...
            if os.path.exists(dist_path):
                ...
                self.optimizations.append(
                    f"📊 حجم ملفات البناء: {size_mb:.2f} MB ({file_count} ملف)"
                )
    ...
    def analyze_performance_bottlenecks(self):
        ...
        if large_files:
            self.optimizations.append(
                f"⚠️ {len(large_files)} ملف كبير قد يؤثر على الأداء:"
            )
            ...
            for file in files:
                if file.endswith(".py"):
                    ...
                    try:
                        ...
                        joins = len(
                            re.findall(r"join\(|JOIN\s+", content, re.IGNORECASE)
                        )
                        subqueries = len(
                            re.findall(
                                r"subquery\(|EXISTS\s*\(", content, re.IGNORECASE
                            )
                        )
        ...
        if complex_queries > 0:
            self.optimizations.append(
                f"⚠️ {complex_queries} ملف يحتوي على استعلامات معقدة"
            )
        else:
    ...
    def generate_performance_report(self):
        ...
        info_count = len(self.optimizations) - success_count - warning_count
