...
def migrate_invoices():
    ...
    try:
        # دعم كلا المسارين (src أو بدونها) ليتوافق مع Pylance والتشغيل الفعلي
        try:
            from src.models.invoice import Invoice as OldInvoice
        except ImportError:
            from models.invoice import Invoice as OldInvoice
        try:
            from src.models.unified_invoice import UnifiedInvoice, UnifiedInvoiceItem
        except ImportError:
        ...
        for old_invoice in old_invoices:
            ...
            # نقل عناصر الفاتورة
            for old_item in (getattr(old_invoice, 'items', []) or []):
