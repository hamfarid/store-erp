...
const ProductManagementComplete = () => {
  ...
  const [_selectedProducts, setSelectedProducts] = useState([])
  const [_activeTab, _setActiveTab] = useState('products')
  ...
  // معالجة الاستيراد
  const handleImport = async (data, _file) => {
    try {
  ...
  // معالجة التصدير
  const handleExport = async (data, format, _filename) => {
    try {
