...
const StatusBadge = ({ status }) => {
  const config = {
    ...
    cancelled: { label: 'ملغاة', color: 'status-badge--neutral', icon: XCircle },
  };
...
const InvoiceRow = ({ invoice, onView, onEdit, onDelete, onPrint, onSend }) => {
  ...
  return (
    <tr className="hover:bg-hover transition-colors group">
      <td className="px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-tertiary">{invoice.items} منتجات</p>
          </div>
        </div>
      </td>
      <td className="px-6 py-4">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-primary">{invoice.customer}</span>
        </div>
      </td>
      <td className="px-6 py-4">
      ...
      <td className="px-6 py-4">
        <p className="font-semibold text-primary">{invoice.amount.toLocaleString()} ج.م</p>
        {invoice.status === 'partial' && (
          <div className="mt-1">
            ...
            <p className="text-xs text-tertiary mt-0.5">متبقي: {remaining.toLocaleString()}</p>
          </div>
        )}
      </td>
      <td className="px-6 py-4">
      ...
      <td className="px-6 py-4">
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <div className="relative">
            ...
            {showMenu && (
              <div className="absolute left-0 top-full mt-1 w-48 bg-primary rounded-xl shadow-xl border border-light py-2 z-10">
                <button
                  onClick={() => { onSend(invoice); setShowMenu(false); }}
                  className="w-full flex items-center gap-2 px-4 py-2 hover:bg-hover text-secondary"
                >
                  <Send size={16} />
...
const InvoicePage = () => {
  ...
  return (
    <div className="page-container" dir="rtl">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-secondary">إدارة فواتير المبيعات والمشتريات</p>
          </div>
        ...
        <div className="stats-grid grid-cols-1 md:grid-cols-4">
          <div className="stats-card hover:shadow-lg transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                <p className="stats-card-title">إجمالي الفواتير</p>
                <p className="stats-card-value">{totalAmount.toLocaleString()} ج.م</p>
              </div>
          ...
          <div className="stats-card hover:shadow-lg transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                <p className="stats-card-title">المدفوع</p>
                <p className="text-2xl font-bold text-emerald-600">{totalPaid.toLocaleString()} ج.م</p>
              </div>
          ...
          <div className="stats-card hover:shadow-lg transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                <p className="stats-card-title">قيد الانتظار</p>
                <p className="text-2xl font-bold text-amber-600">{pendingAmount.toLocaleString()} ج.م</p>
              </div>
          ...
          <div className="stats-card hover:shadow-lg transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                <p className="stats-card-title">متأخرة</p>
        ...
        {/* Filters */}
        <div className="entity-card shadow-sm">
          <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
            <div className="flex flex-col sm:flex-row gap-4 flex-1 w-full lg:w-auto">
              ...
              <div className="relative flex-1 max-w-md">
                ...
                <input
                  ...
                  className="w-full pr-12 pl-4 py-3 bg-secondary border-0 rounded-xl focus:ring-2 focus:ring-teal-500 transition-all"
                />
              </div>
              ...
              <select
                ...
                className="px-4 py-3 bg-secondary border-0 rounded-xl focus:ring-2 focus:ring-teal-500"
              >
            ...
            <div className="flex items-center gap-2">
              <button className="flex items-center gap-2 px-4 py-3 border-2 border-light rounded-xl hover:bg-hover transition-colors">
                <Download size={18} className="text-secondary" />
                <span className="text-sm font-medium text-secondary">تصدير</span>
              </button>
              <button className="flex items-center gap-2 px-4 py-3 border-2 border-light rounded-xl hover:bg-hover transition-colors">
                <RefreshCw size={18} className="text-secondary" />
                <span className="text-sm font-medium text-secondary">تحديث</span>
              </button>
            </div>
          </div>
        </div>
      ...
      {/* Table */}
      <div className="entity-card overflow-hidden shadow-sm" data-testid="invoices-table">
        <table className="w-full">
          <thead className="bg-secondary border-b border-light">
            <tr>
              ...
              <th className="px-6 py-4 text-right text-sm font-semibold text-secondary">العميل</th>
              <th className="px-6 py-4 text-right text-sm font-semibold text-secondary">
                <button className="flex items-center gap-1 hover:text-teal-600">
                  التاريخ
                  <ArrowUpDown size={14} />
        ...
        {filteredInvoices.length === 0 && (
          <div className="py-16 text-center">
            <FileText className="mx-auto text-tertiary mb-4" size={48} />
            <h3 className="text-lg font-semibold text-primary mb-1">لا توجد فواتير</h3>
            <p className="text-secondary">لم يتم العثور على فواتير تطابق معايير البحث</p>
          </div>
        )}
      </div>
      ...
      <div className="mt-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button className="p-2 border border-light rounded-lg hover:bg-hover transition-colors disabled:opacity-50" disabled>
            <ChevronRight size={18} className="text-secondary" />
          </button>
          <button className="w-10 h-10 bg-teal-600 text-white rounded-lg font-medium">1</button>
          <button className="w-10 h-10 hover:bg-hover rounded-lg font-medium text-secondary">2</button>
          <button className="w-10 h-10 hover:bg-hover rounded-lg font-medium text-secondary">3</button>
          <button className="p-2 border border-light rounded-lg hover:bg-hover transition-colors">
