      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 bg-secondary">
        <div className="w-full max-w-md">
          ...
          <div className="lg:hidden flex items-center justify-center gap-3 mb-8">
            ...
            <span className="text-2xl font-bold text-primary">Store Pro</span>
          </div>

          {/* Header */}
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-primary mb-2">مرحباً بعودتك</h1>
            <p className="text-secondary">سجل الدخول للوصول إلى لوحة التحكم</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="relative">
                <Mail className="absolute right-4 top-1/2 -translate-y-1/2 text-tertiary" size={20} />
                <input
                  type="email"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  placeholder="example@company.com"
                  className="w-full pr-12 pl-4 py-3.5 bg-primary border-2 border-light rounded-xl focus:ring-0 focus:border-teal-500 transition-colors text-primary placeholder:text-tertiary"
                  required
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <Lock className="absolute right-4 top-1/2 -translate-y-1/2 text-tertiary" size={20} />
                <input
                  type={showPassword ? 'text' : 'password'}
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  placeholder="••••••••••"
                  className="w-full pr-12 pl-12 py-3.5 bg-primary border-2 border-light rounded-xl focus:ring-0 focus:border-teal-500 transition-colors text-primary placeholder:text-tertiary"
                  required
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute left-4 top-1/2 -translate-y-1/2 text-tertiary hover:text-secondary transition-colors"
                >
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4 rounded border-light text-teal-500 focus:ring-teal-500"
                />
                <span className="text-sm text-secondary">تذكرني</span>
              </label>
          ...
          <div className="flex items-center gap-4 my-8">
            ...
            <span className="text-sm text-tertiary">أو</span>
            <div className="flex-1 h-px bg-light" />
          </div>

          {/* Social Login */}
          <div className="grid grid-cols-2 gap-4">
            <button className="flex items-center justify-center gap-2 py-3 border-2 border-light rounded-xl hover:bg-hover transition-colors">
