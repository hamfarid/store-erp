...
class TestAuthAPI:
    ...
    def test_get_current_user_valid_token(self):
        ...
        assert data["email"] == "admin@gaara-scan.ai"
    ...
    def test_get_current_user_invalid_token(self):
        ...
        assert response.status_code == 401
        data = response.json()
        assert "توكن غير صالح" in data["message"]
    ...
    def test_get_current_user_expired_token(self):
        ...
        assert response.status_code == 401
        data = response.json()
        assert "انتهت صلاحية التوكن" in data["message"]
...
class TestAuthAPISecurity:
    ...
    def test_token_contains_no_sensitive_data(self):
        ...
        assert "admin123" not in str(payload)
        
        # التحقق من وجود البيانات المطلوبة فقط
        required_fields = {"sub", "exp", "iat", "type"}
    ...
    def test_multiple_login_attempts(self):
        ...
        correct_login = {"username": "admin", "password": "admin123"}
