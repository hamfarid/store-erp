...
class PermissionBase(BaseModel):
    ...
    """
    name: str = Field(..., description="اسم الصلاحية", example="auth.users.read")
...
class Permission(PermissionBase):
    ...
    updated_at: Optional[datetime] = Field(None, description="تاريخ آخر تحديث للصلاحية")
...
class RoleBase(BaseModel):
    ...
    display_name: str = Field(..., description="اسم العرض للدور", example="مدير النظام")
...
class RoleCreate(RoleBase):
    ...
    مخطط إنشاء دور جديد
    """
    permissions: Optional[List[str]] = Field(None, description="قائمة بأسماء الصلاحيات المراد تعيينها للدور")
...
class Role(RoleBase):
    ...
    updated_at: Optional[datetime] = Field(None, description="تاريخ آخر تحديث للدور")
    permissions: List[Permission] = Field([], description="قائمة بالصلاحيات المعينة للدور")
...
class UserRoleCreate(UserRoleBase):
    ...
    """
    created_by: Optional[int] = Field(None, description="معرف المستخدم الذي أنشأ العلاقة")
...
class PermissionCheck(BaseModel):
    ...
    agent_id: Optional[int] = Field(None, description="معرف وكيل الذكاء الاصطناعي")
    permission: str = Field(..., description="اسم الصلاحية المراد التحقق منها")
    resource_id: Optional[int] = Field(None, description="معرف المورد (للصلاحيات على مستوى السجل)")
    ...
    def validate_ids(cls, v, values):
        ...
        if 'user_id' in values and values['user_id'] is None and 'agent_id' in values and values['agent_id'] is None:
            raise ValueError("يجب توفير معرف المستخدم أو معرف الوكيل على الأقل")
        return v
...
class PermissionCheckResult(BaseModel):
    ...
    """
    has_permission: bool = Field(..., description="هل يملك المستخدم/الوكيل الصلاحية")
...
class ModuleIntegrationInfo(BaseModel):
    ...
    is_registered: bool = Field(..., description="هل المديول مسجل في نظام الصلاحيات")
    permissions_count: int = Field(..., description="عدد الصلاحيات المسجلة للمديول")
...
class ModuleIntegrationStatus(BaseModel):
    ...
    """
    modules: List[ModuleIntegrationInfo] = Field(..., description="قائمة بمعلومات تكامل المديولات")
    total_registered: int = Field(..., description="إجمالي عدد المديولات المسجلة")
    total_required: int = Field(..., description="إجمالي عدد المديولات المطلوبة")
    integration_percentage: float = Field(..., description="نسبة اكتمال التكامل")
