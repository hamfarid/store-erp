...
class PermissionService:
    ...
    async def create_permission(self, name: str, description: str, resource_type: str, action: str) -> Permission:
        """
        إنشاء صلاحية جديدة
    ...
    async def update_permission(self, permission_id: str, name: str = None, description: str = None,
                                resource_type: str = None, action: str = None) -> Permission:
        """
        تحديث صلاحية
    ...
    async def create_role(self, name: str, description: str, is_system_role: bool = False,
    ...
    async def get_organization_roles(self, organization_id: str) -> List[Role]:
        ...
        Returns:
            List[Role]: قائمة الأدوار
        """
        return self.db.query(Role).filter(Role.organization_id == organization_id).all()

    async def update_role(self, role_id: str, name: str = None, description: str = None,
                          is_system_role: bool = None, organization_id: str = None) -> Role:
        """
        تحديث دور
    ...
    async def assign_permission_to_role(self, role_id: str, permission_id: str, assigned_by: str = None) -> bool:
        """
        تعيين صلاحية لدور
        ...
        try:
            ...
            logger.info(f"تم تعيين الصلاحية {permission.name} للدور {role.name}")
            return True
    ...
    async def remove_permission_from_role(self, role_id: str, permission_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"تم إزالة الصلاحية {permission.name} من الدور {role.name}")
            return True
    ...
    async def get_users_with_permission(self, permission_id: str) -> List[str]:
        ...
        role_permissions = self.db.query(RolePermission).filter(RolePermission.permission_id == permission_id).all()
        role_ids = [rp.role_id for rp in role_permissions]

        # الحصول على المستخدمين الذين لديهم هذه الأدوار
        user_roles = self.db.query(UserRole).filter(UserRole.role_id.in_(role_ids)).all()
        return [ur.user_id for ur in user_roles]
