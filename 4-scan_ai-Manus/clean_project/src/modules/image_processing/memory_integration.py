# File: /home/ubuntu/gaara_scan_ai_final_4.2/src/modules/image_processing/memory_integration.py
"""
...
from src.modules.memory.schemas import MemoryCreate, MemorySearch, MemoryTypeEnum, MemoryCategoryEnum
...
class ImageProcessingMemoryIntegration:
    ...
    async def store_image_metadata(
        ...
        try:
            ...
            logger.info("تم تخزين بيانات الصورة %s بنجاح، المعرف: %s", image_id, memory_id)
            return memory_id
    ...
    async def retrieve_image_metadata(
        ...
        try:
            ...
            logger.error("فشل في استرجاع بيانات الصورة %s: %s", image_id, str(e))
            raise
    ...
    async def store_processing_result(
        ...
        try:
            memory_id = await self.memory_service.create_memory(memory_entry)
            logger.info("تم تخزين نتيجة معالجة الصورة %s من نوع %s بنجاح", image_id, process_type)
            return memory_id
        except Exception as e:
            logger.error("فشل في تخزين نتيجة معالجة الصورة %s: %s", image_id, str(e))
            raise
    ...
    async def store_image_history(
        ...
        try:
            memory_id = await self.memory_service.create_memory(memory_entry)
            logger.info("تم تخزين سجل تاريخ الصورة %s للإجراء %s بنجاح", image_id, action)
            return memory_id
        except Exception as e:
            logger.error("فشل في تخزين سجل تاريخ الصورة %s: %s", image_id, str(e))
            raise
    ...
    async def retrieve_image_history(
        ...
        try:
            ...
            logger.info("تم استرجاع %s إدخال من سجل تاريخ الصورة %s", len(history_list), image_id)
            return history_list
        except Exception as e:
            logger.error("فشل في استرجاع سجل تاريخ الصورة %s: %s", image_id, str(e))
            raise
    ...
    async def store_image_collection(
        ...
        try:
            memory_id = await self.memory_service.create_memory(memory_entry)
            logger.info("تم تخزين مجموعة الصور %s (%s) بنجاح مع %s صور", collection_id, name, len(image_ids))
            return memory_id
        except Exception as e:
            logger.error("فشل في تخزين مجموعة الصور %s: %s", collection_id, str(e))
            raise
    ...
    async def retrieve_image_collection(
        ...
        try:
            memories = await self.memory_service.search_memories(memory_query)
            if not memories:
                logger.warning("لم يتم العثور على مجموعة الصور %s", collection_id)
                return None
            ...
            logger.error("فشل في استرجاع مجموعة الصور %s: %s", collection_id, str(e))
            raise
    ...
    async def search_images(
        ...
        try:
            ...
            logger.info("تم العثور على %s صورة مطابقة للبحث", len(results_list))
            return results_list
        except Exception as e:
    ...
    async def get_similar_images(
        ...
        try:
            ...
            for memory in memories:
                ...
                similarity_score = self._calculate_similarity(reference_features, current_features)
            ...
            logger.info("تم العثور على %s صورة مشابهة للصورة %s", len(similar_images), image_id)
            return similar_images
        except Exception as e:
            logger.error("فشل في البحث عن صور مشابهة للصورة %s: %s", image_id, str(e))
            raise
