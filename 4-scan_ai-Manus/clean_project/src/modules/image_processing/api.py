...
async def upload_image(image: UploadFile = File(...)) -> Dict[str, Any]:
    ...
    if not allowed_file(image.filename):
        raise HTTPException(status_code=400, detail="امتداد الملف غير مسموح به")
...
async def process_image(data: Dict[str, Any] = Body(...)) -> Dict[str, Any]:
    ...
    if not os.path.exists(image_path):
        raise HTTPException(status_code=404, detail=f"الصورة {filename} غير موجودة")

    # تحميل الصورة
    image = processor.load_image(image_path)

    if image is None:
        raise HTTPException(status_code=500, detail=f"فشل تحميل الصورة {filename}")
    ...
    for operation in operations:
        ...
        if op_type == "resize":
            ...
            processed_images["resize"] = processor.resize_image(image, width, height)
            ...
            processed_images["crop"] = processor.crop_image(image, x, y, width, height)
...
async def download_image(filename: str) -> FileResponse:
    ...
    if not os.path.exists(file_path):
        raise HTTPException(status_code=404, detail=f"الملف {filename} غير موجود")
