...
class CapabilityBase(BaseModel):
    ...
    name: str = Field(..., min_length=1, max_length=100, description="اسم القدرة")
...
class ModuleBase(BaseModel):
    ...
    name: str = Field(..., min_length=1, max_length=100, description="اسم المديول")
...
class AIAgentBase(BaseModel):
    ...
    name: str = Field(..., min_length=1, max_length=255, description="اسم الوكيل")
    ...
    temperature: float = Field(0.7, ge=0.0, le=1.0, description="درجة حرارة النموذج")
    max_tokens: int = Field(2000, ge=1, description="الحد الأقصى للرموز")
    memory_retention: int = Field(10, ge=1, description="عدد الرسائل المحتفظ بها في الذاكرة")
    ...
    settings: Optional[Dict[str, Any]] = Field(None, description="إعدادات الوكيل")
...
class AIAgentCreate(AIAgentBase):
    """مخطط إنشاء الوكيل"""
    capabilities: Optional[List[str]] = Field(None, description=CAPABILITIES_DESC)
...
class AIAgentUpdate(BaseModel):
    """مخطط تحديث الوكيل"""
    name: Optional[str] = Field(None, min_length=1, max_length=255, description="اسم الوكيل")
    description: Optional[str] = Field(None, description="وصف الوكيل")
    agent_type: Optional[AgentTypeEnum] = Field(None, description=AGENT_TYPE_DESC)
    role: Optional[AgentRoleEnum] = Field(None, description=AGENT_ROLE_DESC)
    status: Optional[AgentStatusEnum] = Field(None, description=AGENT_STATUS_DESC)
    ...
    system_prompt: Optional[str] = Field(None, description="الرسالة النظامية")
    temperature: Optional[float] = Field(None, ge=0.0, le=1.0, description="درجة حرارة النموذج")
    max_tokens: Optional[int] = Field(None, ge=1, description="الحد الأقصى للرموز")
    memory_retention: Optional[int] = Field(None, ge=1, description="عدد الرسائل المحتفظ بها في الذاكرة")
...
class AIAgentResponse(AIAgentBase):
    ...
    created_by: Optional[str] = Field(None, description="معرف المستخدم الذي أنشأ الوكيل")
    ...
    last_used_at: Optional[datetime] = Field(None, description="تاريخ آخر استخدام")
    capabilities: List[str] = Field([], description=CAPABILITIES_DESC)
    modules: List[str] = Field([], description=MODULES_DESC)
...
class AgentTaskBase(BaseModel):
    ...
    input_data: Optional[Dict[str, Any]] = Field(None, description="بيانات الإدخال")
...
class AgentTaskUpdate(BaseModel):
    """مخطط تحديث المهمة"""
    status: Optional[TaskStatusEnum] = Field(None, description=TASK_STATUS_DESC)
    output_data: Optional[Dict[str, Any]] = Field(None, description=OUTPUT_DATA_DESC)
    error_message: Optional[str] = Field(None, description=ERROR_MSG_DESC)
    execution_time_ms: Optional[int] = Field(None, description=EXECUTION_TIME_DESC)
    token_usage: Optional[Dict[str, int]] = Field(None, description=TOKEN_USAGE_DESC)
...
class AgentTaskResponse(AgentTaskBase):
    ...
    started_at: Optional[datetime] = Field(None, description="تاريخ بدء المهمة")
    completed_at: Optional[datetime] = Field(None, description="تاريخ اكتمال المهمة")
...
class AgentConversationUpdate(BaseModel):
    """مخطط تحديث المحادثة"""
    title: Optional[str] = Field(None, description="عنوان المحادثة")
    messages: Optional[List[MessageBase]] = Field(None, description=MESSAGES_DESC)
    token_usage: Optional[Dict[str, int]] = Field(None, description=TOKEN_USAGE_DESC)
...
class AgentConversationResponse(AgentConversationBase):
    ...
    last_message_at: Optional[datetime] = Field(None, description="تاريخ آخر رسالة")
...
class AgentConversationList(BaseModel):
    ...
    items: List[AgentConversationResponse] = Field(..., description="قائمة المحادثات")
...
class AgentFeedbackCreate(AgentFeedbackBase):
    ...
    conversation_id: Optional[str] = Field(None, description=CONVERSATION_ID_DESC)
...
class AgentStatsBase(BaseModel):
    ...
    avg_execution_time_ms: float = Field(0.0, description="متوسط وقت التنفيذ بالمللي ثانية")
    ...
    user_satisfaction: Optional[float] = Field(None, description="رضا المستخدمين")
...
class AgentTaskRequest(BaseModel):
    ...
    async_execution: bool = Field(False, description="ما إذا كان التنفيذ غير متزامن")
