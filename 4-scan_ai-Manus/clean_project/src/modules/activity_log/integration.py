...
def _extract_context_from_args(args: tuple) -> Tuple[Optional[Session], Optional[dict], Optional[BackgroundTasks]]:
...
def _extract_context_from_kwargs(kwargs: dict) -> Tuple[Optional[Session], Optional[dict], Optional[BackgroundTasks]]:
...
def _extract_context(args: tuple, kwargs: dict) -> Tuple[Optional[Session], Optional[dict], Optional[BackgroundTasks]]:
    """استخراج السياق من المعلمات"""
    db_args, current_user_args, background_tasks_args = _extract_context_from_args(args)
    db_kwargs, current_user_kwargs, background_tasks_kwargs = _extract_context_from_kwargs(kwargs)
...
def _process_request_data(kwargs: dict, exclude_params: List[str], include_request_data: bool, include_response_data: bool, result: Any) -> Dict[str, Any]:
    """معالجة بيانات الطلب"""
    request_data = {}
    if include_request_data:
        request_data = {k: v for k, v in kwargs.items() if k not in exclude_params}
...
def _log_system_error(
    ...
    **kwargs
    ...
    """تسجيل خطأ النظام"""
    description = _create_description(module_id, action_id, "system_error", result)
...
def _log_user_error(
    ...
    **kwargs
    ...
    """تسجيل خطأ المستخدم"""
    description = _create_description(module_id, action_id, "user_error", result)
    log_data = _prepare_log_data(
        module_id=module_id,
        action_id=action_id,
        log_type="user_error",
...
async def log_login_attempt(
    ...
    activity_data = _prepare_log_data(
        ...
        include_request_data=True,
        include_response_data=True,
        result={"status": status, "ip_address": ip_address, "user_agent": user_agent},
        current_user={"id": user_id, "username": username} if user_id else None
    )
