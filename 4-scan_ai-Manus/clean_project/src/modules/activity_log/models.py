...
class ActivityLog(Base):
    ...
    updated_at = Column(DateTime(timezone=True), onupdate=func.now(), nullable=True)  # pylint: disable=not-callable
...
class SystemLog(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    activity_log_id = Column(Integer, ForeignKey(ACTIVITY_LOGS_ID_FK), nullable=False)
...
class AILog(Base):
    ...
    interaction_type = Column(String(50), nullable=False, index=True)  # query, response, a2a_communication
...
class LogAction(Base):
    ...
    name_ar = Column(String(100), nullable=True)
    description = Column(Text, nullable=True)
    module_id = Column(String(50), ForeignKey("log_modules.module_id"), nullable=False)
    is_active = Column(Integer, default=1, nullable=False)
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class LogRetentionPolicy(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    log_type = Column(String(20), nullable=False, unique=True)  # system, user, ai
