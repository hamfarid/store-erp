...
class PriorityDeterminer:
    ...
    def start_monitoring(self) -> bool:
        ...
        logger.info("تم بدء مراقبة استهلاك الموارد وتحديد المديولات التي يمكن إغلاقها")
        return True
    ...
    def stop_monitoring(self) -> bool:
        ...
        logger.info("تم إيقاف مراقبة استهلاك الموارد وتحديد المديولات التي يمكن إغلاقها")
        return True
    ...
    def _monitoring_loop(self) -> None:
        ...
        """
        logger.info("بدء حلقة مراقبة استهلاك الموارد وتحديد المديولات التي يمكن إغلاقها")
        ...
        while self.is_monitoring:
            try:
                ...
                is_system_critical, critical_resources = self._is_system_resource_critical(system_resources)

                if is_system_critical:
                    logger.warning(f"استهلاك موارد النظام حرج: {critical_resources}")

                    # تحديد المديولات التي يمكن إغلاقها
                    modules_to_shutdown = self.determine_modules_to_shutdown(critical_resources)

                    # إغلاق المديولات تلقائيًا إذا كان مفعلًا
                    if self.config.get("auto_shutdown", True) and modules_to_shutdown:
                        logger.info(f"إغلاق المديولات تلقائيًا: {modules_to_shutdown}")
        ...
        logger.info("انتهاء حلقة مراقبة استهلاك الموارد وتحديد المديولات التي يمكن إغلاقها")

    def _is_system_resource_critical(self, system_resources: Dict[str, float]) -> Tuple[bool, Dict[str, bool]]:
        """
        التحقق مما إذا كان استهلاك موارد النظام حرجًا.
    ...
    def _get_resource_usage_level(self, resource_usage: Dict[str, float]) -> str:
        """
    ...
    def determine_modules_to_shutdown(self, critical_resources: Dict[str, bool] = None) -> Dict[str, str]:
        """
        ...
        with self.lock:
            ...
            running_modules = {
                module_id: module_info for module_id, module_info in all_modules.items()
            ...
            non_critical_modules = {
                module_id: module_info for module_id, module_info in running_modules.items()
                if module_id not in critical_module_ids
            }
            ...
            for module_id, module_info in non_critical_modules.items():
                ...
                resource_usage = self._get_resource_usage_level(module_info.resource_usage)
    ...
    def get_shutdown_plan(self, resource_type: str = None) -> List[Dict[str, Any]]:
        """
        ...
        with self.lock:
            ...
            if resource_type:
                shutdown_plan.sort(key=lambda x: x["resource_usage"].get(resource_type, 0), reverse=True)
            else:
    ...
    def add_critical_module(self, module_id: str) -> bool:
        ...
        with self.lock:
            ...
            if module_id in critical_modules:
                logger.warning(f"المديول {module_id} موجود بالفعل في قائمة المديولات الحرجة")
                return True
            ...
            if result:
                logger.info(f"تم إضافة المديول {module_id} إلى قائمة المديولات الحرجة")
    ...
    def remove_critical_module(self, module_id: str) -> bool:
        ...
        with self.lock:
            ...
            if module_id not in critical_modules:
                logger.warning(f"المديول {module_id} غير موجود في قائمة المديولات الحرجة")
                return True
            ...
            if result:
                logger.info(f"تم إزالة المديول {module_id} من قائمة المديولات الحرجة")
    ...
    def update_resource_usage_thresholds(self, level: str, **kwargs) -> bool:
        ...
        with self.lock:
            ...
            if result:
                logger.info(f"تم تحديث عتبات استهلاك الموارد لمستوى {level}: {kwargs}")

            return result

    def get_resource_usage_thresholds(self, level: str = None) -> Dict[str, Any]:
        """
    ...
    def update_shutdown_order(self, shutdown_order: List[Dict[str, Any]]) -> bool:
        """
