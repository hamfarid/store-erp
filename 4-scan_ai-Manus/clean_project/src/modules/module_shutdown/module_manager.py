...
class ModuleInfo:
    ...
    def __init__(self, module_id: str, name: str, description: str, priority: int = 5,
                 dependencies: Optional[List[str]] = None, resource_limits: Optional[Dict[str, float]] = None):
        """
...
class ModuleManager:
        ...
        with self.lock:
            ...
            for other_id, other_info in self.modules.items():
                if module_id in other_info.dependencies:
                    logger.error(f"لا يمكن إلغاء تسجيل المديول {module_id} لأن المديول {other_id} يعتمد عليه")
                    return False
        ...
        with self.lock:
            ...
            for dep_id in module_info.dependencies:
                if dep_id not in self.modules:
                    logger.error(f"المديول {dep_id} الذي يعتمد عليه {module_id} غير مسجل")
                    return False

                dep_info = self.modules[dep_id]
                if dep_info.state != ModuleState.RUNNING:
                    logger.info(f"بدء تشغيل المديول {dep_id} الذي يعتمد عليه {module_id}")
                    if not self.start_module(dep_id):
                        logger.error(f"فشل بدء تشغيل المديول {dep_id} الذي يعتمد عليه {module_id}")
                        return False
            ...
            try:
                ...
                # على سبيل المثال، يمكن استدعاء دالة بدء التشغيل الخاصة بالمديول
    ...
    def stop_module(self, module_id: str, force: bool = False) -> bool:
        ...
        with self.lock:
            ...
            # التحقق من حالة المديول
            if module_info.state in [ModuleState.STOPPED, ModuleState.STOPPING]:
            ...
            if not force:
                ...
                if dependent_modules:
                    logger.error(f"لا يمكن إيقاف المديول {module_id} لأن المديولات التالية تعتمد عليه: {', '.join(dependent_modules)}")
                    return False
            ...
            try:
                ...
                # على سبيل المثال، يمكن استدعاء دالة إيقاف التشغيل الخاصة بالمديول
    ...
    def pause_module(self, module_id: str) -> bool:
        ...
        with self.lock:
            ...
            try:
                ...
                # على سبيل المثال، يمكن استدعاء دالة الإيقاف المؤقت الخاصة بالمديول
    ...
    def resume_module(self, module_id: str) -> bool:
        ...
        with self.lock:
            ...
            try:
                ...
                logger.error(f"فشل استئناف تشغيل المديول {module_id}: {str(e)}")
                return False
    ...
    def restart_module(self, module_id: str) -> bool:
        ...
        with self.lock:
            ...
            if not self.stop_module(module_id):
                logger.error(f"فشل إيقاف المديول {module_id} أثناء إعادة التشغيل")
                return False

            # بدء تشغيل المديول
            if not self.start_module(module_id):
                logger.error(f"فشل بدء تشغيل المديول {module_id} أثناء إعادة التشغيل")
                return False
            ...
            for module_id, module_info in self.modules.items():
                if module_info.state == ModuleState.RUNNING and module_info.process_id:
                    try:
                        ...
                        module_info.resource_usage["cpu_percent"] = process.cpu_percent(interval=0.1)
                        module_info.resource_usage["memory_percent"] = process.memory_percent()

                        # تحديث وقت آخر تحديث
                        module_info.last_update = datetime.now()
                    except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess) as e:
                        logger.warning(f"خطأ في تحديث استهلاك الموارد للمديول {module_id}: {str(e)}")
    ...
    def is_resource_critical(self, module_id: str) -> Tuple[bool, Dict[str, bool]]:
        """
    ...
    def get_modules_by_state(self, state: str) -> List[str]:
        ...
        """
        return [module_id for module_id, module_info in self.modules.items() if module_info.state == state]

    def get_modules_by_priority(self, min_priority: int = 1, max_priority: int = 10) -> List[str]:
        """
    ...
    def _build_dependency_tree(self, module_id: str, tree: Dict[str, List[str]]) -> None:
        """
    ...
    def register_startup_callback(self, module_id: str, callback: Callable) -> None:
        """
    ...
    def register_shutdown_callback(self, module_id: str, callback: Callable) -> None:
        """
    ...
    def register_pause_callback(self, module_id: str, callback: Callable) -> None:
        """
    ...
    def register_resume_callback(self, module_id: str, callback: Callable) -> None:
        """
