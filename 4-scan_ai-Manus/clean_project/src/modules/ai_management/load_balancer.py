...
class LoadBalancer:
    ...
    def select_agent(
        ...
        try:
            ...
            filtered_agents = self._filter_agents_by_request_type(agents, request)
            ...
            selected_agent = self._apply_selection_strategy(filtered_agents, request)
            ...
            raise LoadBalancerError(f"خطأ غير متوقع أثناء اختيار الوكيل: {str(e)}")
    ...
    def handle_failover(
        ...
        try:
            ...
            if not available_agents:
                ...
                raise NoAvailableAgentsError("لا توجد وكلاء بديلة متاحة للتحويل التلقائي")

            # تطبيق تصفية أولية بناءً على نوع الطلب
            filtered_agents = self._filter_agents_by_request_type(available_agents, request)
            ...
            if not selected_agent:
                ...
                raise AgentSelectionError("فشل في اختيار وكيل بديل للتحويل التلقائي")

            logger.info("تم اختيار الوكيل البديل %s للتحويل التلقائي بعد فشل الوكيل {failed_agent_id}", selected_agent['id'])
            ...
            raise LoadBalancerError(f"خطأ غير متوقع أثناء معالجة التحويل التلقائي: {str(e)}")
    ...
    def update_agent_stats(
        ...
        try:
            ...
            logger.debug("تم تحديث إحصائيات الوكيل %s: وقت الاستجابة={response_time}، نجاح={success}، الرموز={tokens}", agent_id)
        except Exception as e:
    ...
    def _filter_agents_by_user_preferences(
        ...
        try:
            ...
            logger.error("خطأ أثناء تصفية الوكلاء بناءً على تفضيلات المستخدم: %s", str(e))
            return agents
    ...
    def _select_agent_by_cost(self, agents: List[Dict[str, Any]], request: Dict[str, Any]) -> Dict[str, Any]:
        """
