...
def initialize_setup_extended(db: Session) -> schemas.SetupStatusResponse:
    ...
    if setup_status:
        ...
        setup_status.token_expires_at = datetime.now(timezone.utc) + timedelta(hours=24)
    else:
        # إنشاء حالة جديدة
        setup_status = models.SetupStatus(
            is_completed=False,
...
def get_step_data_extended(db: Session, step_id: str) -> Optional[schemas.StepDataResponse]:
    """
    ...
    # تسجيل الحدث
    log_setup_event(db, "get_step_data", f"تم الحصول على بيانات الخطوة {step_id}", {"step_id": step_id})
...
def validate_step_data_extended(db: Session, step_id: str, step_data: Dict[str, Any]) -> schemas.ValidationResult:
    """
    ...
    if step_id == "welcome":
        ...
        if "branches" not in step_data or not isinstance(step_data["branches"], list):
            errors.append("يجب إدخال الفروع")
        else:
            # التحقق من وجود فرع رئيسي واحد على الأقل
            main_branch_count = sum(1 for branch in step_data["branches"] if branch.get("is_main", False))
        ...
        if "admin_password" not in step_data or not step_data["admin_password"]:
            ...
            is_valid, message, score = security.validate_password_strength(step_data["admin_password"])
            if not is_valid:
        ...
        if "admin_email" not in step_data or not step_data["admin_email"]:
            ...
            import re
            if not re.match(r'^[\w\.-]+@[\w\.-]+\.\w+$', step_data["admin_email"]):
        ...
        if "roles" in step_data and isinstance(step_data["roles"], list):
            ...
            admin_role_count = sum(1 for role in step_data["roles"] if role.get("is_admin", False))
            if admin_role_count == 0:
                errors.append("يجب تحديد دور مسؤول واحد على الأقل")

    elif step_id == "module_selection":
        # التحقق من بيانات اختيار وتكوين المديولات
        if "enabled_modules" not in step_data or not isinstance(step_data["enabled_modules"], list):
            ...
            for module_id in step_data["enabled_modules"]:
                ...
                if not any(module["id"] == module_id for module in available_modules):
                ...
                if module_id in module_dependencies:
                    for dependency in module_dependencies[module_id]:
                        if dependency not in step_data["enabled_modules"]:
                            errors.append(f"المديول {module_id} يعتمد على المديول {dependency}")

    elif step_id == "ai_settings":
        # التحقق من بيانات إعدادات الذكاء الاصطناعي
        if "enabled_agents" not in step_data or not isinstance(step_data["enabled_agents"], list):
        ...
        if "email_enabled" in step_data and step_data["email_enabled"]:
            ...
            if "from_email" not in step_data or not step_data["from_email"]:
                ...
                import re
                if not re.match(r'^[\w\.-]+@[\w\.-]+\.\w+$', step_data["from_email"]):
        ...
        validation_result = security.validate_security_settings(security_settings)
        ...
        if "backup_enabled" in step_data and step_data["backup_enabled"]:
            ...
            if "backup_retention" not in step_data or not isinstance(step_data["backup_retention"], int) or step_data["backup_retention"] <= 0:
    ...
    # تسجيل الحدث
    log_setup_event(db, "validate_step_data", f"تم التحقق من صحة بيانات الخطوة {step_id}", {
        "step_id": step_id,
...
def update_step_data_extended(db: Session, step_id: str, step_data: Dict[str, Any]) -> schemas.StepUpdateResponse:
    """
    ...
    try:
        ...
        # تسجيل الحدث
        log_setup_event(db, "update_step_data", f"تم تحديث بيانات الخطوة {step_id}", {
            "step_id": step_id
        })
        ...
        # تسجيل الحدث
        log_setup_event(db, "update_step_data_error", f"خطأ في تحديث بيانات الخطوة {step_id}", {
            "step_id": step_id,
            "error": str(e)
        })
...
def next_step_extended(db: Session) -> schemas.SetupStatusResponse:
    ...
    if not setup_status:
        ...
        # تسجيل الحدث
        log_setup_event(db, "next_step", "تم إنشاء حالة الإعداد والانتقال إلى الخطوة الأولى", {
            "current_step": setup_status.current_step
        })
    ...
    if setup_status.is_completed:
        # تسجيل الحدث
        log_setup_event(db, "next_step", "محاولة الانتقال إلى الخطوة التالية بعد اكتمال الإعداد", {
            "current_step": setup_status.current_step
        })
    ...
    if next_step_index >= len(SETUP_STEPS_EXTENDED):
        # تسجيل الحدث
        log_setup_event(db, "next_step", "محاولة الانتقال إلى ما بعد الخطوة الأخيرة", {
            "current_step": setup_status.current_step
        })
    ...
    # تسجيل الحدث
    log_setup_event(db, "next_step", f"تم الانتقال إلى الخطوة {setup_status.current_step}", {
        "current_step": setup_status.current_step,
...
def previous_step_extended(db: Session) -> schemas.SetupStatusResponse:
    ...
    if not setup_status:
        ...
        # تسجيل الحدث
        log_setup_event(db, "previous_step", "تم إنشاء حالة الإعداد والانتقال إلى الخطوة الأولى", {
            "current_step": setup_status.current_step
        })
    ...
    if setup_status.is_completed:
        # تسجيل الحدث
        log_setup_event(db, "previous_step", "محاولة العودة إلى الخطوة السابقة بعد اكتمال الإعداد", {
            "current_step": setup_status.current_step
        })
    ...
    if previous_step_index < 0:
        # تسجيل الحدث
        log_setup_event(db, "previous_step", "محاولة العودة إلى ما قبل الخطوة الأولى", {
            "current_step": setup_status.current_step
        })
    ...
    # تسجيل الحدث
    log_setup_event(db, "previous_step", f"تم العودة إلى الخطوة {setup_status.current_step}", {
        "current_step": setup_status.current_step,
...
def validate_setup_completion_extended(db: Session) -> schemas.ValidationResult:
    """
    ...
    # تسجيل الحدث
    log_setup_event(db, "validate_setup_completion", "تم التحقق من اكتمال جميع الخطوات الإلزامية", {
        "is_valid": len(errors) == 0,
        "errors": errors
    })
...
def complete_setup_extended(db: Session) -> schemas.SetupCompletionResponse:
    ...
    try:
        ...
        # تسجيل الحدث
        log_setup_event(db, "complete_setup", "تم إكمال عملية الإعداد بنجاح", {})
        ...
        # تسجيل الحدث
        log_setup_event(db, "complete_setup_error", "خطأ في إكمال عملية الإعداد", {
            "error": str(e)
        })
...
def log_setup_event(db: Session, event_type: str, message: str, details: Dict[str, Any]) -> None:
    """
    تسجيل حدث الإعداد
...
def get_enabled_modules(db: Session) -> List[str]:
    ...
    return ["core", "ai", "disease_diagnosis", "memory", "activity_log", "import_export"]
...
def update_notification_settings(db: Session, settings: Dict[str, Any]) -> None:
...
def update_backup_import_export_settings(db: Session, settings: Dict[str, Any]) -> None:
