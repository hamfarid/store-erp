...
def get_step_data(db: Session, step_id: str) -> schemas.StepDataResponse:
    ...
    if step_id == "welcome":
        ...
        notification_settings = notification_service.get_notification_settings(db)
        data = {
        ...
        data = {
            ...
            "session_timeout": security_settings.get("session_timeout", 30),  # بالدقائق
    ...
    if step_id == "system_settings":
        ...
        if "admin_password" in data:
            ...
            if not password:
                ...
                if policy.get("require_special_chars", True) and not any(
                    ...
                    errors.append("كلمة المرور يجب أن تحتوي على حرف خاص واحد على الأقل")
        ...
        if "central_ai" in data and "model_type" in data["central_ai"]:
            ...
            if data["central_ai"]["model_type"] == "cloud" and not data[
                ...
                errors.append("مفتاح API مطلوب لنموذج الذكاء الاصطناعي السحابي")
...
def update_step_data(
    ...
    try:
        ...
        # تسجيل نشاط الإعداد
        log_setup_activity(db, step_id, "info", f"تم تحديث بيانات الخطوة {step_id}")
...
def previous_step(db: Session) -> schemas.SetupStatusResponse:
    ...
    # تسجيل نشاط الإعداد
    log_setup_activity(db, prev_step_id, "info", f"تم العودة إلى الخطوة {prev_step_id}")
...
def complete_setup(db: Session) -> schemas.SetupCompletionResponse:
    ...
    try:
        ...
        # تسجيل نشاط الإعداد
        log_setup_activity(db, "complete", "info", "تم إكمال عملية الإعداد بنجاح")
        ...
        # تسجيل الخطأ
        log_setup_activity(
            db, "complete", "error", "فشل في إكمال عملية الإعداد", {"error": str(e)}
        )
...
def _post_setup_actions(db: Session) -> None:
    ...
    for module in modules:
        try:
            ...
            log_setup_activity(
                db, "initialize_module", "info", f"تم تهيئة المديول {module.name}"
            )
        except Exception as e:
            log_setup_activity(
                db,
    ...
    try:
        ...
        log_setup_activity(db, "initial_backup", "info", "تم إنشاء نسخة احتياطية أولية")
    except Exception as e:
        log_setup_activity(
            db,
            "initial_backup",
...
def log_setup_activity(
    db: Session, step: str, status: str, message: str, details: Dict[str, Any] = None
) -> None:
    """
    تسجيل نشاط الإعداد
