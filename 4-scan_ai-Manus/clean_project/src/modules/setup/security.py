...
def validate_security_settings(security_settings: schemas.SecuritySettings) -> schemas.SecurityValidationResponse:
    """
    ...
    if security_settings.use_ssl:
        if not security_settings.ssl_cert_path or not security_settings.ssl_key_path:
            is_valid = False
            warnings.append("تم تمكين SSL ولكن لم يتم تحديد مسارات الشهادة والمفتاح")
        else:
            # التحقق من وجود الملفات
            if not os.path.exists(security_settings.ssl_cert_path):
                is_valid = False
                warnings.append(f"ملف شهادة SSL غير موجود: {security_settings.ssl_cert_path}")
            if not os.path.exists(security_settings.ssl_key_path):
                is_valid = False
                warnings.append(f"ملف مفتاح SSL غير موجود: {security_settings.ssl_key_path}")
    else:
        recommendations.append("يوصى بتمكين SSL لتأمين الاتصالات")

    # التحقق من مهلة الجلسة
    if security_settings.session_timeout < 15:
        warnings.append("مهلة الجلسة قصيرة جداً، قد تؤدي إلى تسجيل خروج متكرر للمستخدمين")
    ...
    if security_settings.max_login_attempts < 3:
        warnings.append("الحد الأقصى لمحاولات تسجيل الدخول منخفض جداً، قد يؤدي إلى قفل المستخدمين بسهولة")
    elif security_settings.max_login_attempts > 10:
        warnings.append("الحد الأقصى لمحاولات تسجيل الدخول مرتفع جداً، قد يسمح بهجمات القوة الغاشمة")

    # التحقق من مدة القفل
    if security_settings.lockout_duration < 5:
        warnings.append("مدة القفل قصيرة جداً، قد لا تكون فعالة ضد هجمات القوة الغاشمة")

    # التحقق من أيام انتهاء صلاحية كلمة المرور
    if security_settings.password_expiry_days < 30:
        warnings.append("فترة انتهاء صلاحية كلمة المرور قصيرة جداً، قد تسبب إزعاجاً للمستخدمين")
    elif security_settings.password_expiry_days > 180:
        warnings.append("فترة انتهاء صلاحية كلمة المرور طويلة جداً، قد تشكل مخاطر أمنية")
    ...
    if not security_settings.rate_limiting:
        ...
        if security_settings.rate_limiting.get("enabled", False):
            if security_settings.rate_limiting.get("max_requests", 0) < 10:
                warnings.append("الحد الأقصى للطلبات منخفض جداً، قد يؤثر على أداء النظام")
...
def test_ssl_connection(host: str, port: int, cert_path: str, key_path: str) -> Tuple[bool, str]:
    """
    اختبار اتصال SSL
...
def log_security_event(db: Session, event_type: str, details: Dict[str, Any], user_id: Optional[int] = None) -> None:
    """
    تسجيل حدث أمني
...
def sanitize_input(input_str: str) -> str:
    ...
    sanitized = re.sub(r'(\b(select|insert|update|delete|drop|alter|exec|union|where)\b)',
                       lambda match: match.group(1).upper(), sanitized, flags=re.IGNORECASE)
