...
class RobustnessAnalyzer:
    ...
    def test_comprehensive_robustness(self, model_name, test_images):
        ...
        for test_name, test_results in robustness_tests.items():
            if test_results:
                ...
                if accuracies:
                    avg_robustness[test_name] = sum(accuracies) / len(accuracies)

        # تصنيف المتانة الإجمالية
        if avg_robustness:
            overall_robustness = sum(avg_robustness.values()) / len(avg_robustness)
            robustness_category = self._categorize_robustness(overall_robustness)
        else:
    ...
    def test_noise_robustness(self, model_name, test_images):
        ...
        for noise_level in noise_levels:
            ...
            for image_path in test_images:
                try:
                    original_result = self.processor.predict_single_model(image_path, model_name)
                    if "error" in original_result:
                        continue

                    noisy_image_path = self.add_noise(image_path, noise_level)
                    noisy_result = self.processor.predict_single_model(noisy_image_path, model_name)

                    if "error" not in noisy_result:
                        original_predictions.append(original_result['prediction'])
            ...
            if original_predictions and noisy_predictions:
                accuracy = accuracy_score(original_predictions, noisy_predictions)
        ...
        for factor in brightness_factors:
            ...
            for image_path in test_images:
                try:
                    ...
                    modified_image_path = self.adjust_brightness(image_path, factor)
                    modified_result = self.processor.predict_single_model(modified_image_path, model_name)

                    if "error" not in modified_result:
                        original_predictions.append(original_result['prediction'])
                        modified_predictions.append(modified_result['prediction'])
            ...
            if original_predictions and modified_predictions:
                accuracy = accuracy_score(original_predictions, modified_predictions)
        ...
        for factor in contrast_factors:
            ...
            for image_path in test_images:
                try:
                    ...
                    modified_image_path = self.adjust_contrast(image_path, factor)
                    modified_result = self.processor.predict_single_model(modified_image_path, model_name)
        ...
        for radius in blur_radii:
            ...
            for image_path in test_images:
                try:
                    ...
                    blurred_result = self.processor.predict_single_model(blurred_image_path, model_name)

                    if "error" not in blurred_result:
                        original_predictions.append(original_result['prediction'])
                        blurred_predictions.append(blurred_result['prediction'])
            ...
            if original_predictions and blurred_predictions:
                accuracy = accuracy_score(original_predictions, blurred_predictions)
        ...
        for factor in scale_factors:
            ...
            for image_path in test_images:
                try:
                    ...
                    scaled_result = self.processor.predict_single_model(scaled_image_path, model_name)
            ...
            if original_predictions and scaled_predictions:
                accuracy = accuracy_score(original_predictions, scaled_predictions)
        ...
        for percentage in occlusion_percentages:
            ...
            for image_path in test_images:
                try:
                    ...
                    occluded_image_path = self.add_occlusion(image_path, percentage)
                    occluded_result = self.processor.predict_single_model(occluded_image_path, model_name)

                    if "error" not in occluded_result:
                        original_predictions.append(original_result['prediction'])
                        occluded_predictions.append(occluded_result['prediction'])
            ...
            if original_predictions and occluded_predictions:
                accuracy = accuracy_score(original_predictions, occluded_predictions)
        ...
        for quality in quality_levels:
            ...
            for image_path in test_images:
                try:
                    ...
                    compressed_image_path = self.compress_image(image_path, quality)
                    compressed_result = self.processor.predict_single_model(compressed_image_path, model_name)

                    if "error" not in compressed_result:
                        original_predictions.append(original_result['prediction'])
                        compressed_predictions.append(compressed_result['prediction'])
            ...
            if original_predictions and compressed_predictions:
                accuracy = accuracy_score(original_predictions, compressed_predictions)
    ...
    def create_robustness_comparison(self, models_to_compare, test_images):
        ...
        for test_type in test_types:
            ...
            for model_name in models_to_compare:
                if model_name in self.robustness_data and test_type in self.robustness_data[model_name]:
                    comparison["comparison_results"][test_type][model_name] = self.robustness_data[model_name][test_type]

        # إضافة النتائج الإجمالية
        comparison["overall_comparison"] = {}
        for model_name in models_to_compare:
            if model_name in self.robustness_data and "overall_results" in self.robustness_data[model_name]:
        ...
        if comparison["overall_comparison"]:
            ...
            comparison["most_robust_model"] = {
                ...
                "average_robustness": best_model[1].get("average_robustness", 0),
                "robustness_category": best_model[1].get("robustness_category", "غير معروف")
            }

        return comparison

    def visualize_robustness_comparison(self, comparison_results, output_dir=None):
        ...
        if "overall_comparison" in comparison_results:
            ...
            for model_name in models:
                if model_name in comparison_results["overall_comparison"]:
                    ...
                    robustness_scores.append(
                        comparison_results["overall_comparison"][model_name].get("average_robustness", 0)
                    )
            ...
            if model_names and robustness_scores:
                ...
                if output_dir:
                    plot_path = os.path.join(output_dir, "overall_robustness_comparison.png")
        ...
        for test_type, test_title in test_types:
            if test_type in comparison_results["comparison_results"]:
                ...
                if output_dir:
                    plot_path = os.path.join(output_dir, f"{test_type}_comparison.png")
                    plt.savefig(plot_path)
    ...
    def generate_robustness_report(self, comparison_results, output_dir=None):
        ...
        plots = self.visualize_robustness_comparison(comparison_results, output_dir)
        ...
        report = {
            ...
            "test_images_count": comparison_results.get("test_images_count", 0),
            "overall_results": comparison_results.get("overall_comparison", {}),
            "most_robust_model": comparison_results.get("most_robust_model", {}),
    ...
    def analyze_robustness_data(self, comparison_results):
        ...
        for model_name in models:
            ...
            if "overall_comparison" in comparison_results and model_name in comparison_results["overall_comparison"]:
                ...
                for test_type, test_name in test_types:
                    if test_type in comparison_results["comparison_results"] and model_name in comparison_results["comparison_results"][test_type]:
                        test_results = comparison_results["comparison_results"][test_type][model_name]
                        accuracies = [value["accuracy"] for key, value in test_results.items() if isinstance(value, dict) and "accuracy" in value]

                        if accuracies:
                            avg_accuracy = sum(accuracies) / len(accuracies)
                            if avg_accuracy >= 0.8:
                                strengths.append(f"{test_name} ({avg_accuracy:.2f})")
                            elif avg_accuracy <= 0.5:
                                weaknesses.append(f"{test_name} ({avg_accuracy:.2f})")
            ...
            if "weaknesses" in insights["strengths_and_weaknesses"][model_name]:
                for weakness in insights["strengths_and_weaknesses"][model_name]["weaknesses"]:
                    if "الضوضاء" in weakness:
                        recommendations.append("تحسين مقاومة الضوضاء عن طريق تدريب النموذج على صور مع ضوضاء مختلفة")
                    elif "الإضاءة" in weakness:
                        recommendations.append("تحسين مقاومة تغييرات الإضاءة عن طريق تدريب النموذج على صور بمستويات إضاءة مختلفة")
                    elif "التباين" in weakness:
                        recommendations.append("تحسين مقاومة تغييرات التباين عن طريق تدريب النموذج على صور بمستويات تباين مختلفة")
                    elif "الدوران" in weakness:
                        recommendations.append("تحسين مقاومة الدوران عن طريق تدريب النموذج على صور مدورة بزوايا مختلفة")
                    elif "الضبابية" in weakness:
                        recommendations.append("تحسين مقاومة الضبابية عن طريق تدريب النموذج على صور مضببة بدرجات مختلفة")
                    elif "الحجم" in weakness:
                        recommendations.append("تحسين مقاومة تغيير الحجم عن طريق تدريب النموذج على صور بأحجام مختلفة")
                    elif "الحجب" in weakness:
                        recommendations.append("تحسين مقاومة الحجب عن طريق تدريب النموذج على صور مع حجب جزئي")
                    elif "الضغط" in weakness:
                        recommendations.append("تحسين مقاومة الضغط عن طريق تدريب النموذج على صور مضغوطة بجودات مختلفة")
        ...
        if "overall_comparison" in comparison_results:
            ...
            most_robust_model = max(
                [(model, data.get("average_robustness", 0)) for model, data in comparison_results["overall_comparison"].items()],
                key=lambda x: x[1]
            )
            ...
            for model_name in models:
                if model_name in insights["strengths_and_weaknesses"]:
                    strengths = set([s.split(" (")[0] for s in insights["strengths_and_weaknesses"][model_name]["strengths"]])
                    weaknesses = set([w.split(" (")[0] for w in insights["strengths_and_weaknesses"][model_name]["weaknesses"]])
            ...
            insights["comparative_analysis"]["common_strengths"] = list(common_strengths)
            insights["comparative_analysis"]["common_weaknesses"] = list(common_weaknesses)
