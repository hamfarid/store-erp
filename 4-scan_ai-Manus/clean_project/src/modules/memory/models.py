...
from sqlalchemy import Column, Integer, String, Text, Float, Boolean, DateTime, ForeignKey, Table, JSON, Enum, UniqueConstraint
...
memory_tags = Table(
    'memory_tags',
    Base.metadata,
    Column('memory_id', String(36), ForeignKey(MEMORIES_ID_FK, ondelete='CASCADE'), primary_key=True),
...
class Memory(Base):
    ...
    access_level = Column(Enum(MemoryAccess), nullable=False, default=MemoryAccess.PRIVATE, index=True)
    ...
    # بيانات إضافية - تم تغيير الاسم من metadata إلى extra_data لتجنب تعارض الاسم المحجوز
    extra_data = Column(JSON, nullable=True)

    # العلاقات
    tags = relationship("Tag", secondary=memory_tags, back_populates="memories")
    entities = relationship("Entity", secondary=memory_entities, back_populates="memories")
...
class Entity(Base):
    ...
    entity_data = Column(JSON, nullable=True)  # تم تغيير الاسم من metadata إلى entity_data
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)

    # العلاقات
    memories = relationship("Memory", secondary=memory_entities, back_populates="entities")
...
class MemoryAccessLog(Base):
    ...
    id = Column(Integer, primary_key=True)
    memory_id = Column(String(36), ForeignKey(MEMORIES_ID_FK, ondelete='CASCADE'), nullable=False, index=True)
