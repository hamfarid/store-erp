...
class InferenceEngine:
    ...
    def perform_inference(self, request_data: DiagnosisRequest, ml_model_outputs: Optional[List[Dict[str, Any]]] = None) -> List[DiagnosisResult]:
        ...
        if hasattr(request_data, 'symptoms_description') and request_data.symptoms_description:
            symptom_results = self._infer_from_symptoms(request_data.symptoms_description)
    ...
    def _infer_from_symptoms(self, symptoms_description: str) -> List[DiagnosisResult]:
        ...
        if self.kb_manager:
            ...
            for entry in entries:
                confidence = self._calculate_symptom_match_confidence(symptoms_description, entry.symptoms)
    ...
    def _infer_from_ml_outputs(self, ml_outputs: List[Dict[str, Any]]) -> List[DiagnosisResult]:
        """الاستدلال من مخرجات نماذج التعلم الآلي.
            ...
            if model_type == "classification":
                ...
                if predictions.get("is_anomalous", False):
                    results.append(DiagnosisResult(
                        ...
                        description=predictions.get("details", "تم اكتشاف شذوذ"),
                        source=f"ml_model_{model_type}",
                        evidence=[],
    ...
    def _find_kb_entry_for_disease(self, disease_name: str) -> Optional[KnowledgeBaseEntry]:
    ...
    def _calculate_symptom_match_confidence(self, symptoms_description: str, known_symptoms: List[str]) -> float:
    ...
    def _deduplicate_and_sort_results(self, results: List[DiagnosisResult]) -> List[DiagnosisResult]:
        ...
        for result in results:
            if result.disease_name not in unique_results or result.confidence > unique_results[result.disease_name].confidence:
