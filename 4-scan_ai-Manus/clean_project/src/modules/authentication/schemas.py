...
from src.modules.authentication.models import TokenType, TokenStatus, AuthProvider, MFAMethod, LoginStatus
...
class UserSessionUpdate(BaseModel):
    ...
    last_activity_at: Optional[datetime] = Field(None, description="وقت آخر نشاط")
    expires_at: Optional[datetime] = Field(None, description=EXPIRES_AT_DESC)
    revoked_at: Optional[datetime] = Field(None, description=REVOKE_TIME_DESC)
    is_active: Optional[bool] = Field(None, description="ما إذا كانت الجلسة نشطة")
    metadata: Optional[Dict[str, Any]] = Field(None, description=METADATA_DESC)
...
class MFAMethodConfigurationCreate(BaseModel):
    ...
    mfa_config_id: str = Field(..., description="معرف تكوين المصادقة متعددة العوامل")
...
class AuthLogCreate(BaseModel):
    ...
    username_attempt: Optional[str] = Field(None, description="محاولة اسم المستخدم")
    action: str = Field(..., description="الإجراء")
    status: LoginStatus = Field(..., description=STATUS_DESC)
    ip_address: Optional[str] = Field(None, description=IP_ADDRESS_DESC)
    user_agent: Optional[str] = Field(None, description=USER_AGENT_DESC)
    provider: Optional[AuthProvider] = Field(None, description=AUTH_PROVIDER_DESC)
...
class PasswordResetConfirmRequest(BaseModel):
    ...
    def validate_password_strength(cls, v):
        ...
        if not re.search(r'[!@#$%^&*(),.?":{}|<>]', v):
            raise ValueError("يجب أن تحتوي كلمة المرور على حرف خاص واحد على الأقل")
...
class LoginResponse(BaseModel):
    ...
    mfa_token: Optional[str] = Field(None, description="رمز المصادقة متعددة العوامل")
