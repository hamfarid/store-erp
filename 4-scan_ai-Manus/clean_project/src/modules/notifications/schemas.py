...
from src.modules.notifications.config import NOTIFICATION_TYPES, NOTIFICATION_PRIORITIES, NOTIFICATION_CHANNELS
...
class NotificationCreateSchema(BaseModel):
    ...
    channels: List[str] = Field(default_factory=list, description=NOTIFICATION_CHANNELS_DESCRIPTION)
    ...
    def validate_type(cls, v):
        if v not in NOTIFICATION_TYPES.values():
            raise ValueError(f"نوع الإشعار غير صالح. القيم المسموح بها: {', '.join(NOTIFICATION_TYPES.values())}")
        return v

    @validator('priority')
    def validate_priority(cls, v):
        if v not in NOTIFICATION_PRIORITIES.values():
            raise ValueError(f"أولوية الإشعار غير صالحة. القيم المسموح بها: {', '.join(NOTIFICATION_PRIORITIES.values())}")
        return v
    ...
    def validate_channels(cls, v):
        for channel in v:
            if channel not in NOTIFICATION_CHANNELS.values():
                raise ValueError(f"قناة الإشعار غير صالحة. القيم المسموح بها: {', '.join(NOTIFICATION_CHANNELS.values())}")
        return v
...
class NotificationPreferenceCreateSchema(BaseModel):
    ...
    user_id: int
    notification_type: str = Field(..., description=NOTIFICATION_TYPE_DESCRIPTION)
...
class ScheduledNotificationCreateSchema(BaseModel):
    ...
    type: str = Field(..., description=NOTIFICATION_TYPE_DESCRIPTION)
    priority: str = Field(..., description="أولوية الإشعار")
    channels: List[str] = Field(..., description=NOTIFICATION_CHANNELS_DESCRIPTION)
    metadata: Optional[Dict[str, Any]] = None
...
class NotificationBulkActionSchema(BaseModel):
    ...
    def validate_action(cls, v):
        allowed_actions = ['mark_read', 'mark_unread', 'archive', 'unarchive', 'delete']
        if v not in allowed_actions:
            raise ValueError(f"الإجراء غير صالح. القيم المسموح بها: {', '.join(allowed_actions)}")
        return v
