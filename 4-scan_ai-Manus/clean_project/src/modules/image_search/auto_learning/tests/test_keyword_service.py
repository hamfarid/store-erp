...
from modules.image_search.auto_learning.keyword_management.models import Keyword, KeywordRelation
from modules.image_search.auto_learning.keyword_management.schemas import KeywordCreate, KeywordUpdate, KeywordRelationCreate
...
class TestKeywordService:
    ...
    def test_add_keyword_relation(self, keyword_service, db_session):
        ...
        db_session.query.side_effect = [
            MagicMock(filter=MagicMock(return_value=MagicMock(first=MagicMock(return_value=mock_source)))),
            MagicMock(filter=MagicMock(return_value=MagicMock(first=MagicMock(return_value=mock_target)))),
        ...
        with patch('modules.image_search.auto_learning.keyword_management.service.KeywordRelation', return_value=mock_relation):
            result = keyword_service.add_keyword_relation(source_id, relation_data)
    ...
    def test_add_keyword_relation_source_not_found(self, keyword_service, db_session):
        """
    ...
    def test_get_keywords(self, keyword_service, db_session):
        ...
        # تنفيذ الاختبار
        result = keyword_service.get_keywords(skip, limit, category, plant_part, search)
