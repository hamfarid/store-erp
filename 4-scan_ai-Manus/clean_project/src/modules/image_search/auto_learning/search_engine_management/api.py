...
from modules.image_search.auto_learning.utils.constants import ERROR_MESSAGES, SUCCESS_MESSAGES, DEFAULT_VALUES
...
async def get_search_engines(
    ...
    """
    try:
        return service.get_search_engines(skip, limit, engine_type, active, search)
    except Exception as exc:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=ERROR_MESSAGES["GENERAL"]["INTERNAL_ERROR"],
...
async def get_search_engine_stats(
    ...
    """
    try:
        stats = service.get_search_engine_stats(engine_id, start_date, end_date)
        if not stats:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=ERROR_MESSAGES["SEARCH_ENGINE"]["NOT_FOUND"],
...
async def update_load_balancing_strategy(
    ...
    try:
        ...
        return {"message": SUCCESS_MESSAGES["SEARCH_ENGINE"]["UPDATED"], "data": result}
    except Exception as exc:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=ERROR_MESSAGES["GENERAL"]["INTERNAL_ERROR"],
