...
from .config import MEMORY_API_URL, MEMORY_API_TIMEOUT
...
class MemoryIntegration:
    ...
    def store_keyword_data(self, keyword_id: int, keyword_data: Dict[str, Any]) -> bool:
        """
    ...
    def store_source_data(self, source_id: int, source_data: Dict[str, Any]) -> bool:
        """
    ...
    def retrieve_keyword_data(
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                logger.info("تم استرجاع بيانات الكلمة المفتاحية من الذاكرة المركزية بنجاح")
                return data
            else:
                logger.error("فشل استرجاع بيانات الكلمة المفتاحية من الذاكرة المركزية")
                return None
    ...
    def retrieve_source_data(
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                logger.info("تم استرجاع بيانات المصدر من الذاكرة المركزية بنجاح")
                return data
            else:
            ...
            logger.error(f"خطأ في استرجاع بيانات المصدر من الذاكرة المركزية: {str(e)}")
            return None
    ...
    def retrieve_search_engine_data(
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                logger.info("تم استرجاع بيانات محرك البحث من الذاكرة المركزية بنجاح")
                return data
            else:
                logger.error("فشل استرجاع بيانات محرك البحث من الذاكرة المركزية")
                return None
    ...
    def retrieve_similar_searches(
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                logger.info(f"تم استرجاع عمليات بحث مماثلة لاستعلام '{query}' بنجاح")
                return data if isinstance(data, list) else []
            else:
                logger.error(f"فشل استرجاع عمليات بحث مماثلة لاستعلام '{query}'")
                return []
    ...
    def update_entity_data(
        self, entity_type: str, entity_id: int, data: Dict[str, Any], event_type: str
    ) -> bool:
        """
        ...
        try:
            ...
            response = requests.put(
                f"{self.api_url}/update", json=memory_data, timeout=self.timeout
            )
