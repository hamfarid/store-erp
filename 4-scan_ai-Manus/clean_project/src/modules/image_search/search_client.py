...
try:
    ...
    logging.warning("Core settings module not found. API keys must be handled manually or via environment variables.")
    settings = None
...
SEARCH_ENGINE_ID = os.getenv("IMAGE_SEARCH_ENGINE_ID", "YOUR_CX_OR_ENDPOINT_ID_HERE")  # مثال: معرف Google Custom Search CX
SEARCH_API_ENDPOINT = os.getenv("IMAGE_SEARCH_API_ENDPOINT", "https://www.googleapis.com/customsearch/v1")  # مثال: نقطة نهاية Google Custom Search API
...
class WebSearchClient:
    ...
    def __init__(self, api_key: Optional[str] = None, engine_id: Optional[str] = None, endpoint: Optional[str] = None):
        """تهيئة عميل البحث.
        ...
        if settings:
            self.api_key = api_key or settings.get("SEARCH_API_KEY", SEARCH_API_KEY)
            self.engine_id = engine_id or settings.get("SEARCH_ENGINE_ID", SEARCH_ENGINE_ID)
            self.endpoint = endpoint or settings.get("SEARCH_API_ENDPOINT", SEARCH_API_ENDPOINT)
        else:
        ...
        if self.api_key == "YOUR_API_KEY_HERE":
            logger.warning("مفتاح API البحث غير مكوّن. ستكون وظائف البحث محدودة أو محاكاة.")
            # إجبار المحاكاة إذا كان المفتاح مفقودًا
            global USE_SIMULATION_FALLBACK
            USE_SIMULATION_FALLBACK = True

    def search_images(self, query: str, count: int = 10, start_index: int = 1, **kwargs) -> List[str]:
        ...
        logger.info(f"إجراء بحث صور حقيقي عن: '{query}' (العدد={count}، البداية={start_index})")
        results = []
        num_fetched = 0
        current_start = start_index

        # غالبًا ما تعيد واجهات API بحد أقصى 10 نتائج لكل صفحة، لذا قم بالتكرار إذا كان هناك حاجة إلى المزيد
        while num_fetched < count:
            results_per_page = min(10, count - num_fetched)  # بحد أقصى 10 لكل صفحة Google CSE
            params = {
                "key": self.api_key,
                "cx": self.engine_id,
                "q": query,
            ...
            try:
                ...
                if "items" in data:
                    page_results = [item.get("link") for item in data["items"] if item.get("link")]
                    results.extend(page_results)
                    num_fetched += len(page_results)
                    logger.debug(f"تم جلب {len(page_results)} عناوين URL للصور من الصفحة التي تبدأ عند {current_start}.")

                    # التحقق مما إذا كانت هناك المزيد من النتائج المتاحة (خاص بهيكل استجابة API)
                    if "queries" in data and "nextPage" in data["queries"]:
                        current_start = data["queries"]["nextPage"][0]["startIndex"]
                    else:
                        logger.info("لا توجد المزيد من الصفحات المتاحة وفقًا لاستجابة API.")
                        break  # لا مزيد من الصفحات
                else:
                    logger.info(f"لم يتم العثور على نتائج صور للاستعلام '{query}' بدءًا من الفهرس {current_start}.")
                ...
                logger.error(f"حدث مهلة أثناء البحث عن الصور للاستعلام: {query}")
                break
            except requests.exceptions.RequestException as e:
                logger.error(f"خطأ أثناء طلب البحث عن الصور للاستعلام '{query}': {e}")
                # تسجيل نص الاستجابة إذا كان مفيدًا وليس كبيرًا جدًا
                if hasattr(e, 'response') and e.response is not None:
                    logger.error(f"حالة الاستجابة: {e.response.status_code}, النص: {e.response.text[:500]}")
                break
            except Exception as e:
                logger.error(f"خطأ غير متوقع أثناء تحليل نتائج البحث للاستعلام '{query}': {e}", exc_info=True)
                break

        logger.info(f"انتهى البحث عن '{query}'. تم العثور على {len(results)} إجمالي عناوين URL للصور.")
        return results[:count]  # إرجاع فقط حتى العدد المطلوب

    def _simulate_search_images(self, query: str, count: int = 10) -> List[str]:
        ...
        for i in range(count):
            ...
            safe_query = query.replace(' ', '_').replace('/', '_')  # تنظيف أساسي
    ...
    def search_images_by_disease(self, disease_name: str, count: int = 10, **kwargs) -> List[str]:
    ...
    def search_images_by_pest(self, pest_name: str, count: int = 10, **kwargs) -> List[str]:
    ...
    def search_images_by_crop(self, crop_name: str, condition: str = None, count: int = 10, **kwargs) -> List[str]:
