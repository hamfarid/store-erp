...
class PerformanceAnalyzer:
    ...
    def analyze_performance_trend(
        ...
        try:
            ...
            if not metrics or len(metrics) < self.config["trend_analysis"]["min_data_points"]:
                logger.warning(
            ...
            if metric_type == "system":
                main_metrics = ["cpu.total_percent", "memory.percent", "disk.percent"]
            elif metric_type == "module":
                main_metrics = ["performance.response_time_ms", "performance.error_rate", "resources.cpu_percent"]
            elif metric_type == "ai":
                main_metrics = ["performance.inference_time_ms", "performance.accuracy", "resources.gpu_percent"]
            elif metric_type == "database":
                main_metrics = ["performance.query_time_ms", "performance.error_rate", "resources.connection_usage_percent"]
            else:
                main_metrics = []
            ...
            if not target_metric:
                logger.warning(f"No suitable metrics found for trend analysis of {metric_type}.")
                return None
    ...
    def detect_performance_anomalies(
        ...
        try:
            ...
            metrics = self.data_store.get_metrics(
                metric_type, entity_id, time_range)
            if not metrics or len(metrics) < self.config["anomaly_detection"]["min_data_points"]:
                logger.warning(
    ...
    def identify_low_performing_modules(
            self, threshold: float = None, time_range: str = "1d") -> List[str]:
        """
        ...
        try:
            ...
            for module_id in active_modules:
                ...
                metrics = self.data_store.get_metrics("module", module_id, time_range)
                if not metrics:
                    continue
    ...
    def identify_resource_intensive_modules(
        ...
        try:
            ...
            resource_intensive_modules.sort(
                key=lambda module_id: self._get_average_resource_usage(module_id, resource_key, time_range),
                reverse=True
            )
        ...
        try:
            ...
            if entity_id is None:
                ...
                for module_id in low_performing_modules:
                    module_recommendations = self._generate_module_recommendations(module_id)
                ...
                for ai_model_id in ai_models:
                    ai_recommendations = self._generate_ai_recommendations(ai_model_id)
                    recommendations.extend(ai_recommendations)

                # توصيات لقواعد البيانات
                databases = self.data_store.get_active_entities("database")
                for db_name in databases:
                    db_recommendations = self._generate_database_recommendations(db_name)
                ...
                if entity_type == "module":
                    recommendations = self._generate_module_recommendations(entity_id)
                elif entity_type == "ai":
                    recommendations = self._generate_ai_recommendations(entity_id)
                elif entity_type == "database":
                    recommendations = self._generate_database_recommendations(entity_id)
                else:
            ...
            if max_recommendations > 0 and len(recommendations) > max_recommendations:
    ...
    def _generate_module_recommendations(self, module_id: str) -> List[Recommendation]:
        """
    ...
    def _generate_ai_recommendations(self, ai_model_id: str) -> List[Recommendation]:
        """
    ...
    def _generate_database_recommendations(self, db_name: str) -> List[Recommendation]:
        """
        ...
        if "performance.active_connections" in df.columns:
            ...
            max_connections = df["resources.connection_limit"].mean() if "resources.connection_limit" in df.columns else 100
    ...
    def detect_data_drift(self, reference_data, current_data, column_mapping=None):
        """
        ...
        if not EVIDENTLY_AVAILABLE:
            logger.warning("Evidently library not available. Cannot detect data drift.")
            return None
    ...
    def detect_target_drift(self, reference_data, current_data, column_mapping=None):
        """
        ...
        """
        if not EVIDENTLY_AVAILABLE:
            logger.warning("Evidently library not available. Cannot detect target drift.")
            return None
