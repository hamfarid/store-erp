...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("settings_api.log"), logging.StreamHandler()],
)
...
async def update_general_settings(
    ...
    try:
        ...
        valid, message = settings_service.validate_settings("general", settings)
        if not valid:
        ...
        raise HTTPException(
            status_code=500, detail=f"حدث خطأ أثناء تحديث الإعدادات العامة: {str(e)}"
        )
...
async def get_company_settings(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"حدث خطأ أثناء الحصول على إعدادات الشركة: {str(e)}"
        )
...
async def update_company_settings(
    ...
    try:
        # التحقق من صحة الإعدادات
        valid, message = settings_service.validate_settings("company", settings)
        if not valid:
            raise HTTPException(status_code=400, detail=message)
        ...
        raise HTTPException(
            status_code=500, detail=f"حدث خطأ أثناء تحديث إعدادات الشركة: {str(e)}"
        )
...
async def get_system_settings(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"حدث خطأ أثناء الحصول على إعدادات النظام: {str(e)}"
        )
...
async def update_system_settings(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"حدث خطأ أثناء تحديث إعدادات النظام: {str(e)}"
        )


@router.get("/connection")
async def get_connection_settings(current_user: dict = Depends(get_current_user)):
    """
...
async def update_connection_settings(
    ...
    try:
        # التحقق من صحة الإعدادات
        valid, message = settings_service.validate_settings("connection", settings)
        if not valid:
            raise HTTPException(status_code=400, detail=message)

        # تحديث إعدادات الاتصال
        updated_settings = settings_service.update_connection_settings(settings)
        ...
        raise HTTPException(
            status_code=500, detail=f"حدث خطأ أثناء تحديث إعدادات الاتصال: {str(e)}"
        )
...
async def get_all_settings(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"حدث خطأ أثناء الحصول على جميع الإعدادات: {str(e)}"
        )
...
async def reset_settings(
    ...
    if settings_type not in ["general", "company", "system", "connection", "all"]:
        raise HTTPException(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"حدث خطأ أثناء إعادة تعيين الإعدادات: {str(e)}"
        )
...
async def download_exported_settings(
    ...
    if not os.path.exists(file_path):
        raise HTTPException(status_code=404, detail=f"الملف غير موجود: {file_name}")
...
async def import_settings(
    ...
    try:
        ...
        # إخفاء كلمات المرور وبيانات الاعتماد الحساسة
        if settings_type == "connection":
            imported_settings = _mask_sensitive_data(imported_settings, "connection")
        ...
        raise HTTPException(
            status_code=500, detail=f"حدث خطأ أثناء استيراد الإعدادات: {str(e)}"
        )
    finally:
