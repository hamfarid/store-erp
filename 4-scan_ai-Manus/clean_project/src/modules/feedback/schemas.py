...
from .config import FeedbackType, FeedbackPriority, FeedbackStatus, FEEDBACK_SETTINGS


class FeedbackBase(BaseModel):
    """النموذج الأساسي للتغذية الراجعة"""
    title: str = Field(..., min_length=3, max_length=FEEDBACK_SETTINGS["max_feedback_title_length"])
    description: str = Field(..., min_length=10, max_length=FEEDBACK_SETTINGS["max_feedback_description_length"])
    ...
    def title_must_be_valid(cls, v):
        if not re.match(r'^[\u0600-\u06FFa-zA-Z0-9\s\-_.,!?()]+$', v):
            raise ValueError('العنوان يجب أن يحتوي على أحرف وأرقام وعلامات ترقيم فقط')
        return v
...
class FeedbackUpdate(BaseModel):
    ...
    title: Optional[str] = Field(None, min_length=3, max_length=FEEDBACK_SETTINGS["max_feedback_title_length"])
    description: Optional[str] = Field(None, min_length=10, max_length=FEEDBACK_SETTINGS["max_feedback_description_length"])
    ...
    @validator('title')
    def title_must_be_valid(cls, v):
        if v is not None and not re.match(r'^[\u0600-\u06FFa-zA-Z0-9\s\-_.,!?()]+$', v):
            raise ValueError('العنوان يجب أن يحتوي على أحرف وأرقام وعلامات ترقيم فقط')
        return v
...
class CommentBase(BaseModel):
    ...
    content: str = Field(..., min_length=1, max_length=FEEDBACK_SETTINGS["max_comment_length"])
...
class TagBase(BaseModel):
    ...
    color: Optional[str] = Field(None, regex=r'^#[0-9A-Fa-f]{6}$')  # لون بصيغة HEX (#RRGGBB)
