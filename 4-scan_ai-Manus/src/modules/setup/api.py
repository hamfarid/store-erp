...
async def get_setup_status(db: Session = Depends(get_db)):
    ...
    try:
        ...
        activity_log_service.log_activity(
            db, "setup", "error", "فشل في الحصول على حالة الإعداد", {"error": str(e)}
        )
        raise HTTPException(
...
async def initialize_setup(db: Session = Depends(get_db)):
    ...
    try:
        ...
        activity_log_service.log_activity(
            db, "setup", "error", "فشل في تهيئة عملية الإعداد", {"error": str(e)}
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def update_step_data(
    step_id: str, step_data: Dict[str, Any] = Body(...), db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        validation_result = setup_service.validate_step_data(db, step_id, step_data)
...
async def previous_step(db: Session = Depends(get_db)):
    ...
    try:
        ...
        activity_log_service.log_activity(
            db, "setup", "error", "فشل في العودة إلى الخطوة السابقة", {"error": str(e)}
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def complete_setup(db: Session = Depends(get_db)):
    ...
    try:
        ...
        activity_log_service.log_activity(
            db, "setup", "error", "فشل في إكمال عملية الإعداد", {"error": str(e)}
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.post(
    "/test-database-connection", response_model=schemas.DatabaseConnectionTestResponse
)
...
@router.post("/test-email-settings", response_model=schemas.EmailSettingsTestResponse)
async def test_email_settings(
    email_settings: schemas.EmailSettingsTest = Body(...), db: Session = Depends(get_db)
):
    """
...
async def get_countries(db: Session = Depends(get_db)):
    ...
    try:
        ...
        activity_log_service.log_activity(
            db, "setup", "error", "فشل في الحصول على قائمة الدول", {"error": str(e)}
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_currencies(db: Session = Depends(get_db)):
    ...
    try:
        ...
        activity_log_service.log_activity(
            db, "setup", "error", "فشل في الحصول على قائمة العملات", {"error": str(e)}
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_languages(db: Session = Depends(get_db)):
    ...
    try:
        ...
        activity_log_service.log_activity(
            db, "setup", "error", "فشل في الحصول على قائمة اللغات", {"error": str(e)}
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def reset_setup(db: Session = Depends(get_db)):
    ...
    try:
        ...
        activity_log_service.log_activity(
            db, "setup", "error", "فشل في إعادة تعيين عملية الإعداد", {"error": str(e)}
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.post(
    "/validate-security-settings", response_model=schemas.SecurityValidationResponse
)
...
async def get_setup_logs(db: Session = Depends(get_db)):
    ...
    try:
        ...
        activity_log_service.log_activity(
            db, "setup", "error", "فشل في الحصول على سجلات الإعداد", {"error": str(e)}
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
