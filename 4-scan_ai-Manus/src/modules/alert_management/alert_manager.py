# File: /home/ubuntu/ai_web_organized/src/modules/alert_management/alert_manager.py
"""
from flask import g
...
def create_alert(alert_type, severity, module, message_ar, message_en, details=None):
    """إنشاء تنبيه جديد"""
    try:
...
def get_alerts(status=None, severity=None, module=None, start_date=None, end_date=None, limit=None):
    ...
    try:
        ...
        if status:
            alerts["alerts"] = [alert for alert in alerts["alerts"] if alert["status"] == status]

        # تصفية التنبيهات حسب الخطورة
        if severity:
            alerts["alerts"] = [alert for alert in alerts["alerts"] if alert["severity"] == severity]

        # تصفية التنبيهات حسب المديول
        if module:
            alerts["alerts"] = [alert for alert in alerts["alerts"] if alert["module"] == module]

        # تصفية التنبيهات حسب التاريخ
        if start_date:
            alerts["alerts"] = [alert for alert in alerts["alerts"] if alert["timestamp"] >= start_date]

        if end_date:
            alerts["alerts"] = [alert for alert in alerts["alerts"] if alert["timestamp"] <= end_date]

        # ترتيب التنبيهات حسب التاريخ (الأحدث أولاً)
        alerts["alerts"] = sorted(alerts["alerts"], key=lambda x: x["timestamp"], reverse=True)
...
def send_alert_notifications(alert):
    ...
    try:
        ...
        if not isinstance(config, dict):
            logger.warning("Config is not a dictionary, skipping notifications")
            return False
        ...
        if alert_level.get("email", False) and notifications_config.get("email", False):
            send_email_notification(alert, config)

        # إرسال إشعار بالرسائل القصيرة
        if alert_level.get("sms", False) and notifications_config.get("sms", False):
            send_sms_notification(alert, config)

        # إرسال إشعار بالدفع
        if alert_level.get("push", False) and notifications_config.get("push", False):
            send_push_notification(alert, config)

        # إرسال إشعار بالويب هوك
        if alert_level.get("webhook", False) and notifications_config.get("webhook", False):
...
def send_sms_notification(alert, config):
    """إرسال إشعار بالرسائل القصيرة"""
    # ملاحظة: هذه دالة تجريبية، في التطبيق الحقيقي يجب استخدام خدمة رسائل قصيرة مثل Twilio
    try:
        # التحقق من وجود مستلمين
        ...
        # تسجيل الإشعار
        logger.info(f"SMS notification would be sent to {recipients}: {message}")
...
def send_push_notification(alert, config):
    """إرسال إشعار بالدفع"""
    # ملاحظة: هذه دالة تجريبية، في التطبيق الحقيقي يجب استخدام خدمة إشعارات مثل Firebase Cloud Messaging
    try:
        # التحقق من وجود مستلمين
        ...
        # تسجيل الإشعار
        logger.info(f"Push notification would be sent to {recipients}: {notification}")
...
class AlertMonitor:
    ...
    def start(self):
        ...
        self.thread.start()
        logger.info(f"Alert monitor started with interval {self.interval} seconds")
    ...
    def _monitor_resources(self):
        ...
        while self.running:
            try:
                ...
                config = get_alerts_config()
                if not isinstance(config, dict):
                    logger.warning("Config is not a dictionary, skipping monitoring")
                    continue
                ...
                try:
                    ...
                    from ..resource_monitoring.resource_collector import get_cpu_usage, get_memory_usage, get_disk_usage, get_network_usage
