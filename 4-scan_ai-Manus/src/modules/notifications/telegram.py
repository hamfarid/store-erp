...
class TelegramNotificationService:
    ...
    def __init__(self):
        ...
        self.admin_chat_ids = self._parse_admin_chat_ids(os.getenv('TELEGRAM_ADMIN_CHAT_IDS', ''))
        ...
        if self.enabled and (not self.bot_token or not self.default_chat_id):
            logger.warning("تم تفعيل إشعارات تيليجرام لكن لم يتم توفير رمز البوت أو معرف المحادثة الافتراضي")
    ...
    def _parse_admin_chat_ids(self, admin_chat_ids_str: str) -> List[str]:
        ...
        return [chat_id.strip() for chat_id in admin_chat_ids_str.split(',') if chat_id.strip()]
    ...
    def send_message(self,
        ...
        if not target_chat_id:
            raise ValueError("لم يتم توفير معرف المحادثة ولا يوجد معرف افتراضي")
...
"""
    ...
    def send_admin_notification(self,
        ...
        for admin_chat_id in self.admin_chat_ids:
            try:
                ...
                logger.error(f"فشل إرسال إشعار للمسؤول {admin_chat_id}: {str(e)}")
    ...
    def send_user_blocked_alert(self,
                                ...
                                failed_attempts: int,
                                timestamp: str,
                                additional_info: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        """
