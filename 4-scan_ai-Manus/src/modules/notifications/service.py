...
from src.modules.notifications.models import (
    Notification, NotificationChannel, NotificationTemplate, NotificationPreference,
    ScheduledNotification, WebhookSubscription
)
from src.modules.notifications.schemas import (
    NotificationCreateSchema, NotificationSchema, NotificationTemplateCreateSchema,
    NotificationTemplateSchema, NotificationPreferenceCreateSchema, NotificationPreferenceSchema,
    ScheduledNotificationCreateSchema, ScheduledNotificationSchema, WebhookSubscriptionCreateSchema,
    WebhookSubscriptionSchema, NotificationBulkActionSchema, NotificationFilterSchema,
    NotificationSendTestSchema
)
from src.modules.notifications.config import (
    NOTIFICATIONS_CONFIG, NOTIFICATION_TYPES, NOTIFICATION_PRIORITIES, NOTIFICATION_CHANNELS
)
...
class NotificationService:
    ...
    def create_notification(self, data: NotificationCreateSchema) -> NotificationSchema:
        """
        إنشاء إشعار جديد
        ...
        try:
            ...
            if data.user_id:
                ...
                if not user:
                    raise ValueError(f"المستخدم بالمعرف {data.user_id} غير موجود")
    ...
    def get_notifications(self, filters: NotificationFilterSchema, skip: int = 0, limit: int = 100) -> List[NotificationSchema]:
        """
    ...
    def get_notification_by_id(self, notification_id: int) -> Optional[NotificationSchema]:
        """
        ...
        try:
            ...
            return NotificationSchema.from_orm(notification) if notification else None
        except SQLAlchemyError as e:
            logger.error(f"خطأ في الحصول على الإشعار بالمعرف {notification_id}: {str(e)}")
            raise

    def mark_notification_as_read(self, notification_id: int) -> Optional[NotificationSchema]:
        """
        تحديد إشعار كمقروء
    ...
    def mark_notification_as_unread(self, notification_id: int) -> Optional[NotificationSchema]:
        """
        ...
        try:
            ...
            logger.error(f"خطأ في تحديد الإشعار {notification_id} كغير مقروء: {str(e)}")
            self.db.rollback()
            raise

    def archive_notification(self, notification_id: int) -> Optional[NotificationSchema]:
        """
        أرشفة إشعار
    ...
    def unarchive_notification(self, notification_id: int) -> Optional[NotificationSchema]:
        """
        إلغاء أرشفة إشعار
        ...
        try:
            ...
            logger.error(f"خطأ في إلغاء أرشفة الإشعار {notification_id}: {str(e)}")
            self.db.rollback()
            raise
    ...
    def bulk_action(self, data: NotificationBulkActionSchema) -> Dict[str, int]:
        """
        ...
        try:
            affected_count = 0
            query = self.db.query(Notification).filter(Notification.id.in_(data.notification_ids))

            if data.action == "mark_read":
                affected_count = query.update({"is_read": True, "updated_at": datetime.utcnow()}, synchronize_session=False)
            elif data.action == "mark_unread":
                affected_count = query.update({"is_read": False, "updated_at": datetime.utcnow()}, synchronize_session=False)
            elif data.action == "archive":
                affected_count = query.update({"is_archived": True, "updated_at": datetime.utcnow()}, synchronize_session=False)
            elif data.action == "unarchive":
                affected_count = query.update({"is_archived": False, "updated_at": datetime.utcnow()}, synchronize_session=False)
            ...
            logger.error(f"خطأ في تنفيذ الإجراء الجماعي ({data.action}): {str(e)}")
            self.db.rollback()
            raise
    ...
    def send_notification(self, notification_id: int) -> None:
        ...
        try:
            notification = self.db.query(Notification).filter(Notification.id == notification_id).first()
            if not notification:
                logger.warning(f"الإشعار بالمعرف {notification_id} غير موجود للإرسال")
                return
            ...
            for channel in channels:
                try:
                    if channel.channel == NOTIFICATION_CHANNELS["EMAIL"] and NOTIFICATIONS_CONFIG["EMAIL_NOTIFICATIONS_ENABLED"]:
                    ...
                    logger.error(f"خطأ في إرسال الإشعار {notification_id} عبر قناة {channel.channel}: {str(e)}")
    ...
    def _send_email_notification(self, notification: Notification, channel: NotificationChannel) -> None:
        """
        ...
        logger.info(f"إرسال بريد إلكتروني إلى {notification.user.email} بعنوان: {subject}")

        channel.status = "sent"

    def _send_in_app_notification(self, notification: Notification, channel: NotificationChannel) -> None:
        """
        ...
        المعلمات:
            ...
            channel: كائن قناة الإشعار
        """
        logger.info(f"إشعار داخل النظام للمستخدم {notification.user_id}: {notification.title}")
        channel.status = "sent"

    def _send_sms_notification(self, notification: Notification, channel: NotificationChannel) -> None:
        """
        ...
        logger.info(f"إرسال رسالة قصيرة إلى {notification.user.phone}: {sms_content}")

        channel.status = "sent"

    def _send_push_notification(self, notification: Notification, channel: NotificationChannel) -> None:
        """
        ...
        logger.info(f"إرسال إشعار Push إلى {device_token}: {notification.title}")

        channel.status = "sent"

    def _send_webhook_notification(self, notification: Notification, channel: NotificationChannel) -> None:
        """
    ...
    def create_template(self, data: NotificationTemplateCreateSchema) -> NotificationTemplateSchema:
        """
    ...
    def get_templates(self, skip: int = 0, limit: int = 100) -> List[NotificationTemplateSchema]:
        """
        ...
        """
        try:
            templates = self.db.query(NotificationTemplate).offset(skip).limit(limit).all()
            return [NotificationTemplateSchema.from_orm(t) for t in templates]
        except SQLAlchemyError as e:
            logger.error(f"خطأ في الحصول على قوالب الإشعارات: {str(e)}")
            raise

    def get_template_by_id(self, template_id: int) -> Optional[NotificationTemplateSchema]:
        """
        ...
        try:
            ...
            return NotificationTemplateSchema.from_orm(template) if template else None
        except SQLAlchemyError as e:
            logger.error(f"خطأ في الحصول على القالب بالمعرف {template_id}: {str(e)}")
            raise

    def get_template_by_code(self, template_code: str) -> Optional[NotificationTemplateSchema]:
        """
        ...
        """
        try:
            template = self.db.query(NotificationTemplate).filter(NotificationTemplate.code == template_code).first()
            return NotificationTemplateSchema.from_orm(template) if template else None
        except SQLAlchemyError as e:
            logger.error(f"خطأ في الحصول على القالب بالرمز {template_code}: {str(e)}")
            raise

    def update_template(self, template_id: int, data: NotificationTemplateCreateSchema) -> Optional[NotificationTemplateSchema]:
        """
        تحديث قالب إشعار
    ...
    def get_user_preferences(self, user_id: int) -> List[NotificationPreferenceSchema]:
        """
        ...
        """
        try:
            preferences = self.db.query(NotificationPreference).filter(NotificationPreference.user_id == user_id).all()
            return [NotificationPreferenceSchema.from_orm(p) for p in preferences]
        except SQLAlchemyError as e:
            logger.error(f"خطأ في الحصول على تفضيلات المستخدم {user_id}: {str(e)}")
            raise

    def update_user_preference(self, user_id: int, data: NotificationPreferenceCreateSchema) -> NotificationPreferenceSchema:
        """
    ...
    def create_scheduled_notification(self, data: ScheduledNotificationCreateSchema) -> ScheduledNotificationSchema:
        """
        إنشاء إشعار مجدول
    ...
    def process_scheduled_notifications(self) -> int:
        ...
        try:
            ...
            scheduled_notifications = self.db.query(ScheduledNotification).filter(
                ...
                ScheduledNotification.is_processed.is_(False)
            ).all()
            ...
            for sn in scheduled_notifications:
                try:
                    ...
                    logger.error(f"خطأ في معالجة الإشعار المجدول {sn.id}: {str(e)}")
    ...
    def create_webhook_subscription(self, data: WebhookSubscriptionCreateSchema) -> WebhookSubscriptionSchema:
        """
    ...
    def get_webhook_subscriptions(self, skip: int = 0, limit: int = 100) -> List[WebhookSubscriptionSchema]:
        """
        ...
        """
        try:
            subscriptions = self.db.query(WebhookSubscription).offset(skip).limit(limit).all()
            return [WebhookSubscriptionSchema.from_orm(s) for s in subscriptions]
        except SQLAlchemyError as e:
            logger.error(f"خطأ في الحصول على اشتراكات Webhook: {str(e)}")
            raise

    def update_webhook_subscription(self, subscription_id: int, data: WebhookSubscriptionCreateSchema) -> Optional[WebhookSubscriptionSchema]:
        """
        ...
        """
        try:
            subscription = self.db.query(WebhookSubscription).filter(WebhookSubscription.id == subscription_id).first()
            if subscription:
                for key, value in data.dict(exclude_unset=True).items():
            ...
            logger.error(f"خطأ في تحديث اشتراك Webhook {subscription_id}: {str(e)}")
            self.db.rollback()
            raise
    ...
    def delete_webhook_subscription(self, subscription_id: int) -> bool:
        ...
        try:
            ...
            logger.error(f"خطأ في حذف اشتراك Webhook {subscription_id}: {str(e)}")
            self.db.rollback()
            raise

    def send_test_notification(self, data: NotificationSendTestSchema) -> Dict[str, Any]:
        """
        إرسال إشعار اختباري
        ...
        for channel_code in data.channels:
            try:
                ...
                if channel_code == NOTIFICATION_CHANNELS["EMAIL"] and data.email:
                    self._send_email_notification(dummy_notification, dummy_channel)
                elif channel_code == NOTIFICATION_CHANNELS["SMS"] and data.phone:
                    self._send_sms_notification(dummy_notification, dummy_channel)
                ...
                results[channel_code] = {"status": dummy_channel.status, "error": dummy_channel.error}
            except Exception as e:
                logger.error(f"خطأ في إرسال إشعار اختباري عبر قناة {channel_code}: {str(e)}")
