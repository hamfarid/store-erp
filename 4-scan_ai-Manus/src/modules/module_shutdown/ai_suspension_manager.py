...
class AISuspensionManager:
    ...
    def register_ai_module(self, ai_module_id: str, name: str = None) -> bool:
        ...
        with self.lock:
            ...
            ai_modules[ai_module_id] = {
                ...
                "auto_resume": True,
                "default_suspension_time": self.config.get("default_suspension_time", 300)
            }
    ...
    def _send_suspension_notification(self, record: Dict[str, Any]) -> None:
        ...
        if email:
            try:
                ...
                logger.info(f"إرسال إشعار التعليق عبر البريد الإلكتروني إلى {email}")
            except Exception as e:
                logger.error(f"فشل إرسال إشعار التعليق عبر البريد الإلكتروني: {str(e)}")
    ...
    def update_auto_resume(self, ai_module_id: str, auto_resume: bool) -> bool:
        ...
        with self.lock:
            ...
            if result:
                logger.info(f"تم تحديث الاستئناف التلقائي لمديول الذكاء الصناعي {ai_module_id}: {auto_resume}")
    ...
    def update_default_suspension_time(self, ai_module_id: str, default_suspension_time: int) -> bool:
        ...
        with self.lock:
            ...
            if result:
                logger.info(f"تم تحديث وقت التعليق الافتراضي لمديول الذكاء الصناعي {ai_module_id}: {default_suspension_time}")
    ...
    def get_suspension_stats(self, ai_module_id: str = None) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "min_suspension_time": timedelta(0) if len(self.ai_suspensions) > 0 else None,
            "suspension_reasons": {},
    ...
    def schedule_suspension(self, ai_module_id: str, suspend_at: datetime,
        ...
        with self.lock:
            ...
            if result:
                logger.info(f"تم جدولة تعليق مديول الذكاء الصناعي {ai_module_id} في {suspend_at}")
    ...
    def cancel_scheduled_suspension(self, ai_module_id: str, task_index: int = None) -> bool:
        ...
        with self.lock:
            ...
            if result:
                logger.info(f"تم إلغاء جدولة تعليق مديول الذكاء الصناعي {ai_module_id}")
