...
class PerformanceMonitoringInterface:
    ...
    def setup_routes(self):
        ...
        def index():
            """الصفحة الرئيسية"""
            return render_template('admin_panel/performance_monitoring/dashboard.html')
        ...
        def login():
            ...
            return render_template('admin_panel/performance_monitoring/login.html')
        ...
        def system_dashboard():
            ...
            return render_template('admin_panel/performance_monitoring/system.html')

        @app.route('/modules')
        def modules_dashboard():
            """لوحة معلومات أداء المديولات"""
            return render_template('admin_panel/performance_monitoring/modules.html')

        @app.route('/ai')
        def ai_dashboard():
            """لوحة معلومات أداء الذكاء الصناعي"""
            return render_template('admin_panel/performance_monitoring/ai.html')

        @app.route('/database')
        def database_dashboard():
            """لوحة معلومات أداء قواعد البيانات"""
            return render_template('admin_panel/performance_monitoring/database.html')

        @app.route('/reports')
        def reports_dashboard():
            """لوحة معلومات التقارير"""
            return render_template('admin_panel/performance_monitoring/reports.html')

        @app.route('/alerts')
        def alerts_dashboard():
            """لوحة معلومات التنبيهات"""
            return render_template('admin_panel/performance_monitoring/alerts.html')
                ...
                if system_metrics:
                    ...
                    if "cpu.total_percent" in latest_metrics:
                        ...
                        if cpu_percent >= cpu_thresholds["critical"]:
                            self._add_alert("system", "cpu", "critical", f"CPU usage is critical: {cpu_percent:.2f}%")
                        elif cpu_percent >= cpu_thresholds["warning"]:
                            self._add_alert("system", "cpu", "warning", f"CPU usage is high: {cpu_percent:.2f}%")
                    ...
                    if "memory.percent" in latest_metrics:
                        ...
                        if memory_percent >= memory_thresholds["critical"]:
                            self._add_alert("system", "memory", "critical", f"Memory usage is critical: {memory_percent:.2f}%")
                        elif memory_percent >= memory_thresholds["warning"]:
                            self._add_alert("system", "memory", "warning", f"Memory usage is high: {memory_percent:.2f}%")
                    ...
                    if "disk.percent" in latest_metrics:
                        ...
                        if disk_percent >= disk_thresholds["critical"]:
                            self._add_alert("system", "disk", "critical", f"Disk usage is critical: {disk_percent:.2f}%")
                        elif disk_percent >= disk_thresholds["warning"]:
                            self._add_alert("system", "disk", "warning", f"Disk usage is high: {disk_percent:.2f}%")
                    ...
                    if module_metrics:
                        ...
                        if "performance.error_rate" in latest_metrics:
                            ...
                            if error_rate >= error_thresholds["critical"]:
                                self._add_alert("module", module_id, "critical", f"Error rate for module {module_id} is critical: {error_rate:.2f}")
                            elif error_rate >= error_thresholds["warning"]:
                                self._add_alert("module", module_id, "warning", f"Error rate for module {module_id} is high: {error_rate:.2f}")
                        ...
                        if "resources.cpu_percent" in latest_metrics:
                            ...
                            if cpu_percent >= cpu_thresholds["critical"]:
                                self._add_alert("module", module_id, "critical", f"CPU usage for module {module_id} is critical: {cpu_percent:.2f}%")
                            elif cpu_percent >= cpu_thresholds["warning"]:
                                self._add_alert("module", module_id, "warning", f"CPU usage for module {module_id} is high: {cpu_percent:.2f}%")
                    ...
                    if ai_metrics:
                        ...
                        if "resources.gpu_percent" in latest_metrics:
                            ...
                            if gpu_percent >= gpu_thresholds["critical"]:
                                self._add_alert("ai", ai_model_id, "critical", f"GPU usage for AI model {ai_model_id} is critical: {gpu_percent:.2f}%")
                            elif gpu_percent >= gpu_thresholds["warning"]:
                                self._add_alert("ai", ai_model_id, "warning", f"GPU usage for AI model {ai_model_id} is high: {gpu_percent:.2f}%")
                ...
                for db_name in databases:
                    ...
                    if db_metrics:
                        ...
                        if "performance.error_rate" in latest_metrics:
                            ...
                            if error_rate >= error_thresholds["critical"]:
                                self._add_alert("database", db_name, "critical", f"Error rate for database {db_name} is critical: {error_rate:.2f}")
                            elif error_rate >= error_thresholds["warning"]:
                                self._add_alert("database", db_name, "warning", f"Error rate for database {db_name} is high: {error_rate:.2f}")
