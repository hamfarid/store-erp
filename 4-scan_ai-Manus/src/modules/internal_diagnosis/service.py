...
# Import logger
try:
    from src.modules.log_activity.activity_logger import actividad_logger  # Corrected import
...
try:
    from src.modules.training_pipeline import ModelRegistry, DataPreprocessor
except ImportError:
    logger.error(
...
class InternalDiagnosisService:
    ...
    def _initialize_db_kb_schema(self):
        ...
        try:
            ...
            logger.error(
                "Error initializing SQLite KB schema at %s: %s", self.kb_path, e,
                exc_info=True)
        finally:
            if conn:
                conn.close()
