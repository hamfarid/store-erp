# File: /home/ubuntu/ai_web_organized/src/modules/ai_usage_reports/report_generator.py
"""
...
class AIUsageReportGenerator:
    ...
    def __init__(self, data_dir=None):
        ...
        self.usage_data_file = os.path.join(self.data_dir, 'ai_usage_data.json')
    ...
    def _load_usage_data(self):
        ...
        try:
            ...
            logger.error("خطأ أثناء تحميل بيانات استخدام الذكاء الصناعي: %s", str(e))
            return []
    ...
    def _save_usage_data(self):
        ...
        try:
            ...
            logger.error("خطأ أثناء حفظ بيانات استخدام الذكاء الصناعي: %s", str(e))

    def add_usage_record(self, user_id, agent_id, question, answer, timestamp=None, module=None, is_within_scope=None):
        """
        ...
        try:
            ...
            # إضافة تحليل ما إذا كان السؤال والإجابة ضمن اختصاصات المستخدم والوكيل
            if is_within_scope is not None:
                usage_record["is_within_scope"] = is_within_scope
            else:
                # تحليل تلقائي (محاكاة)
                usage_record["is_within_scope"] = self._analyze_scope(user_id, agent_id, question, answer)
    ...
    def _analyze_scope(self, user_id, agent_id, question, answer):
        ...
        try:
            ...
            forbidden_words = ["كلمة_سر", "password", "خاص", "سري", "private", "secret"]
    ...
    def generate_daily_report(self, date=None):
        ...
        try:
            ...
            start_of_day = datetime(report_date.year, report_date.month, report_date.day).isoformat()
            end_of_day = (datetime(report_date.year, report_date.month, report_date.day) + timedelta(days=1) - timedelta(microseconds=1)).isoformat()

            # فلترة بيانات الاستخدام حسب التاريخ
            daily_usage = [record for record in self.usage_data if start_of_day <= record["timestamp"] <= end_of_day]
    ...
    def generate_weekly_report(self, end_date=None):
        ...
        try:
            ...
            start_of_week = (end_date - timedelta(days=end_date.weekday() + 7)).replace(hour=0, minute=0, second=0, microsecond=0).isoformat()
            end_of_week = (end_date - timedelta(days=end_date.weekday()) - timedelta(microseconds=1)).isoformat()

            # فلترة بيانات الاستخدام حسب التاريخ
            weekly_usage = [record for record in self.usage_data if start_of_week <= record["timestamp"] <= end_of_week]
    ...
    def generate_monthly_report(self, month=None, year=None):
        ...
        try:
            ...
            # فلترة بيانات الاستخدام حسب التاريخ
            monthly_usage = [record for record in self.usage_data if start_of_month <= record["timestamp"] <= end_of_month]
    ...
    def generate_user_report(self, user_id, start_date=None, end_date=None):
        ...
        try:
            ...
            if end_date:
                end_date = (datetime.strptime(end_date, "%Y-%m-%d") + timedelta(days=1) - timedelta(microseconds=1)).isoformat()
            else:
                end_date = datetime.now().isoformat()

            # فلترة بيانات الاستخدام حسب المستخدم والتاريخ
            user_usage = [record for record in self.usage_data if record["user_id"] == user_id and start_date <= record["timestamp"] <= end_date]
    ...
    def generate_agent_report(self, agent_id, start_date=None, end_date=None):
        ...
        try:
            ...
            agent_usage = [record for record in self.usage_data if record["agent_id"] == agent_id and start_date <= record["timestamp"] <= end_date]
    ...
    def generate_module_report(self, module, start_date=None, end_date=None):
        ...
        try:
            ...
            module_usage = [record for record in self.usage_data if record.get("module") == module and start_date <= record["timestamp"] <= end_date]
    ...
    def _calculate_within_scope_percentage(self, usage_data):
        ...
        try:
            ...
            records_with_scope_info = [record for record in usage_data if "is_within_scope" in record]

            if not records_with_scope_info:
                return 100.0

            # حساب عدد السجلات ضمن الاختصاصات
            within_scope_count = sum(1 for record in records_with_scope_info if record["is_within_scope"])

            # حساب النسبة المئوية
            return (within_scope_count / len(records_with_scope_info)) * 100

        except Exception as e:
            logger.error("خطأ أثناء حساب نسبة الأسئلة والإجابات ضمن الاختصاصات: %s", str(e))
            return 100.0
    ...
    def _calculate_module_distribution(self, usage_data):
        ...
        try:
            ...
            module_distribution = {module: (count / total_count) * 100 for module, count in module_counts.items()}
    ...
    def _calculate_agent_activity(self, usage_data):
        ...
        try:
            ...
            # تحويل إلى قاموس
            agent_activity = {agent_id: count for agent_id, count in sorted_agents}

            return agent_activity

        except Exception as e:
            logger.error("خطأ أثناء حساب نشاط وكلاء الذكاء الصناعي: %s", str(e))
            return {}
    ...
    def _extract_top_questions(self, usage_data, limit=10):
        ...
        try:
            ...
            top_questions_list = [{"question": question, "count": count} for question, count in top_questions]
    ...
    def _generate_visual_report(self, report, report_type):
        ...
        try:
            ...
            if "module_distribution" in report and report["module_distribution"]:
                ...
                # حفظ الرسم البياني
                module_chart_path = os.path.join(visual_reports_dir, f"{report_filename}_module_distribution.png")
                plt.savefig(module_chart_path)
                plt.close()

                logger.info("تم إنشاء الرسم البياني لتوزيع المديولات: %s", module_chart_path)

            # إنشاء رسم بياني لنشاط المستخدمين
            if "user_activity" in report and report["user_activity"]:
                plt.figure(figsize=(10, 6))
                users = list(report["user_activity"].keys())[:10]  # أخذ أول 10 مستخدمين
                ...
                # حفظ الرسم البياني
                user_chart_path = os.path.join(visual_reports_dir, f"{report_filename}_user_activity.png")
                plt.savefig(user_chart_path)
                plt.close()

                logger.info("تم إنشاء الرسم البياني لنشاط المستخدمين: %s", user_chart_path)
            ...
            if "agent_activity" in report and report["agent_activity"]:
                ...
                # حفظ الرسم البياني
                agent_chart_path = os.path.join(visual_reports_dir, f"{report_filename}_agent_activity.png")
                plt.savefig(agent_chart_path)
                plt.close()

                logger.info("تم إنشاء الرسم البياني لنشاط وكلاء الذكاء الصناعي: %s", agent_chart_path)
            ...
            if "daily_activity" in report and report["daily_activity"]:
                ...
                # حفظ الرسم البياني
                daily_chart_path = os.path.join(visual_reports_dir, f"{report_filename}_daily_activity.png")
                plt.savefig(daily_chart_path)
                plt.close()

                logger.info("تم إنشاء الرسم البياني للنشاط اليومي: %s", daily_chart_path)
            ...
            if "weekly_activity" in report and report["weekly_activity"]:
                ...
                # حفظ الرسم البياني
                weekly_chart_path = os.path.join(visual_reports_dir, f"{report_filename}_weekly_activity.png")
                plt.savefig(weekly_chart_path)
                plt.close()

                logger.info("تم إنشاء الرسم البياني للنشاط الأسبوعي: %s", weekly_chart_path)
            ...
            if "within_scope_percentage" in report:
                ...
                sizes = [report["within_scope_percentage"], 100 - report["within_scope_percentage"]]
                ...
                # حفظ الرسم البياني
                scope_chart_path = os.path.join(visual_reports_dir, f"{report_filename}_within_scope.png")
                plt.savefig(scope_chart_path)
                plt.close()

                logger.info("تم إنشاء الرسم البياني لنسبة الأسئلة والإجابات ضمن الاختصاصات: %s", scope_chart_path)

        except Exception as e:
            logger.error("خطأ أثناء إنشاء التقرير المرئي: %s", str(e))

    def export_data(self, format="json", start_date=None, end_date=None, module=None):
        """
        ...
        try:
            ...
            filtered_data = [record for record in self.usage_data if start_date <= record["timestamp"] <= end_date]

            if module:
                filtered_data = [record for record in filtered_data if record.get("module") == module]
            ...
            if format.lower() == "csv":
                ...
                export_path = os.path.join(self.reports_dir, f"{export_filename}.csv")
                df.to_csv(export_path, index=False, encoding='utf-8')

            else:  # json
                # حفظ البيانات في ملف JSON
                export_path = os.path.join(self.reports_dir, f"{export_filename}.json")
                with open(export_path, 'w', encoding='utf-8') as f:
                    json.dump(filtered_data, f, ensure_ascii=False, indent=2)

            logger.info("تم تصدير بيانات استخدام الذكاء الصناعي: %s", export_path)

            return export_path

        except Exception as e:
            logger.error("خطأ أثناء تصدير بيانات استخدام الذكاء الصناعي: %s", str(e))
            return None
