...
def check_agent_permission(
    ...
    # التحقق من الصلاحيات
    return permission_service.has_permission(user_id, permission)  # pylint: disable=no-member
...
def get_agent_permissions(
    ...
    if agent.owner_id == user_id:
        ...
        permissions = _check_user_permissions(permission_service, user_id, ALL_PERMISSIONS)
...
def grant_agent_permission(
    ...
    # منح الصلاحية
    return permission_service.grant_permission(user_id, permission)  # pylint: disable=no-member
...
def revoke_agent_permission(
    ...
    # إلغاء الصلاحية
    return permission_service.revoke_permission(user_id, permission)  # pylint: disable=no-member
...
def get_user_agents(
    ...
    for agent in agent_service.get_all_agents():
        if any(permission_service.has_permission(user_id, permission) for permission in ALL_PERMISSIONS):  # pylint: disable=no-member
