...
from sqlalchemy import Column, Integer, String, Text, Float, Boolean, DateTime, ForeignKey, Table, JSON, Enum
...
agent_capability = Table(
    "agent_capability",
    Base.metadata,
    Column("agent_id", String(36), ForeignKey(AI_AGENTS_ID_FK), primary_key=True),
...
agent_module = Table(
    ...
    Base.metadata,
    Column("agent_id", String(36), ForeignKey(AI_AGENTS_ID_FK), primary_key=True),
    Column("module_id", String(36), ForeignKey("modules.id"), primary_key=True)
)
...
class AIAgent(Base):
    ...
    access_level = Column(String(50), default="private", nullable=False, index=True)
    ...
    modules = relationship("Module", secondary=agent_module, back_populates="agents")
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "capabilities": [capability.name for capability in self.capabilities] if self.capabilities else [],
            "modules": [module.name for module in self.modules] if self.modules else []
        }
...
class Capability(Base):
    ...
    # العلاقات
    agents = relationship("AIAgent", secondary=agent_capability, back_populates="capabilities")
...
class Module(Base):
    ...
    # العلاقات
    agents = relationship("AIAgent", secondary=agent_module, back_populates="modules")
...
class AgentTask(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    agent_id = Column(String(36), ForeignKey(AI_AGENTS_ID_FK, ondelete='CASCADE'), nullable=False, index=True)
