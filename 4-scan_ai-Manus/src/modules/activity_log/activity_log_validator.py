...
class ActivityLogValidator:
    ...
    def validate_event_logging(
        ...
        if not logs:
            logger.warning(f"لم يتم العثور على سجل للحدث {event_type} في النافذة الزمنية المحددة")
            return False, None
        ...
        for log in logs:
            log_details = json.loads(log.details) if isinstance(log.details, str) else log.details
        ...
        logger.warning(f"لم يتم العثور على سجل للحدث {event_type} بالتفاصيل المتوقعة")
        return False, logs[0]
    ...
    def validate_user_action_logging(
        ...
        if not log:
            logger.warning(f"لم يتم العثور على سجل للإجراء {action_type} للمستخدم {user_id}")
            return False, None
    ...
    def validate_security_event_logging(
        ...
        # إذا لم يتم العثور على تطابق، فشل التحقق
        logger.warning(f"لم يتم العثور على سجل للحدث الأمني {event_type} لعنوان IP {ip_address}")
        return False, logs[0]
    ...
    def validate_ai_action_logging(
        ...
        # إذا لم يتم العثور على سجل، فشل التحقق
        if not log:
            logger.warning(f"لم يتم العثور على سجل للإجراء {action_type} لوكيل الذكاء الاصطناعي {agent_id}")
            return False, None
