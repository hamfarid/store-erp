...
PREMIUM_ML_URL = os.environ.get("PREMIUM_ML_URL", "http://premium_ml_service:5031")
CODEX_URL = os.environ.get("CODEX_URL", "http://codex_service:5032")
PYDANTIC_URL = os.environ.get("PYDANTIC_URL", "http://pydantic_ai_service:5033")
...
def log_model_usage(user_id: int, task_type: str, model_key: str, request_data: Dict[str, Any], response_data: Dict[str, Any]) -> None:
    """
...
def select_model():
    ...
    if not task_type or not user_id:
        return jsonify({"error": "Missing required fields: task_type, user_id"}), 400
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            return jsonify({"error": f"Model service error: {response.text}"}), response.status_code

        response_data = response.json()

        # تسجيل استخدام النموذج
        log_model_usage(user_id, task_type, model_key, request_data, response_data)
