...
from ..utils.constants import ERROR_MESSAGES, DEFAULT_VALUES
from .schemas import (
    KeywordCreate, KeywordUpdate, KeywordResponse,
    KeywordRelationCreate, KeywordRelationResponse
)
...
@router.post("/", response_model=KeywordResponse, status_code=status.HTTP_201_CREATED, summary="إنشاء كلمة مفتاحية جديدة")
...
@router.get("/", response_model=List[KeywordResponse], summary="الحصول على قائمة الكلمات المفتاحية")
async def get_keywords(
    ...
    plant_part: Optional[str] = Query(None, description="تصفية حسب جزء النبات"),
    search: Optional[str] = Query(None, description="البحث في نص الكلمة المفتاحية"),
    service: KeywordService = Depends(get_keyword_service)
):
    """
...
@router.get("/{keyword_id}", response_model=KeywordResponse, summary="الحصول على كلمة مفتاحية محددة")
...
@router.put("/{keyword_id}", response_model=KeywordResponse, summary="تحديث كلمة مفتاحية")
...
@router.delete("/{keyword_id}", status_code=status.HTTP_204_NO_CONTENT, summary="حذف كلمة مفتاحية")
...
@router.post("/{keyword_id}/relations", response_model=KeywordRelationResponse, status_code=status.HTTP_201_CREATED, summary="إضافة علاقة بين كلمتين مفتاحيتين")
...
@router.get("/{keyword_id}/relations", response_model=List[KeywordRelationResponse], summary="الحصول على علاقات كلمة مفتاحية")
...
@router.delete("/{keyword_id}/relations/{relation_id}", status_code=status.HTTP_204_NO_CONTENT, summary="حذف علاقة بين كلمتين مفتاحيتين")
