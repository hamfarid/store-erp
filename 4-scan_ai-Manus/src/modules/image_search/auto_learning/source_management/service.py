...
from .models import (
    TrustedSource, SourceCategory, SourceRating, SourceBlacklistEntry,
    SourceVerification, SourceUsageLog, SourceTypeEnum, SourceAnalytics
)
from .schemas import (
    TrustedSourceCreate, TrustedSourceUpdate, TrustedSourceResponse,
    SourceCategoryCreate, SourceCategoryResponse,
    SourceRatingCreate, SourceRatingResponse,
    SourceBlacklistEntryCreate, SourceBlacklistEntryResponse,
    SourceVerificationCreate, SourceVerificationResponse,
    SourceUsageLogCreate, SourceUsageLogResponse,
    SourceStatistics, SourceCategoryUpdate, SourceBlacklistEntryUpdate, SourceAnalyticsCreate
)
...
class SourceManagementService:
    ...
    def _log_to_memory(self, action: str, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.error(f"حدث خطأ أثناء تسجيل الإجراء في الذاكرة المركزية: {str(e)}")
    ...
    def _source_to_response(self, source: TrustedSource) -> TrustedSourceResponse:
        """
    ...
    def _category_to_response(self, category: SourceCategory) -> SourceCategoryResponse:
        """
    ...
    def create_source(self, source_data: TrustedSourceCreate) -> TrustedSourceResponse:
        """
        ...
        try:
            ...
            logger.info(f"تم إنشاء مصدر موثوق جديد: {new_source.name} ({domain})")
    ...
    def update_source(self, source_id: int, source_data: TrustedSourceUpdate) -> TrustedSourceResponse:
        """
        ...
        try:
            # البحث عن المصدر
            source = self.db.query(TrustedSource).filter(TrustedSource.id == source_id).first()
            ...
            if 'url' in update_data:
                update_data['domain'] = self._extract_domain(update_data['url'])
            ...
            logger.info(f"تم تحديث المصدر الموثوق: {source.name} ({source.domain})")
    ...
    def delete_source(self, source_id: int) -> bool:
        ...
        try:
            ...
            logger.info(f"تم حذف المصدر الموثوق: {source_info['name']} ({source_info['domain']})")
    ...
    def get_source_by_id(self, source_id: int) -> Optional[TrustedSourceResponse]:
        """
    ...
    def get_source_by_domain(self, domain: str) -> Optional[TrustedSourceResponse]:
        """
        ...
        try:
            ...
            source = self.db.query(TrustedSource).filter(TrustedSource.domain == clean_domain).first()
            ...
            logger.error(f"حدث خطأ أثناء البحث عن مصدر موثوق بواسطة النطاق: {str(e)}")
            logger.error(traceback.format_exc())
            raise
    ...
    def get_sources(self,
                    ...
                    sort_order: str = "desc") -> Tuple[List[TrustedSourceResponse], int]:
        """
        ...
        try:
            ...
            if min_trust_level is not None:
                query = query.filter(TrustedSource.trust_level >= min_trust_level)
            if is_academic is not None:
                query = query.filter(TrustedSource.is_academic == is_academic)
            if is_government is not None:
                query = query.filter(TrustedSource.is_government == is_government)
            if is_commercial is not None:
                query = query.filter(TrustedSource.is_commercial == is_commercial)
            if search_query:
                query = query.filter(
                    or_(
            ...
            return [self._source_to_response(source) for source in sources], total_count

        except Exception as e:
            logger.error(f"حدث خطأ أثناء الحصول على قائمة المصادر الموثوقة: {str(e)}")
            logger.error(traceback.format_exc())
            raise

    def get_trusted_sources(self, min_trust_level: int = 50) -> List[TrustedSourceResponse]:
        """
        ...
        try:
            ...
            logger.error(f"حدث خطأ أثناء الحصول على المصادر الموثوقة: {str(e)}")
            logger.error(traceback.format_exc())
            raise

    def update_source_trust_level(self, source_id: int, new_rating: int) -> bool:
        """
        ...
        try:
            ...
            logger.info(f"تم تحديث مستوى الثقة للمصدر {source.name}: {source.trust_level}")
    ...
    def add_source_rating(self, rating_data: SourceRatingCreate) -> SourceRatingResponse:
        """
        ...
        try:
            # البحث عن المصدر
            source = self.db.query(TrustedSource).filter(TrustedSource.id == rating_data.source_id).first()

            if not source:
                raise ValueError(f"المصدر الموثوق غير موجود: {rating_data.source_id}")
            ...
            logger.info(f"تم إضافة تقييم جديد للمصدر {source.name}: {rating_data.rating}")
    ...
    def get_source_statistics(self) -> SourceStatistics:
        ...
        try:
            # إجمالي عدد المصادر
            total_sources = self.db.query(func.count(TrustedSource.id)).scalar() or 0

            # عدد المصادر النشطة
            active_sources = self.db.query(func.count(TrustedSource.id)).filter(
                TrustedSource.is_active
            ).scalar() or 0

            # متوسط مستوى الثقة
            avg_trust_level = self.db.query(func.avg(TrustedSource.trust_level)).scalar() or 0

            # عدد المصادر الأكاديمية
            academic_sources = self.db.query(func.count(TrustedSource.id)).filter(
                TrustedSource.is_academic
            ).scalar() or 0

            # عدد المصادر الحكومية
            government_sources = self.db.query(func.count(TrustedSource.id)).filter(
                TrustedSource.is_government
            ).scalar() or 0

            # عدد المصادر التجارية
            commercial_sources = self.db.query(func.count(TrustedSource.id)).filter(
                TrustedSource.is_commercial
            ).scalar() or 0

            # عدد المصادر في القائمة السوداء
            blacklisted_sources = self.db.query(func.count(TrustedSource.id)).join(
                SourceBlacklistEntry, TrustedSource.id == SourceBlacklistEntry.source_id
            ).filter(
                SourceBlacklistEntry.is_active
            ).scalar() or 0

            # عدد المصادر المتحقق منها
            verified_sources = self.db.query(func.count(TrustedSource.id)).filter(
                TrustedSource.is_verified
            ).scalar() or 0
            ...
            logger.error(f"حدث خطأ أثناء الحصول على إحصائيات المصادر الموثوقة: {str(e)}")
            logger.error(traceback.format_exc())
            raise

    # ===== إدارة فئات المصادر =====

    def create_category(self, category_data: SourceCategoryCreate) -> SourceCategoryResponse:
        """
        ...
        try:
            ...
            if existing_category:
                raise ValueError(f"فئة المصادر موجودة مسبقاً: {category_data.name}")
    ...
    def delete_source_category(self, category_id: int) -> bool:
        ...
        try:
            # البحث عن الفئة
            category = self.db.query(SourceCategory).filter(SourceCategory.id == category_id).first()
    ...
    def add_to_blacklist(self, blacklist_data: SourceBlacklistEntryCreate) -> SourceBlacklistEntryResponse:
        """
        ...
        try:
            # البحث عن المصدر
            source = self.db.query(TrustedSource).filter(TrustedSource.id == blacklist_data.source_id).first()

            if not source:
                raise ValueError(f"المصدر الموثوق غير موجود: {blacklist_data.source_id}")
            ...
            if existing_entry:
                raise ValueError(f"المصدر موجود بالفعل في القائمة السوداء: {source.name}")
            ...
            self.db.rollback()
            logger.error(f"حدث خطأ أثناء إضافة مصدر إلى القائمة السوداء: {str(e)}")
            logger.error(traceback.format_exc())
            raise

    # ===== إدارة التحقق من المصادر =====

    def verify_source(self, verification_data: SourceVerificationCreate) -> SourceVerificationResponse:
        """
        التحقق من مصدر
        ...
        try:
            # البحث عن المصدر
            source = self.db.query(TrustedSource).filter(TrustedSource.id == verification_data.source_id).first()

            if not source:
                raise ValueError(f"المصدر الموثوق غير موجود: {verification_data.source_id}")
            ...
            logger.info(f"تم التحقق من المصدر {source.name}: {verification_data.is_verified}")
    ...
    def log_source_usage(self, usage_data: SourceUsageLogCreate) -> SourceUsageLogResponse:
        """
        تسجيل استخدام مصدر
        ...
        try:
            # البحث عن المصدر
            source = self.db.query(TrustedSource).filter(TrustedSource.id == usage_data.source_id).first()

            if not source:
                raise ValueError(f"المصدر الموثوق غير موجود: {usage_data.source_id}")
            ...
            source.success_count = (source.success_count or 0) + (1 if usage_data.success else 0)

            # حساب معدل النجاح
            if source.usage_count > 0:
                source.success_rate = (source.success_count / source.usage_count) * 100
            ...
            logger.info(f"تم تسجيل استخدام المصدر {source.name}: {usage_data.usage_type}")
    ...
    def update_source_category(self, category_id: int, category: SourceCategoryUpdate) -> SourceCategory:
        if not self._check_permission("update", "source"):
            raise PermissionError(NO_READ_SOURCE_PERMISSION)
        db_category = self.db.query(SourceCategory).filter(SourceCategory.id == category_id).first()
        if not db_category:
            raise ValueError(SOURCE_CATEGORY_NOT_FOUND)
    ...
    def get_source_blacklist_entry(self, entry_id: int) -> SourceBlacklistEntry:
        if not self._check_permission("read", "blacklist"):
            raise PermissionError(NO_READ_SOURCE_PERMISSION)
        entry = self.db.query(SourceBlacklistEntry).filter(SourceBlacklistEntry.id == entry_id).first()
        if not entry:
            raise ValueError(SOURCE_BLACKLIST_ENTRY_NOT_FOUND)
        return entry

    def create_source_blacklist_entry(self, entry: SourceBlacklistEntryCreate) -> SourceBlacklistEntry:
        if not self._check_permission("create", "blacklist"):
            raise PermissionError(NO_READ_SOURCE_PERMISSION)
        existing_entry = self.db.query(SourceBlacklistEntry).filter(SourceBlacklistEntry.url == entry.url).first()
        if existing_entry:
    ...
    def update_source_blacklist_entry(self, entry_id: int, entry: SourceBlacklistEntryUpdate, user_id: int) -> SourceBlacklistEntry:
        if not self._check_permission("update", "blacklist"):
            raise PermissionError(NO_READ_SOURCE_PERMISSION)
        db_entry = self.db.query(SourceBlacklistEntry).filter(SourceBlacklistEntry.id == entry_id).first()
        if not db_entry:
            raise ValueError(SOURCE_BLACKLIST_ENTRY_NOT_FOUND)
    ...
    def delete_source_analytics(self, source_id: int, user_id: int) -> bool:
        if not self._check_permission("delete", "source"):
            raise PermissionError(NO_READ_SOURCE_PERMISSION)
        db_analytics = self.db.query(SourceAnalytics).filter(SourceAnalytics.source_id == source_id).first()
        if not db_analytics:
    ...
    def create_source_analytics(self, analytics: SourceAnalyticsCreate) -> SourceAnalytics:
        if not self._check_permission("create", "source"):
            raise PermissionError(NO_READ_SOURCE_PERMISSION)
