...
from src.modules.image_search.auto_learning.utils.constants import ERROR_MESSAGES, SUCCESS_MESSAGES, DEFAULT_VALUES
...
from src.modules.image_search.auto_learning.source_management.service import SourceService
...
@router.post("/", response_model=TrustedSourceResponse, status_code=status.HTTP_201_CREATED, summary="إنشاء مصدر موثوق جديد")
...
@router.get("/", response_model=List[TrustedSourceResponse], summary="الحصول على قائمة المصادر الموثوقة")
async def get_sources(
    skip: int = Query(0, description="عدد العناصر للتخطي"),
    limit: int = Query(DEFAULT_VALUES["PAGE_SIZE"], description="الحد الأقصى لعدد العناصر للإرجاع"),
    category: Optional[str] = Query(None, description="تصفية حسب التصنيف"),
    min_trust_level: Optional[int] = Query(None, description="الحد الأدنى لمستوى الثقة"),
    search: Optional[str] = Query(None, description="البحث في نطاق المصدر"),
    service: SourceService = Depends(get_source_service)
):
    """
    ...
    """
    try:
        return service.get_sources(skip, limit, category, min_trust_level, search)
    except Exception as exc:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=ERROR_MESSAGES["GENERAL"]["INTERNAL_ERROR"]
...
@router.get("/{source_id}", response_model=TrustedSourceResponse, summary="الحصول على مصدر موثوق محدد")
...
@router.put("/{source_id}", response_model=TrustedSourceResponse, summary="تحديث مصدر موثوق")
...
@router.delete("/{source_id}", status_code=status.HTTP_204_NO_CONTENT, summary="حذف مصدر موثوق")
...
@router.post("/{source_id}/verify", response_model=SourceVerificationResponse, status_code=status.HTTP_201_CREATED, summary="إضافة تحقق من مصدر")
...
@router.get("/{source_id}/verifications", response_model=List[SourceVerificationResponse], summary="الحصول على تحققات مصدر")
...
@router.post("/{source_id}/blacklist", status_code=status.HTTP_200_OK, summary="إضافة مصدر إلى القائمة السوداء")
...
@router.post("/{source_id}/remove-from-blacklist", status_code=status.HTTP_200_OK, summary="إزالة مصدر من القائمة السوداء")
async def remove_from_blacklist(
    ...
    try:
        ...
        return {"message": SUCCESS_MESSAGES["SOURCE"]["REMOVED_FROM_BLACKLIST"]}
    except Exception as exc:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=ERROR_MESSAGES["GENERAL"]["INTERNAL_ERROR"]
