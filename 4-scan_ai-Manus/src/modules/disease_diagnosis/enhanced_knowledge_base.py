...
class EnhancedAgriculturalKnowledgeBase:
    ...
    def _load_knowledge_base(self):
        ...
        try:
            ...
            if os.path.exists(diseases_file):
                ...
                logger.info("تم تحميل %d مرض من %s", len(self.diseases), diseases_file)
            ...
            if os.path.exists(crops_file):
                ...
                logger.info("تم تحميل %d محصول من %s", len(self.crops), crops_file)
            ...
            if os.path.exists(treatments_file):
                ...
                logger.info("تم تحميل %d علاج من %s", len(self.treatments), treatments_file)
            ...
            if os.path.exists(varieties_file):
                ...
                logger.info("تم تحميل %d صنف من %s", len(self.varieties), varieties_file)
            ...
            if os.path.exists(hybridization_file):
                ...
                logger.info("تم تحميل معلومات التهجين من %s", hybridization_file)
    ...
    def _parse_scientific_journals(self, content):
        ...
        if "## الرسائل العلمية والأبحاث الأكاديمية" in content:
            section = content.split("## الرسائل العلمية والأبحاث الأكاديمية")[1].split("##")[0]

            if "### مجلات علمية متخصصة" in section:
                journals_section = section.split("### مجلات علمية متخصصة")[1].split("###")[0]
                journals = []
    ...
    def _parse_journal_entry(self, journal):
        ...
        for line in lines:
            if line.startswith(self.NUMBERED_LIST_START):
                ...
                journal_info["website"] = line.split(self.WEBSITE_LABEL)[1].strip()
            elif "المجالات:" in line:
                journal_info["fields"] = [field.strip() for field in line.split("المجالات:")[1].strip().split(",")]
    ...
    def _parse_seed_companies(self, content):
        ...
        if "## شركات البذور المعتمدة" in content:
            section = content.split("## شركات البذور المعتمدة")[1].split("##")[0]
            companies = []
    ...
    def _parse_fertilizer_companies(self, content):
        ...
        if "## شركات الأسمدة والمبيدات" in content:
            section = content.split("## شركات الأسمدة والمبيدات")[1].split("##")[0]
            companies = []
    ...
    def _parse_company_entry(self, company):
        ...
        for line in lines:
            if line.startswith(self.NUMBERED_LIST_START):
                ...
                company_info["website"] = line.split(self.WEBSITE_LABEL)[1].strip()
            elif self.PRODUCTS_LABEL in line:
                company_info["products"] = [product.strip() for product in line.split(self.PRODUCTS_LABEL)[1].strip().split(",")]
            elif self.DATABASE_LABEL in line:
                company_info["database"] = line.split(self.DATABASE_LABEL)[1].strip()
    ...
    def _parse_research_labs(self, content):
        ...
        if "## المعامل البحثية المتخصصة" in content:
            section = content.split("## المعامل البحثية المتخصصة")[1].split("##")[0]
            labs = []
    ...
    def _parse_lab_entry(self, lab):
        ...
        for line in lines:
            if line.startswith(self.NUMBERED_LIST_START):
                ...
                lab_info["research_fields"] = [field.strip() for field in line.split("مجالات البحث:")[1].strip().split(",")]
            elif self.DATABASE_LABEL in line:
                lab_info["database"] = line.split(self.DATABASE_LABEL)[1].strip()
    ...
    def _parse_disease_info(self, content):
        ...
        if "## معلومات الأمراض وأعراضها" in content:
            section = content.split("## معلومات الأمراض وأعراضها")[1].split("##")[0]
            diseases = []

            diseases.extend(self._parse_diseases_by_category(section, "### أمراض الخضروات", "vegetables"))
            diseases.extend(self._parse_diseases_by_category(section, "### أمراض المحاصيل الحقلية", "field_crops"))
            diseases.extend(self._parse_diseases_by_category(section, "### أمراض أشجار الفاكهة", "fruit_trees"))

            self.sources["disease_info"] = diseases

    def _parse_diseases_by_category(self, section, category_header, category_name):
        """استخراج أمراض حسب الفئة"""
        diseases = []
        if category_header in section:
            category_section = section.split(category_header)[1].split("###")[0]

            for disease in category_section.split("\n\n"):
                disease_info = self._parse_disease_entry(disease, category_name)
                if disease_info:
    ...
    def _parse_disease_entry(self, disease, category):
        ...
        for line in lines:
            if line.startswith(self.NUMBERED_LIST_START):
                ...
                disease_info["pathogen"] = line.split(self.PATHOGEN_LABEL)[1].strip()
            elif self.SYMPTOMS_LABEL in line:
                disease_info["symptoms"] = line.split(self.SYMPTOMS_LABEL)[1].strip()
            elif self.SOURCE_LABEL in line:
                disease_info["source"] = line.split(self.SOURCE_LABEL)[1].strip()
            elif self.LINK_LABEL in line:
                disease_info["reference"] = line.split(self.LINK_LABEL)[1].strip()
    ...
    def _parse_varieties_info(self, content):
        ...
        if "## مواصفات الأصناف النباتية" in content:
            section = content.split("## مواصفات الأصناف النباتية")[1].split("##")[0]
            varieties = []

            varieties.extend(self._parse_varieties_by_crop(section, "### أصناف الطماطم", "tomato"))
            varieties.extend(self._parse_varieties_by_crop(section, "### أصناف القمح", "wheat"))
            varieties.extend(self._parse_varieties_by_crop(section, "### أصناف الأرز", "rice"))
    ...
    def _parse_variety_entry(self, variety, crop):
        ...
        for line in lines:
            if line.startswith(self.NUMBERED_LIST_START):
                ...
                variety_info["characteristics"] = line.split(self.PROPERTIES_LABEL)[1].strip()
            elif self.SOURCE_LABEL in line:
                variety_info["source"] = line.split(self.SOURCE_LABEL)[1].strip()
            elif self.LINK_LABEL in line:
                variety_info["reference"] = line.split(self.LINK_LABEL)[1].strip()
    ...
    def _parse_hybridization_methods(self, content):
        ...
        if "## طرق التهجين والتزاوج" in content:
            section = content.split("## طرق التهجين والتزاوج")[1].split("##")[0]
            methods = []
    ...
    def _parse_method_entry(self, method):
        ...
        for line in lines:
            if line.startswith(self.NUMBERED_LIST_START):
                ...
                method_info["source"] = line.split(self.SOURCE_LABEL)[1].strip()
            elif self.LINK_LABEL in line:
                method_info["reference"] = line.split(self.LINK_LABEL)[1].strip()
    ...
    def _parse_parent_specifications(self, content):
        ...
        if "## مواصفات الآباء والأمهات في عمليات التهجين" in content:
            section = content.split("## مواصفات الآباء والأمهات في عمليات التهجين")[1].split("##")[0]
            specifications = []
    ...
    def _parse_spec_entry(self, spec):
        ...
        for line in lines:
            if line.startswith(self.NUMBERED_LIST_START):
                ...
                spec_info["desired_traits"] = line.split("الصفات المرغوبة:")[1].strip()
            elif self.SOURCE_LABEL in line:
    ...
    def update_knowledge_base_from_sources(self):
        ...
        try:
            ...
            logger.error("خطأ أثناء تحديث قاعدة المعرفة من المصادر: %s", str(e))
            return False
            ...
            for disease_info in self.sources.get("disease_info", []):
                ...
                if disease_id in self.diseases:
                    ...
                    if category == "vegetables":
                        new_disease["crops"] = ["tomato", "cucumber", "pepper", "eggplant"]
                    elif category == "field_crops":
                        new_disease["crops"] = ["wheat", "corn", "rice", "barley"]
                    elif category == "fruit_trees":
                        new_disease["crops"] = ["apple", "peach", "citrus", "grape"]
    ...
    def _update_varieties_from_sources(self):
        ...
        try:
            ...
            logger.info("تم تحديث أصناف %d محصول من المصادر", len(self.varieties))
        except Exception as e:
    ...
    def _update_hybridization_from_sources(self):
        ...
        try:
            ...
            logger.error("خطأ أثناء تحديث معلومات التهجين من المصادر: %s", str(e))
    ...
    def add_variety(self, crop_id, variety_id, variety_info):
        ...
        try:
            ...
            if variety_id in self.varieties[crop_id]:
                logger.warning("الصنف %s للمحصول %s موجود بالفعل", variety_id, crop_id)
                return False
    ...
    def update_variety(self, crop_id, variety_id, variety_info):
        ...
        try:
            ...
            if crop_id not in self.varieties or variety_id not in self.varieties[crop_id]:
                logger.warning("الصنف %s للمحصول %s غير موجود", variety_id, crop_id)
                return False
    ...
    def get_knowledge_base_stats(self):
        ...
        try:
            ...
            logger.error("خطأ أثناء استرجاع إحصائيات قاعدة المعرفة: %s", str(e))
            return {
                "success": False,
