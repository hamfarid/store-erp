# File: /home/ubuntu/ai_web_organized/src/modules/disease_diagnosis/disease_knowledge_base.py
"""
from flask import g
...
class DiseaseKnowledgeBase:
            ...
            'crops': [
                {
                    ...
                    'diseases': [
                        ...
                        },
                        {
                            ...
                            'symptoms': [
                                'بقع صفراء على الجانب العلوي للأوراق',
                            ...
                            'treatments': [
                                'استخدام مبيدات فطرية مثل الكبريت',
                                'استخدام زيوت نباتية',
                    ...
                    'diseases': [
                        {
                            ...
                            'symptoms': [
                                'بثرات بنية محمرة على السيقان والأوراق',
                                'تمزق الأنسجة النباتية',
                                'ضعف النبات وقلة المحصول'
                            ],
                            'conditions': [
                                'درجات حرارة دافئة (15-30 درجة مئوية)',
                                'رطوبة عالية أو ندى'
                            ],
                            'treatments': [
                                'استخدام مبيدات فطرية',
                        ...
                        },
                        {
                            ...
                            'symptoms': [
                                'استبدال حبوب القمح بكتل سوداء من الجراثيم',
                                'رائحة كريهة تشبه رائحة السمك الفاسد',
                                'سنابل ذات مظهر طبيعي حتى النضج'
                            ],
                            'conditions': [
                                'تلوث البذور بالجراثيم',
                                'ظروف باردة ورطبة أثناء الإنبات'
                            ],
                            'treatments': [
                                'لا يوجد علاج بعد ظهور المرض'
                            ],
                            'prevention': [
                                'معالجة البذور بمبيدات فطرية',
                                'استخدام بذور معتمدة خالية من المرض',
                                'تناوب المحاصيل'
                            ],
                            'images': [
                                'covered_smut_wheat.jpg'
                            ]
                        }
                    ]
                }
    ...
    def get_disease_by_name(self, disease_name: str, crop_name: str = None) -> Optional[Dict[str, Any]]:
        """
        البحث عن مرض باسمه
    ...
    def add_disease(self, crop_name: str, disease_data: Dict[str, Any]) -> bool:
        """
        ...
        for disease in crop.get('diseases', []):
            if disease['name'] == disease_data['name']:
                print(f"خطأ: المرض {disease_data['name']} موجود بالفعل للمحصول {crop_name}")
                return False
        ...
        print(f"تمت إضافة المرض {disease_data['name']} للمحصول {crop_name} بنجاح")
        return True
    ...
    def update_disease(self, disease_name: str, crop_name: str, disease_data: Dict[str, Any]) -> bool:
        """
        تحديث بيانات مرض
        ...
        for i, disease in enumerate(crop.get('diseases', [])):
            if disease['name'] == disease_name:
                ...
                print(f"تم تحديث بيانات المرض {disease_name} للمحصول {crop_name} بنجاح")
                return True
    ...
    def delete_disease(self, disease_name: str, crop_name: str) -> bool:
        ...
        for i, disease in enumerate(crop.get('diseases', [])):
            if disease['name'] == disease_name:
                ...
                print(f"تم حذف المرض {disease_name} من المحصول {crop_name} بنجاح")
                return True
        ...
        for crop in self.knowledge_base.get('crops', []):
            for disease in crop.get('diseases', []):
                if 'symptoms' in disease:
                    ...
                    matching_symptoms = [s for s in symptoms if any(s.lower() in ds.lower() for ds in disease['symptoms'])]
