# File: /home/ubuntu/ai_web_organized/src/modules/disease_diagnosis/db_models.py
...
disease_symptom = Table(
    'disease_symptom',
    Base.metadata,
    Column('disease_id', Integer, ForeignKey(PLANT_DISEASES_ID_FK), primary_key=True),
    Column('symptom_id', Integer, ForeignKey('disease_symptoms.id'), primary_key=True)
)
...
disease_treatment = Table(
    ...
    Base.metadata,
    Column('disease_id', Integer, ForeignKey(PLANT_DISEASES_ID_FK), primary_key=True),
    Column('treatment_id', Integer, ForeignKey('disease_treatments.id'), primary_key=True)
)
...
disease_prevention = Table(
    ...
    Base.metadata,
    Column('disease_id', Integer, ForeignKey(PLANT_DISEASES_ID_FK), primary_key=True),
    Column('prevention_id', Integer, ForeignKey('disease_preventions.id'), primary_key=True)
)
...
disease_condition = Table(
    ...
    Base.metadata,
    Column('disease_id', Integer, ForeignKey(PLANT_DISEASES_ID_FK), primary_key=True),
    Column('condition_id', Integer, ForeignKey('disease_conditions.id'), primary_key=True)
)
...
class Disease(Base):
    ...
    images = relationship("DiseaseImage", back_populates="disease", cascade="all, delete-orphan")
    symptoms = relationship("Symptom", secondary=disease_symptom, backref="diseases")
    treatments = relationship("Treatment", secondary=disease_treatment, backref="diseases")
    preventions = relationship("Prevention", secondary=disease_prevention, backref="diseases")
    conditions = relationship("Condition", secondary=disease_condition, backref="diseases")
    diagnoses = relationship("Diagnosis", back_populates="disease", cascade="all, delete-orphan")
...
class DiseaseImage(Base):
    ...
    id = Column(Integer, primary_key=True)
    disease_id = Column(Integer, ForeignKey(PLANT_DISEASES_ID_FK), nullable=False)
