...
async def get_all_permissions(current_user: dict = Depends(get_current_user)):
    ...
    if not check_permission(current_user, 'manage_roles'):
        raise HTTPException(status_code=403, detail="ليس لديك صلاحية لإدارة الأدوار")
...
async def get_all_users(current_user: dict = Depends(get_current_user)):
    ...
    if not check_permission(current_user, 'manage_users'):
        raise HTTPException(status_code=403, detail=NO_USER_MANAGEMENT_PERMISSION)
...
async def create_user(
    ...
    users_data = load_data(USERS_FILE)
    if any(u['username'] == user_data['username'] for u in users_data['users']):
...
async def update_user(
    ...
    users_data = load_data(USERS_FILE)
    user_index = next((i for i, u in enumerate(users_data['users']) if u['id'] == user_id), None)
    ...
    if 'username' in user_data:
        if any(u['username'] == user_data['username'] and u['id'] != user_id for u in users_data['users']):
            raise HTTPException(status_code=400, detail="Username already exists")

    if 'email' in user_data:
        if any(u['email'] == user_data['email'] and u['id'] != user_id for u in users_data['users']):
            raise HTTPException(status_code=400, detail="Email already exists")
