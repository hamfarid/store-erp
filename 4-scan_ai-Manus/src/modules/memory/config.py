...
class MemoryDatabaseSettings(BaseModel):
    ...
    connection_string: str = Field(
        default=os.getenv("MEMORY_DB_CONNECTION_STRING", "sqlite:///memory.db"),
...
class MemoryRetentionSettings(BaseModel):
    ...
    permanent_retention: bool = Field(
        default=os.getenv("MEMORY_PERMANENT_RETENTION", "False").lower() == "true",
...
class MemorySecuritySettings(BaseModel):
    ...
    encryption_enabled: bool = Field(
        default=os.getenv("MEMORY_ENCRYPTION_ENABLED", "True").lower() == "true",
    ...
    access_control_enabled: bool = Field(
        default=os.getenv("MEMORY_ACCESS_CONTROL_ENABLED", "True").lower() == "true",
...
class MemoryAIIntegrationSettings(BaseModel):
    ...
    enabled: bool = Field(
        default=os.getenv("MEMORY_AI_INTEGRATION_ENABLED", "True").lower() == "true",
    ...
    auto_summarization: bool = Field(
        default=os.getenv("MEMORY_AUTO_SUMMARIZATION", "True").lower() == "true",
...
class MemoryConfig(BaseModel):
    ...
    max_memory_size_mb: int = Field(
        default=int(os.getenv("MEMORY_MAX_SIZE_MB", "10240")),  # 10 GB افتراضياً
    ...
    def get_memory_types(self) -> List[str]:
        ...
        return ["short_term", "long_term", "semantic", "episodic", "procedural"]
