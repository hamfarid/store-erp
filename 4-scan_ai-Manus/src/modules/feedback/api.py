...
async def create_feedback(
    ...
    try:
        # التحقق من الصلاحيات
        if not check_permission(current_user, config.FeedbackPermissions.ADD_FEEDBACK):
            raise HTTPException(status_code=403, detail="ليس لديك صلاحية لإضافة تغذية راجعة")

        feedback = feedback_service.create_feedback(feedback_data, current_user["id"])
...
async def get_all_feedback(
    ...
    try:
        # التحقق من الصلاحيات
        if not check_permission(current_user, config.FeedbackPermissions.VIEW_FEEDBACK):
            raise HTTPException(status_code=403, detail="ليس لديك صلاحية لعرض التغذية الراجعة")

        # تحويل المعلمات
        filter_params = schemas.FeedbackFilter(
            status=getattr(config.FeedbackStatus, status.upper()) if status else None,
            feedback_type=getattr(config.FeedbackType, feedback_type.upper()) if feedback_type else None,
            priority=getattr(config.FeedbackPriority, priority.upper()) if priority else None,
...
async def get_feedback(
    ...
    try:
        ...
        feedback = feedback_service.get_feedback_by_id(feedback_id, current_user["id"])
        if not feedback:
            raise HTTPException(status_code=404, detail="التغذية الراجعة غير موجودة")

        # التحقق من صلاحية عرض التفاصيل
        if not check_permission(current_user, config.FeedbackPermissions.VIEW_FEEDBACK_DETAILS):
        ...
        logger.error(f"خطأ في الحصول على التغذية الراجعة {feedback_id}: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_feedback(
    ...
    """حذف تغذية راجعة"""
    try:
        result = feedback_service.delete_feedback(feedback_id, current_user["id"])
        if not result:
            raise HTTPException(status_code=404, detail="التغذية الراجعة غير موجودة")
...
async def add_comment(
    ...
    try:
        # التحقق من الصلاحيات
        if not check_permission(current_user, config.FeedbackPermissions.ADD_FEEDBACK):
            raise HTTPException(status_code=403, detail="ليس لديك صلاحية لإضافة تعليقات")
        ...
        logger.error(f"خطأ في إضافة تعليق على التغذية الراجعة {feedback_id}: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_comments(
    ...
    try:
        # التحقق من الصلاحيات
        if not check_permission(current_user, config.FeedbackPermissions.VIEW_FEEDBACK_DETAILS):
            raise HTTPException(status_code=403, detail="ليس لديك صلاحية لعرض التعليقات")
        ...
        logger.error(f"خطأ في الحصول على تعليقات التغذية الراجعة {feedback_id}: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_comment(
    ...
    """حذف تعليق"""
    try:
        result = feedback_service.delete_comment(comment_id, current_user["id"])
        if not result:
...
async def add_vote(
    ...
    try:
        # التحقق من الصلاحيات
        if not check_permission(current_user, config.FeedbackPermissions.ADD_FEEDBACK):
            raise HTTPException(status_code=403, detail="ليس لديك صلاحية للتصويت")
        ...
        logger.error(f"خطأ في إضافة تصويت على التغذية الراجعة {feedback_id}: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def remove_vote(
    ...
    try:
        ...
        logger.error(f"خطأ في إزالة تصويت من التغذية الراجعة {feedback_id}: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_vote_count(
    ...
    try:
        # التحقق من الصلاحيات
        if not check_permission(current_user, config.FeedbackPermissions.VIEW_FEEDBACK):
            raise HTTPException(status_code=403, detail="ليس لديك صلاحية لعرض التصويتات")
        ...
        logger.error(f"خطأ في الحصول على عدد تصويتات التغذية الراجعة {feedback_id}: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def add_attachment(
    ...
    try:
        ...
        logger.error(f"خطأ في إضافة مرفق للتغذية الراجعة {feedback_id}: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_attachment(
    ...
    """حذف مرفق"""
    try:
        result = feedback_service.delete_attachment(attachment_id, current_user["id"])
        if not result:
...
async def add_tag_to_feedback(
    ...
    try:
        result = feedback_service.add_tag_to_feedback(feedback_id, tag_id, current_user["id"])
        if not result:
            raise HTTPException(status_code=404, detail="التغذية الراجعة أو الوسم غير موجود")
...
async def remove_tag_from_feedback(
    ...
    try:
        result = feedback_service.remove_tag_from_feedback(feedback_id, tag_id, current_user["id"])
        if not result:
            raise HTTPException(status_code=404, detail="الارتباط بين التغذية الراجعة والوسم غير موجود")
...
async def get_feedback_statistics(
    ...
    try:
        # التحقق من الصلاحيات
        if not check_permission(current_user, config.FeedbackPermissions.MANAGE_FEEDBACK):
            raise HTTPException(status_code=403, detail="ليس لديك صلاحية لعرض إحصائيات التغذية الراجعة")
...
async def get_trending_feedback(
    ...
    try:
        ...
        logger.error(f"خطأ في الحصول على التغذية الراجعة الأكثر شعبية: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def cleanup_old_feedback(
    ...
    try:
        # التحقق من الصلاحيات
        if not check_permission(current_user, config.FeedbackPermissions.MANAGE_FEEDBACK):
            raise HTTPException(status_code=403, detail="ليس لديك صلاحية لتنظيف التغذية الراجعة")
