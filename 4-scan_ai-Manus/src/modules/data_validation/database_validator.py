...
class DatabaseValidator:
        ...
        try:
            ...
            if db_type == "sqlite":
                ...
                if not MYSQL_AVAILABLE:
                    logger.error("MySQL connector غير متوفر. يرجى تثبيت mysql-connector-python")
                    return False
            ...
            if db_type == "sqlite":
                ...
                for index in indices:
                    if index["unique"] == 1:
                        ...
                        unique_key = [column["name"] for column in index_columns]
                        if len(unique_key) == 1:
    ...
    def validate_table_schema(self, table_name: str) -> Tuple[bool, List[Dict[str, Any]]]:
        """
        ...
        if table_name not in self.constraints:
            logger.error(f"الجدول '{table_name}' غير موجود في قيود قاعدة البيانات")
            return False, [{"table": table_name, "error": f"الجدول '{table_name}' غير موجود في قيود قاعدة البيانات"}]

        schema = self.get_table_schema(table_name)
        if not schema:
            logger.error(f"لم يتم العثور على مخطط الجدول '{table_name}'")
            return False, [{"table": table_name, "error": f"لم يتم العثور على مخطط الجدول '{table_name}'"}]
        ...
        actual_not_null = {column for column, details in schema.get("columns", {}).items() if details.get("not_null")}
    ...
    def validate_data_integrity(self, table_name: str) -> Tuple[bool, List[Dict[str, Any]]]:
        """
        ...
        try:
            ...
            for column, check in check_constraints.items():
                if isinstance(check, list):
                    ...
                    cursor.execute(f"SELECT COUNT(*) FROM {table_name} WHERE {column} NOT IN ({placeholders})", check)
                    count = cursor.fetchone()[0]
                    ...
                    if "min" in check:
                        cursor.execute(f"SELECT COUNT(*) FROM {table_name} WHERE {column} < ?", (check["min"],))
                        count = cursor.fetchone()[0]
                    ...
                    if "max" in check:
                        cursor.execute(f"SELECT COUNT(*) FROM {table_name} WHERE {column} > ?", (check["max"],))
                        count = cursor.fetchone()[0]
            ...
            return False, [{"table": table_name, "error": f"خطأ في التحقق من تكامل البيانات: {str(e)}"}]

    def validate_all_tables(self) -> Dict[str, Tuple[bool, List[Dict[str, Any]]]]:
        """
        ...
        for table_name in self.constraints.keys():
            # التحقق من صحة مخطط الجدول
            schema_valid, schema_errors = self.validate_table_schema(table_name)
    ...
    def repair_data_integrity(self, table_name: str, auto_fix: bool = False) -> Tuple[bool, List[Dict[str, Any]]]:
        """
        ...
        if is_valid:
            logger.info(f"لا توجد مشكلات في تكامل البيانات في الجدول '{table_name}'")
            return True, []

        if not auto_fix:
            logger.info(f"تم العثور على {len(errors)} مشكلة في تكامل البيانات في الجدول '{table_name}'، لكن لم يتم تفعيل الإصلاح التلقائي")
            return False, errors
        ...
        try:
            ...
            for column in constraints.get("not_null", []):
                ...
                if count > 0:
                    ...
                    if default_value is not None:
                        cursor.execute(f"UPDATE {table_name} SET {column} = ? WHERE {column} IS NULL", (default_value,))
            ...
            for column, check in check_constraints.items():
                if isinstance(check, list) and len(check) > 0:
                    ...
                    cursor.execute(f"SELECT id FROM {table_name} WHERE {column} NOT IN ({placeholders})", check)
                    invalid_ids = [row[0] for row in cursor.fetchall()]

                    if invalid_ids:
                        id_placeholders = ", ".join(["?"] * len(invalid_ids))
                        cursor.execute(f"UPDATE {table_name} SET {column} = ? WHERE id IN ({id_placeholders})", [default_value] + invalid_ids)
                        self.connection.commit()
                    ...
                    if "min" in check:
                        min_value = check["min"]
                        cursor.execute(f"SELECT id FROM {table_name} WHERE {column} < ?", (min_value,))
                        invalid_ids = [row[0] for row in cursor.fetchall()]

                        if invalid_ids:
                            id_placeholders = ", ".join(["?"] * len(invalid_ids))
                            cursor.execute(f"UPDATE {table_name} SET {column} = ? WHERE id IN ({id_placeholders})", [min_value] + invalid_ids)
                            self.connection.commit()
                    ...
                    if "max" in check:
                        max_value = check["max"]
                        cursor.execute(f"SELECT id FROM {table_name} WHERE {column} > ?", (max_value,))
                        invalid_ids = [row[0] for row in cursor.fetchall()]

                        if invalid_ids:
                            id_placeholders = ", ".join(["?"] * len(invalid_ids))
                            cursor.execute(f"UPDATE {table_name} SET {column} = ? WHERE id IN ({id_placeholders})", [max_value] + invalid_ids)
                            self.connection.commit()
            ...
            for column, fk in foreign_keys.items():
                ...
                if invalid_ids:
                    ...
                    cursor.execute(f"SELECT {ref_column} FROM {ref_table} LIMIT 1")
                    result = cursor.fetchone()

                    if result:
                        valid_value = result[0]
                        id_placeholders = ", ".join(["?"] * len(invalid_ids))
                        cursor.execute(f"UPDATE {table_name} SET {column} = ? WHERE id IN ({id_placeholders})", [valid_value] + invalid_ids)
                        self.connection.commit()
                        ...
                        id_placeholders = ", ".join(["?"] * len(invalid_ids))
                        cursor.execute(f"UPDATE {table_name} SET {column} = NULL WHERE id IN ({id_placeholders})", invalid_ids)
                        self.connection.commit()
            ...
            is_valid, remaining_errors = self.validate_data_integrity(table_name)

            if is_valid:
                logger.info(f"تم إصلاح جميع مشكلات تكامل البيانات في الجدول '{table_name}'")
                return True, actions
            else:
                logger.warning(f"تم إصلاح بعض مشكلات تكامل البيانات في الجدول '{table_name}'، لكن لا تزال هناك {len(remaining_errors)} مشكلة")
                return False, actions + [{"table": table_name, "error": "مشكلات متبقية", "details": remaining_errors}]
        except Exception as e:
            logger.error(f"خطأ في إصلاح تكامل البيانات: {str(e)}")
            return False, [{"table": table_name, "error": f"خطأ في إصلاح تكامل البيانات: {str(e)}"}]

    def add_constraint(self, table_name: str, constraint_type: str, constraint_details: Any) -> bool:
        """
        إضافة قيد جديد.
        ...
        try:
            if constraint_type == "primary_key":
                ...
                if isinstance(constraint_details, list):
                    ...
                    self.constraints[table_name]["unique_keys"].append(constraint_details)
            elif constraint_type == "foreign_keys":
                if isinstance(constraint_details, dict):
                    self.constraints[table_name]["foreign_keys"].update(constraint_details)
                else:
                ...
                if isinstance(constraint_details, list):
                    ...
                    self.constraints[table_name]["not_null"].append(constraint_details)
            elif constraint_type == "check_constraints":
                if isinstance(constraint_details, dict):
                    self.constraints[table_name]["check_constraints"].update(constraint_details)
                else:
            ...
            logger.info(f"تمت إضافة القيد '{constraint_type}' للجدول '{table_name}' بنجاح")
            return True
        except Exception as e:
            logger.error(f"خطأ في إضافة القيد: {str(e)}")
            return False

    def remove_constraint(self, table_name: str, constraint_type: str, constraint_key: Optional[str] = None) -> bool:
        """
        إزالة قيد.
        ...
        try:
            if constraint_type == "primary_key":
                ...
                if constraint_key:
                    if constraint_key in self.constraints[table_name]["unique_keys"]:
                        self.constraints[table_name]["unique_keys"].remove(constraint_key)
                    else:
                        logger.warning(f"المفتاح الفريد '{constraint_key}' غير موجود في الجدول '{table_name}'")
                else:
                ...
                if constraint_key:
                    if constraint_key in self.constraints[table_name]["foreign_keys"]:
                        ...
                        logger.warning(f"المفتاح الأجنبي '{constraint_key}' غير موجود في الجدول '{table_name}'")
                else:
                    self.constraints[table_name]["foreign_keys"] = {}
            elif constraint_type == "not_null":
                if constraint_key:
                    if constraint_key in self.constraints[table_name]["not_null"]:
                        self.constraints[table_name]["not_null"].remove(constraint_key)
                    else:
                        logger.warning(f"قيد عدم الفراغ '{constraint_key}' غير موجود في الجدول '{table_name}'")
                else:
                ...
                if constraint_key:
                    if constraint_key in self.constraints[table_name]["check_constraints"]:
                        ...
                        logger.warning(f"قيد التحقق '{constraint_key}' غير موجود في الجدول '{table_name}'")
                else:
            ...
            logger.info(f"تمت إزالة القيد '{constraint_type}' من الجدول '{table_name}' بنجاح")
            return True
        except Exception as e:
            logger.error(f"خطأ في إزالة القيد: {str(e)}")
            return False

    def get_constraints(self, table_name: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if table_name:
            if table_name not in self.constraints:
                logger.warning(f"الجدول '{table_name}' غير موجود في قيود قاعدة البيانات")
                return {}
