...
class Alerter:
    ...
    def __init__(self, metrics_collector, config_path="../config/monitoring.yaml"):
        ...
        self.cooldown_period = self.config.get("alerting", {}).get("cooldown_period_seconds", 300)

        # Thread control
        self.running = False
        self.evaluation_thread = None
        self.evaluation_interval = self.config.get("alerting", {}).get("evaluation_interval_seconds", 60)
    ...
    def _evaluate_rules(self, metrics):
        ...
        for rule in rules:
            ...
            try:
                ...
                if isinstance(value, dict) and metric_path.endswith("response_times"):
                    ...
                    if "avg" in value:
                        ...
                        logger.debug(f"Specific response time metric not found in {metric_path}")
                        continue
            ...
            try:
                ...
                logger.warning(f'Type error comparing metric "{metric_path}" with threshold for rule "{rule_name}"')
                continue
            except Exception as e:
    ...
    def _trigger_alert(self, rule, current_value):
        ...
        message = f'ALERT [{severity.upper()}]: Rule "{rule_name}" triggered. Metric {metric} ({current_value}) {condition} {threshold}.'
    ...
    def _resolve_alert(self, rule):
        ...
        for channel_name in rule["channels"]:
            ...
            try:
                ...
                logger.error(f'Failed to send resolution notification via "{channel_name}" for "{rule_name}": {e}')
...
alerting:
  ...
  rules:
    ...
    - name: "High Memory Usage"
      ...
      threshold: 80.0
      severity: "error"
      channels: ["log_channel", "email_channel"]
