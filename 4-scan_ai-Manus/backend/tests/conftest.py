...
def test_settings():
    ...
    return {
        ...
        "test_username": "test@gaara.ai",  # Use email for login
...
def setup_test_database():
    ...
    try:
        ...
        # Skip if engine is None (e.g., for unit tests that don't need DB)
        if engine is None:
            yield
            return
        ...
        try:
            ...
            existing_user = db.query(User).filter(User.email == "test@gaara.ai").first()
        ...
        # If database setup fails, just yield and let tests proceed
        yield
...
def pytest_runtest_setup(item):
    ...
    # تخطي اختبارات e2e إذا لم يكن Playwright أو Selenium متاحاً
    if "e2e" in item.keywords:
        # التحقق من Playwright أولاً
        try:
            from playwright.sync_api import sync_playwright
        except ImportError:
            # إذا لم يكن Playwright متاحاً، جرب Selenium
            try:
                from selenium import webdriver
            except ImportError:
                pytest.skip("Playwright أو Selenium غير متاح لاختبارات e2e")
