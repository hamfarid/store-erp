...
def get_setup_activity_summary(
    ...
    for log in logs:
        ...
        summary["status_counts"][log.status] = summary["status_counts"].get(
            log.status, 0) + 1

        # حساب عدد السجلات حسب الخطوة
        summary["step_counts"][log.step] = summary["step_counts"].get(
            log.step, 0) + 1

        # حساب عدد السجلات حسب المستخدم
        if log.user_id:
            summary["user_counts"][log.user_id] = summary["user_counts"].get(
                log.user_id, 0) + 1

        # حساب عدد السجلات حسب عنوان IP
        if log.ip_address:
            summary["ip_counts"][log.ip_address] = summary["ip_counts"].get(
                log.ip_address, 0) + 1

        # حساب توزيع السجلات حسب الساعة
        hour = log.created_at.hour
        summary["hourly_distribution"][hour] = summary["hourly_distribution"].get(
            hour, 0) + 1

        # حساب توزيع السجلات حسب اليوم
        day = log.created_at.weekday()
        summary["daily_distribution"][day] = summary["daily_distribution"].get(
            day, 0) + 1
