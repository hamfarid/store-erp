...
class EmailSettingsTest(BaseModel):
    ...
    test_recipient: str = Field(...,
                                description="البريد الإلكتروني المستلم للاختبار")
...
class LanguageInfo(BaseModel):
    ...
    rtl: bool = Field(...,
                      description="ما إذا كانت اللغة من اليمين إلى اليسار")
...
class SecuritySettings(BaseModel):
    ...
    password_expiry_days: int = Field(...,
                                      description="أيام انتهاء صلاحية كلمة المرور")
    enable_2fa: bool = Field(..., description="تفعيل المصادقة الثنائية")
    allowed_ips: List[str] = Field(
        default_factory=list,
        description="عناوين IP المسموح بها")
    cors_origins: List[str] = Field(
        default_factory=list,
        description="أصول CORS")
    ...
    rate_limiting: Dict[str,
                        Any] = Field(..., description="تحديد معدل الطلبات")
...
class SetupLogEntry(BaseModel):
    ...
    details: Dict[str, Any] = Field(
        default_factory=dict, description="التفاصيل")
