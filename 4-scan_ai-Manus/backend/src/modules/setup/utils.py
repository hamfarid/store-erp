...
def validate_security_configuration(
        security_settings: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    if not security_settings.get("use_ssl", False):
        warnings.append(
            "لم يتم تفعيل SSL، مما قد يعرض البيانات للخطر أثناء النقل.")
        recommendations.append(
            "قم بتفعيل SSL وتوفير شهادة SSL صالحة لتأمين الاتصالات.")

    # التحقق من مهلة الجلسة
    session_timeout = security_settings.get("session_timeout", 30)
    if session_timeout > 60:
        warnings.append(
            f"مهلة الجلسة طويلة جداً ({session_timeout} دقيقة)، مما قد يزيد من مخاطر الأمان.")
    ...
    if max_login_attempts > 10:
        warnings.append(
            f"الحد الأقصى لمحاولات تسجيل الدخول مرتفع جداً ({max_login_attempts})، مما قد يسمح بهجمات القوة الغاشمة.")
        recommendations.append(
            "قم بتقليل الحد الأقصى لمحاولات تسجيل الدخول إلى 5 محاولات.")
    ...
    if password_expiry_days > 180:
        warnings.append(
            f"مدة انتهاء صلاحية كلمة المرور طويلة جداً ({password_expiry_days} يوم)، مما قد يقلل من أمان النظام.")
        recommendations.append(
            "قم بتقليل مدة انتهاء صلاحية كلمة المرور إلى 90 يوم أو أقل.")

    # التحقق من تفعيل المصادقة الثنائية
    if not security_settings.get("enable_2fa", False):
        warnings.append(
            "لم يتم تفعيل المصادقة الثنائية، مما يقلل من مستوى الأمان.")
        recommendations.append(
            "قم بتفعيل المصادقة الثنائية لزيادة مستوى الأمان.")

    # التحقق من حماية XSS
    if not security_settings.get("xss_protection", True):
        warnings.append(
            "لم يتم تفعيل حماية XSS، مما قد يعرض النظام لهجمات البرمجة النصية عبر المواقع.")
        recommendations.append(
            "قم بتفعيل حماية XSS لحماية النظام من هجمات البرمجة النصية عبر المواقع.")

    # التحقق من حماية CSRF
    if not security_settings.get("csrf_protection", True):
        warnings.append(
            "لم يتم تفعيل حماية CSRF، مما قد يعرض النظام لهجمات تزوير الطلبات عبر المواقع.")
        recommendations.append(
            "قم بتفعيل حماية CSRF لحماية النظام من هجمات تزوير الطلبات عبر المواقع.")

    # التحقق من حماية حقن SQL
    if not security_settings.get("sql_injection_protection", True):
        warnings.append(
            "لم يتم تفعيل حماية حقن SQL، مما قد يعرض قاعدة البيانات للخطر.")
        recommendations.append(
            "قم بتفعيل حماية حقن SQL لحماية قاعدة البيانات من هجمات الحقن.")

    # التحقق من تحديد معدل الطلبات
    rate_limiting = security_settings.get("rate_limiting", {})
    if not rate_limiting.get("enabled", True):
        warnings.append(
            "لم يتم تفعيل تحديد معدل الطلبات، مما قد يعرض النظام لهجمات الحرمان من الخدمة.")
        recommendations.append(
            "قم بتفعيل تحديد معدل الطلبات لحماية النظام من هجمات الحرمان من الخدمة.")
    elif rate_limiting.get("requests_per_minute", 60) > 100:
        warnings.append(
            f"معدل الطلبات المسموح به مرتفع جداً ({rate_limiting.get('requests_per_minute', 60)} طلب في الدقيقة)، مما قد يقلل من فعالية الحماية.")
        recommendations.append(
            "قم بتقليل معدل الطلبات المسموح به إلى 60 طلب في الدقيقة أو أقل.")
...
def check_module_dependencies(
        module_id: str, enabled_modules: list) -> Dict[str, Any]:
    """
