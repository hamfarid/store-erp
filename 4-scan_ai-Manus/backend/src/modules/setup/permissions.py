...
def initialize_setup_permissions(db: Session) -> None:
    ...
    for permission_code, permission_description in SETUP_PERMISSIONS.items():
        ...
        existing_permission = db.query(Permission).filter(
            Permission.code == permission_code).first()
        ...
        if not existing_role:
            ...
            for permission_code in role_data["permissions"]:
                permission = db.query(Permission).filter(
                    Permission.code == permission_code).first()
                if permission:
                    auth_service.assign_permission_to_role(
                        db, new_role.id, permission.id)
        else:
            # تحديث الصلاحيات للدور
            existing_permissions = auth_service.get_role_permissions(
                db, existing_role.id)
            ...
            for permission_code in role_data["permissions"]:
                if permission_code not in existing_permission_codes:
                    ...
                    if permission:
                        auth_service.assign_permission_to_role(
                            db, existing_role.id, permission.id)
...
def check_setup_permission(
    ...
    if step_id and permission_code in OPERATION_PERMISSIONS:
        ...
        if isinstance(step_permissions, dict) and step_id in step_permissions:
            ...
            required_permissions = step_permissions if isinstance(
                step_permissions, list) else [step_permissions]
    ...
    for required_permission in required_permissions:
        if auth_service.check_user_permission(
...
def check_setup_token_permission(
    ...
    if setup_status.setup_token != token:
        log_token_validation_error(
            db,
            "محاولة استخدام رمز إعداد غير صالح",
            permission_code,
            step_id,
            ip_address)
        return False
...
def get_required_permissions_for_operation(
    ...
    if operation in OPERATION_PERMISSIONS:
        # التحقق من صلاحيات العملية للخطوة
        if step_id and isinstance(
                OPERATION_PERMISSIONS[operation],
                dict) and step_id in OPERATION_PERMISSIONS[operation]:
...
def validate_operation_permission(
    ...
    """
    # تحديد الصلاحيات المطلوبة
    required_permissions = get_required_permissions_for_operation(
        operation, step_id)
    ...
    # التحقق من صلاحية رمز الإعداد
    if token:
        return validate_token_permission(
            db, token, operation, step_id, ip_address)
...
def assign_setup_admin_role(db: Session, user_id: int) -> bool:
    ...
    setup_admin_role = db.query(Role).filter(
        Role.code == "setup_admin").first()
...
def get_user_setup_permissions(
        db: Session, user_id: int) -> List[Dict[str, Any]]:
    """
