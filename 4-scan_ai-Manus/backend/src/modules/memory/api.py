...
async def delete_memory(
    ...
    if permanent and not permission_service.check_permission(
            "memory", "permanent_delete"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
@router.get("/{memory_id}/access-logs",
            response_model=List[MemoryAccessLogResponse])
...
async def update_memory_importance(
    ...
    # تحديث درجة الأهمية
    memory_service = MemoryService(db)
    success = memory_service.update_memory_importance(
