...
from .models import MemoryAccess, MemoryCategory, MemoryType
...
class MemoryIntegration:
    ...
    def store_image_search_result(
        ...
        try:
            ...
            logger.info(
                "تم تخزين نتائج البحث عن الصور في الذاكرة: %s",
                memory.id)
            return memory.id

        except Exception as e:
            logger.error(
                "فشل تخزين نتائج البحث عن الصور في الذاكرة: %s",
                str(e))
            return ""

    def get_related_image_searches(
            self,
            query: str,
            limit: int = 5,
        ...
        try:
            ...
            logger.error(
                "فشل استرجاع عمليات البحث عن الصور ذات الصلة: %s",
                str(e))
            return []

    # ==================== تكامل مع مديول تشخيص النباتات ====================

    def store_plant_diagnosis(
            self,
            plant_name: str,
            diagnosis_data: Dict,
            images: List[str],
        ...
        try:
            ...
            logger.info(
                "تم تخزين نتائج تشخيص النباتات في الذاكرة: %s",
                memory.id)
            return memory.id

        except Exception as e:
            logger.error(
                "فشل تخزين نتائج تشخيص النباتات في الذاكرة: %s",
                str(e))
            return ""

    def get_plant_diagnosis_history(
            self,
    ...
    def store_ai_agent_interaction(
            self,
            agent_id: str,
        ...
        try:
            ...
            logger.info(
                "تم تخزين تفاعل وكيل الذكاء الاصطناعي في الذاكرة: %s",
                memory.id)
            return memory.id

        except Exception as e:
            logger.error(
                "فشل تخزين تفاعل وكيل الذكاء الاصطناعي في الذاكرة: %s",
                str(e))
            return ""

    def get_ai_agent_history(
            self,
        ...
        try:
            ...
            logger.error(
                "فشل استرجاع سجل تفاعلات وكيل الذكاء الاصطناعي: %s",
                str(e))
            return []

    # ==================== وظائف عامة ====================

    def semantic_search_across_modules(
            self,
            query: str,
            top_k: int = 5,
            user_id: Optional[str] = None) -> List[Dict]:
        ...
        try:
            ...
            results = self.memory_service.semantic_search(
                query, top_k, user_id)
