...
class EntityBase(BaseModel):
    ...
    meta_data: Optional[Dict[str, Any]] = Field(
        None, description=DESC_ADDITIONAL_METADATA)
...
class MemoryBase(BaseModel):
    ...
    access_level: MemoryAccessEnum = Field(
        MemoryAccessEnum.PRIVATE,
        description=DESC_MEMORY_ACCESS_LEVEL)
    ...
    importance_score: float = Field(
        0.0, ge=0.0, le=1.0, description="درجة أهمية الذاكرة")
    retention_days: int = Field(
        365, ge=1, description="عدد أيام الاحتفاظ بالذاكرة")
    meta_data: Optional[Dict[str, Any]] = Field(
        None, description=DESC_ADDITIONAL_METADATA)
...
class MemoryUpdate(BaseModel):
    ...
    content: Optional[str] = Field(
        None, min_length=1, description="محتوى الذاكرة")
    summary: Optional[str] = Field(None, description="ملخص الذاكرة")
    memory_type: Optional[MemoryTypeEnum] = Field(
        None, description=DESC_MEMORY_TYPE)
    category: Optional[MemoryCategoryEnum] = Field(
        None, description=DESC_MEMORY_CATEGORY)
    access_level: Optional[MemoryAccessEnum] = Field(
        None, description=DESC_MEMORY_ACCESS_LEVEL)
    source_module: Optional[str] = Field(None, description=DESC_SOURCE_MODULE)
    source_id: Optional[str] = Field(None, description="معرف المصدر")
    source_url: Optional[str] = Field(None, description="رابط المصدر")
    importance_score: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="درجة أهمية الذاكرة")
    retention_days: Optional[int] = Field(
        None, ge=1, description="عدد أيام الاحتفاظ بالذاكرة")
    meta_data: Optional[Dict[str, Any]] = Field(
        None, description=DESC_ADDITIONAL_METADATA)
    tags: Optional[List[str]] = Field(None, description=DESC_TAGS_LIST)
...
class MemoryResponse(MemoryBase):
    ...
    expiry_date: Optional[datetime] = Field(
        None, description="تاريخ انتهاء صلاحية الذاكرة")
...
class MemorySearch(BaseModel):
    ...
    created_after: Optional[datetime] = Field(
        None, description="تاريخ الإنشاء بعد")
    created_before: Optional[datetime] = Field(
        None, description="تاريخ الإنشاء قبل")
    min_importance: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="الحد الأدنى لدرجة الأهمية")
    include_archived: bool = Field(
        False, description="تضمين الذكريات المؤرشفة")
...
class SemanticSearchResult(BaseModel):
    ...
    similarity_score: float = Field(..., ge=0.0,
                                    le=1.0, description="درجة التشابه")
...
class MemoryAccessLogBase(BaseModel):
    ...
    details: Optional[Dict[str, Any]] = Field(
        None, description="تفاصيل إضافية")
...
class MemoryStats(BaseModel):
    ...
    memory_type_counts: List[MemoryTypeCount] = Field(
        ..., description="عدد الذكريات حسب النوع")
    category_counts: List[MemoryCategoryCount] = Field(
        ..., description="عدد الذكريات حسب الفئة")
    access_level_counts: List[MemoryAccessLevelCount] = Field(
        ..., description="عدد الذكريات حسب مستوى الوصول")
