...
from .config import REQUIRED_INTEGRATION_MODULES, PermissionScope, PermissionType
...
class ModuleIntegration:
    ...
    def unregister_module(self, module_name: str) -> bool:
        ...
        try:
            ...
            permissions = self.permission_service.get_permissions_by_prefix(
                f"{module_name}.")
            ...
            logger.error(
                f"خطأ أثناء إلغاء تسجيل المديول {module_name}: {str(e)}")
            return False
    ...
    def assign_module_permissions_to_role(
        ...
        try:
            ...
            logger.info(
                f"تم تعيين صلاحيات المديول {module_name} للدور {role_name} بنجاح")
            return True

        except Exception as e:
            logger.error(
                f"خطأ أثناء تعيين صلاحيات المديول {module_name} للدور {role_name}: {str(e)}")
            return False
