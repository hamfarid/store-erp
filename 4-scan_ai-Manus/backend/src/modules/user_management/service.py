...
from .models import Organization, Role, User, UserPreference, UserProfile, UserSession
from .schemas import (
...
class UserService:
    ...
    def create_session(
        ...
        token_payload = {
            ...
            "exp": datetime.utcnow() +
            timedelta(
                minutes=self.config.session_config.expire_minutes),
            "jti": str(
                uuid.uuid4())}
        token = jwt.encode(
            token_payload,
    ...
    def validate_session(self, token: str) -> Optional[UserSession]:
        ...
        """
        try:
            # فك تشفير الرمز (للتحقق من الصحة فقط)
            jwt.decode(
                token,
                self.config.session_config.secret_key,
    ...
    def increment_failed_login_attempts(self, user_id: str) -> Optional[User]:
        ...
        if user.failed_login_attempts >= self.config.password_policy.lockout_threshold:
            user.locked_until = datetime.now(
                timezone.utc) + timedelta(minutes=self.config.password_policy.lockout_duration_minutes)
    ...
    def create_organization(
            ...
            created_by: Optional[str] = None) -> Organization:
        """
        إنشاء مؤسسة جديدة
