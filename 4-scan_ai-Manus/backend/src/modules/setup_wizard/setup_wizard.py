...
class SetupWizard:
    ...
    def _load_default_config(self) -> None:
        ...
        try:
            if os.path.exists(self.config_path):
                ...
                logger.info(
                    f"تم تحميل التكوين الافتراضي من {self.config_path}")
            else:
                logger.warning(
                    f"ملف التكوين الافتراضي غير موجود: {self.config_path}")
    ...
    def next_step(self) -> Dict[str, Any]:
        ...
        if self.current_step < len(self.setup_steps) - 1:
            self.current_step += 1
            logger.info(
                f"الانتقال إلى الخطوة: {self.setup_steps[self.current_step]}")
        else:
            self.complete_setup()
    ...
    def previous_step(self) -> Dict[str, Any]:
        ...
        if self.current_step > 0:
            self.current_step -= 1
            logger.info(
                f"العودة إلى الخطوة: {self.setup_steps[self.current_step]}")
    ...
    def update_step_data(self, step_id: str, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not validation_result["valid"]:
                logger.error(
                    f"بيانات غير صالحة للخطوة {step_id}: {validation_result['errors']}")
                return False
    ...
    def _validate_step_data(
            self, step_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        # التحقق من صحة إعدادات النسخ الاحتياطي
            ...
            if "auto_backup" in data and data["auto_backup"].get(
                    "enabled", False):
                if not data["auto_backup"].get("frequency"):
                    errors.append(
                        "تكرار النسخ الاحتياطي مطلوب عند تفعيل النسخ الاحتياطي التلقائي")
    ...
    def complete_setup(self) -> bool:
        ...
        try:
            ...
            self.config["system_settings"]["setup_date"] = datetime.now(
            ).isoformat()
            ...
            for step_id in self.setup_steps:
                if step_id in imported_config:
                    validation_result = self._validate_step_data(
                        step_id, imported_config[step_id])
                    if not validation_result["valid"]:
                        logger.error(
                            f"بيانات غير صالحة في التكوين المستورد للخطوة {step_id}: {validation_result['errors']}")
                        return False
