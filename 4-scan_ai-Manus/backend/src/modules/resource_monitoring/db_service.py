# File:
# /home/ubuntu/ai_web_organized/src/modules/resource_monitoring/db_service.py
"""
...
from sqlalchemy import create_engine, desc, extract, func
...
def init_default_metrics(session):
    ...
    thresholds = [
        ResourceThreshold(
            metric_id=cpu_usage.id,
            warning_threshold=70,
            critical_threshold=90),
        ResourceThreshold(
            metric_id=memory_usage.id,
            warning_threshold=80,
            critical_threshold=95),
        ResourceThreshold(
            metric_id=disk_usage.id,
            warning_threshold=85,
            critical_threshold=95),
        ResourceThreshold(
            metric_id=db_connections.id,
            warning_threshold=80,
            critical_threshold=95)]
...
def get_metrics_by_type(resource_type):
    ...
    try:
        metrics = session.query(ResourceMetric).filter(
            ResourceMetric.resource_type == resource_type).all()
        return [metric.to_dict() for metric in metrics]
    except SQLAlchemyError as e:
        logger.error(
            f"خطأ أثناء استرجاع المقاييس من النوع {resource_type}: {str(e)}")
        return []
    finally:
        session.close()
...
def get_metric_by_id(metric_id):
    ...
    session = Session()
    try:
        metric = session.query(ResourceMetric).filter(
            ResourceMetric.id == metric_id).first()
...
def get_metric_by_name(metric_name):
    ...
    try:
        metric = session.query(ResourceMetric).filter(
            ResourceMetric.metric_name == metric_name).first()
        return metric.to_dict() if metric else None
    except SQLAlchemyError as e:
...
def delete_metric(metric_id):
    ...
    try:
        ...
        data_points_count = session.query(ResourceDataPoint).filter(
            ResourceDataPoint.metric_id == metric_id).count()
        if data_points_count > 0:
            logger.warning(
                f"لا يمكن حذف المقياس {metric_id} لأنه مرتبط بـ {data_points_count} نقطة بيانات")
            return False
        ...
        session.query(ResourceThreshold).filter(
            ResourceThreshold.metric_id == metric_id).delete()
    ...
    try:
        ...
        for data_point in data_points:
            ...
            if not metric_id:
                ...
                if not metric_name:
                    logger.warning(
                        "تم تخطي نقطة بيانات: يجب توفير معرف المقياس أو اسمه")
                    continue
                ...
                if metric_name in metrics_cache:
                    ...
                    if not metric:
                        logger.warning(
                            f"تم تخطي نقطة بيانات: المقياس {metric_name} غير موجود")
                        continue

                    metric_id = metric.id
                    metrics_cache[metric_name] = metric_id
            else:
                # التحقق من وجود المقياس بواسطة المعرف إذا لم يكن في التخزين
                # المؤقت
                if f"id_{metric_id}" not in metrics_cache:
                    ...
                    if not metric:
                        logger.warning(
                            f"تم تخطي نقطة بيانات: المقياس {metric_id} غير موجود")
                        continue
        ...
        for data_point_obj in added_points:
            check_thresholds(
                session,
                data_point_obj.metric_id,
                data_point_obj.value,
                data_point_obj.id)
...
def get_latest_data_point(metric_id=None, module_name=None, server_id=None):
    ...
    try:
        ...
        latest_point = query.order_by(
            desc(ResourceDataPoint.timestamp)).first()
...
def delete_old_data_points(days_to_keep=30):
    ...
    try:
        ...
        # حذف نقاط البيانات القديمة
        result = session.query(ResourceDataPoint).filter(
            ResourceDataPoint.timestamp < cutoff_date).delete()
...
def get_threshold_by_metric(metric_id):
    ...
    try:
        threshold = session.query(ResourceThreshold).filter(
            ResourceThreshold.metric_id == metric_id).first()
    ...
    try:
        ...
        if threshold:
            ...
            threshold = ResourceThreshold(
                ...
                is_active=threshold_data.get(
                    'isActive',
                    True),
                notification_enabled=threshold_data.get(
                    'notificationEnabled',
                    True))
            session.add(threshold)
...
def delete_threshold(threshold_id):
    ...
    try:
        threshold = session.query(ResourceThreshold).filter(
            ResourceThreshold.id == threshold_id).first()
        if not threshold:
            return None
    ...
    try:
        ...
        if alert_level and threshold.notification_enabled:
            ...
            metric = session.query(ResourceMetric).filter(
                ResourceMetric.id == metric_id).first()
            logger.warning(
                f"تنبيه {alert_level}: تجاوز المقياس {metric.metric_name} العتبة بقيمة {value} {metric.unit}")
...
def aggregate_data(
    ...
    try:
        ...
        for result in results:
            metric = session.query(ResourceMetric).filter(
                ResourceMetric.id == result.metric_id).first()
...
def store_aggregated_data(aggregation_type="hourly"):
    ...
    try:
        ...
        logger.info(
            f"تم تخزين البيانات المجمعة بنجاح لنوع التجميع: {aggregation_type}")
        return True
    except SQLAlchemyError as e:
        session.rollback()
...
def get_aggregated_data(
    ...
    try:
        ...
        if module_name:
            query = query.filter(
                ResourceAggregation.module_name == module_name)
        if server_id:
...
def get_server_by_id(server_id):
    ...
    try:
        server = session.query(Server).filter(
            Server.server_id == server_id).first()
