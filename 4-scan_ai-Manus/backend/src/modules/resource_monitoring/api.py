    ...
    try:
        ...
        if not check_permission(current_user, "view_resources"):
            raise HTTPException(status_code=403,
                                detail="ليس لديك صلاحية لعرض مقاييس الموارد")
...
async def create_metric(
    ...
    try:
        ...
        if not check_permission(current_user, "manage_resources"):
            raise HTTPException(
                status_code=403,
                detail=ERROR_MANAGE_METRICS_PERMISSION)
        ...
        if not metric:
            raise HTTPException(
                status_code=400,
                detail="فشل إنشاء المقياس، قد يكون موجوداً بالفعل")
...
async def delete_metric(
    ...
    try:
        ...
        if result is False:
            raise HTTPException(
                status_code=400,
                detail="لا يمكن حذف المقياس لأنه مرتبط بنقاط بيانات")
...
async def add_data_point(
    ...
    try:
        ...
        if not check_permission(current_user, "add_resource_data"):
            raise HTTPException(status_code=403,
                                detail="ليس لديك صلاحية لإضافة بيانات الموارد")

        # التحقق من صحة البيانات
        if not (data_point.get("metricId") or data_point.get("metricName")):
            raise HTTPException(
                status_code=400,
                detail="معرف المقياس أو اسمه مطلوب")
        if "value" not in data_point:
            raise HTTPException(
                status_code=400,
                detail="قيمة نقطة البيانات مطلوبة")

        result = db_service.add_data_point(data_point)
        if not result:
            raise HTTPException(
                status_code=400,
                detail="فشل إضافة نقطة البيانات")
...
async def add_multiple_data_points(
    ...
    try:
        ...
        if not data_points:
            raise HTTPException(
                status_code=400,
                detail="قائمة نقاط البيانات مطلوبة")
...
async def get_data_points(
    ...
    try:
        ...
        if not check_permission(current_user, "view_resources"):
            raise HTTPException(
                status_code=403,
                detail=ERROR_VIEW_RESOURCES_PERMISSION)
        ...
        if start_time:
            try:
                start_datetime = datetime.fromisoformat(
                    start_time.replace('Z', TIMEZONE_OFFSET))
            except ValueError as exc:
                raise HTTPException(
                    status_code=400,
                    detail="صيغة وقت البداية غير صالحة") from exc

        end_datetime = None
        if end_time:
            try:
                end_datetime = datetime.fromisoformat(
                    end_time.replace('Z', TIMEZONE_OFFSET))
            except ValueError as exc:
                raise HTTPException(
                    status_code=400,
                    detail="صيغة وقت النهاية غير صالحة") from exc
...
async def delete_old_data_points(
    ...
    try:
        ...
        if not check_permission(current_user, "manage_resources"):
            raise HTTPException(status_code=403,
                                detail="ليس لديك صلاحية لإدارة بيانات الموارد")
...
async def get_threshold_by_metric(
    ...
    try:
        ...
        logger.error(
            "خطأ في الحصول على العتبة للمقياس %s: %s",
            metric_id,
            str(e))
        raise HTTPException(status_code=500, detail=str(e)) from e
...
async def create_or_update_threshold(
    ...
    try:
        ...
        if not check_permission(current_user, "manage_resources"):
            raise HTTPException(status_code=403,
                                detail="ليس لديك صلاحية لإدارة عتبات الموارد")
    ...
    try:
        ...
        if start_time:
            try:
                ...
                raise HTTPException(
                    status_code=400,
                    detail="صيغة وقت البداية غير صالحة")
        ...
        if end_time:
            try:
                ...
                raise HTTPException(
                    status_code=400,
                    detail="صيغة وقت النهاية غير صالحة")
...
async def store_aggregated_data(
    ...
    try:
        ...
        if not check_permission(current_user, "manage_resources"):
            raise HTTPException(
                status_code=403,
                detail="ليس لديك صلاحية لإدارة البيانات المجمعة")
        ...
        return {"status": "success", "data": result,
                "message": "تم تخزين البيانات المجمعة بنجاح"}
    except HTTPException:
        raise
    except Exception as e:
...
async def create_or_update_server(
    ...
    try:
        ...
        if not check_permission(current_user, "manage_resources"):
            raise HTTPException(
                status_code=403,
                detail=ERROR_MANAGE_SYSTEM_PERMISSION)
...
async def collect_system_resources(
    ...
    try:
        ...
        return {"status": "success", "data": resources,
                "message": "تم جمع بيانات موارد النظام بنجاح"}
    except HTTPException:
        raise
    except Exception as e:
...
async def collect_database_resources(
    ...
    try:
        ...
        return {"status": "success", "data": resources,
                "message": "تم جمع بيانات موارد قاعدة البيانات بنجاح"}
    except HTTPException:
        raise
    except Exception as e:
...
async def collect_all_resources(
    ...
    try:
        ...
        return {"status": "success", "data": all_resources,
                "message": "تم جمع جميع بيانات الموارد بنجاح"}
    except HTTPException:
        raise
    except Exception as e:
...
async def get_resources_summary(
    ...
    try:
        ...
        if not check_permission(current_user, "view_resources"):
            raise HTTPException(status_code=403,
                                detail="ليس لديك صلاحية لعرض ملخص الموارد")
        ...
        for metric in metrics:
            metric_id = metric["id"]
            latest_point = db_service.get_latest_data_point(
                metric_id=metric_id)
            if latest_point:
...
async def get_resource_trends(
    ...
    try:
        ...
        if not check_permission(current_user, "view_resources"):
            raise HTTPException(status_code=403,
                                detail="ليس لديك صلاحية لعرض اتجاهات الموارد")
