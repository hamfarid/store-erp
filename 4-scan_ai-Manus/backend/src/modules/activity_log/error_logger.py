...
from src.modules.activity_log.models import ActivityLogSeverity, ActivityLogType
...
class ErrorLogger:
    ...
    def log_http_error(
        ...
        try:
            self.activity_log_service.create_log_entry(
                ...
                # استخدام الطابع الزمني كمعرف فريد
            ...
            logger.error(
                "Failed to log HTTP error to activity log: %s",
                str(e))
    ...
    def log_application_error(
        ...
        if additional_context:
            # تنظيف البيانات الحساسة قبل التسجيل
            sanitized_context = self._sanitize_sensitive_data(
                additional_context)
        ...
        try:
            ...
            # تسجيل الخطأ في السجل المحلي إذا فشل التسجيل في سجل النشاط
            logger.error(
                "Failed to log application error to activity log: %s",
                str(e))
            logger.error(traceback.format_exc())
    ...
    def log_security_error(
        ...
        try:
            ...
            # تسجيل الخطأ في السجل المحلي إذا فشل التسجيل في سجل النشاط
            logger.error(
                "Failed to log security error to activity log: %s",
                str(e))
            logger.error(traceback.format_exc())

    def _get_severity_from_http_code(
            self, error_code: int) -> ActivityLogSeverity:
        """
        ...
        def _sanitize_recursive(obj):
            if isinstance(obj, dict):
                ...
                for k, v in obj.items():
                    ...
                    is_sensitive = any(field.lower() in k.lower()
