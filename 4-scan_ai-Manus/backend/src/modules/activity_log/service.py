...
async def create_activity_log(db: Session,
                              log_data: Union[schemas.ActivityLogCreate,
                                              schemas.UserActionLogCreate,
                                              schemas.SystemActionLogCreate,
                                              schemas.AIActionLogCreate],
                              background_tasks: Optional[BackgroundTasks] = None) -> models.ActivityLog:
    """
    إنشاء سجل نشاط جديد
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"خطأ في إنشاء سجل النشاط: {str(e)}") from e
...
async def get_activity_logs(
    ...
    try:
        ...
        if filters.log_type:
            query = query.filter(
                models.ActivityLog.log_type == filters.log_type)

        if filters.module_id:
            query = query.filter(
                models.ActivityLog.module_id == filters.module_id)

        if filters.action_id:
            query = query.filter(
                models.ActivityLog.action_id == filters.action_id)
        ...
        if filters.start_date:
            query = query.filter(
                models.ActivityLog.created_at >= filters.start_date)

        if filters.end_date:
            query = query.filter(
                models.ActivityLog.created_at <= filters.end_date)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"خطأ في الحصول على سجلات النشاط: {str(e)}") from e
...
async def get_activity_log_by_id(
    ...
    try:
        log = db.query(
            models.ActivityLog).filter(
            models.ActivityLog.id == log_id).first()
        ...
        raise HTTPException(
            status_code=500,
            detail=f"خطأ في الحصول على سجل النشاط: {str(e)}") from e
...
async def get_system_log_by_activity_id(
    ...
    try:
        log = db.query(
            models.SystemLog).filter(
            models.SystemLog.activity_log_id == activity_log_id).first()
        ...
        raise HTTPException(
            status_code=500,
            detail=f"خطأ في الحصول على سجل النظام: {str(e)}") from e
...
async def get_user_log_by_activity_id(
    ...
    try:
        log = db.query(
            models.UserLog).filter(
            models.UserLog.activity_log_id == activity_log_id).first()

        if not log:
            raise HTTPException(
                status_code=404,
                detail="سجل المستخدم غير موجود")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"خطأ في الحصول على سجل المستخدم: {str(e)}") from e
...
async def get_ai_log_by_activity_id(
    ...
    try:
        log = db.query(
            models.AILog).filter(
            models.AILog.activity_log_id == activity_log_id).first()

        if not log:
            raise HTTPException(
                status_code=404,
                detail="سجل الذكاء الاصطناعي غير موجود")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"خطأ في الحصول على سجل الذكاء الاصطناعي: {str(e)}") from e
...
async def get_log_modules(
    ...
    try:
        return db.query(
            models.LogModule).filter(
            models.LogModule.is_active == 1).all()

    except Exception as e:
        logger.error("خطأ في الحصول على مديولات السجل: %s", str(e))
        raise HTTPException(
            status_code=500,
            detail=f"خطأ في الحصول على مديولات السجل: {str(e)}") from e
...
async def get_log_actions(
    ...
    try:
        query = db.query(
            models.LogAction).filter(
            models.LogAction.is_active == 1)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"خطأ في الحصول على إجراءات السجل: {str(e)}") from e
...
async def get_log_users(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"خطأ في الحصول على مستخدمي السجل: {str(e)}") from e
...
async def get_activity_log_statistics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"خطأ في الحصول على إحصائيات سجل النشاط: {str(e)}") from e
...
async def export_activity_logs(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"خطأ في تصدير سجلات النشاط: {str(e)}") from e
...
def export_to_csv(logs: List[Dict[str, Any]]) -> bytes:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"خطأ في تصدير السجلات إلى CSV: {str(e)}") from e
...
def export_to_xlsx(logs: List[Dict[str, Any]]) -> bytes:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"خطأ في تصدير السجلات إلى XLSX: {str(e)}") from e
...
def export_to_pdf(logs: List[Dict[str, Any]]) -> bytes:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"خطأ في تصدير السجلات إلى PDF: {str(e)}") from e
...
async def cleanup_old_logs(
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "تم جدولة تنظيف السجلات القديمة"}

    except Exception as e:
        logger.error("خطأ في تنظيف السجلات القديمة: %s", str(e))
        raise HTTPException(
            status_code=500,
            detail=f"خطأ في تنظيف السجلات القديمة: {str(e)}") from e
...
async def archive_logs(
    ...
    try:
        ...
        archive_file = os.path.join(
            archive_path, f"{log_type}_logs_{timestamp}.json")
        ...
        logger.info("تم أرشفة %d سجل من نوع %s إلى %s",
                    len(logs), log_type, archive_file)
...
async def log_user_action(
    ...
    try:
        ...
        log_data = schemas.ActivityLogCreate(
            ...
            log_data={
                "user_id": action_data.user_id,
                "action_type": action_data.action_type if hasattr(
                    action_data,
                    'action_type') else None,
                "resource_type": action_data.resource_type if hasattr(
                    action_data,
                    'resource_type') else None,
                "resource_id": action_data.resource_id if hasattr(
                    action_data,
                    'resource_id') else None,
                "before_state": action_data.before_state if hasattr(
                    action_data,
                    'before_state') else None,
                "after_state": action_data.after_state if hasattr(
                    action_data,
                    'after_state') else None},
        ...
        raise HTTPException(
            status_code=500,
            detail=f"خطأ في تسجيل إجراء المستخدم: {str(e)}") from e
...
async def log_ai_interaction(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"خطأ في تسجيل تفاعل الذكاء الاصطناعي: {str(e)}") from e
...
async def log_activity(
    ...
    try:
        ...
        raise HTTPException(status_code=500,
                            detail=f"خطأ في تسجيل النشاط: {str(e)}") from e
