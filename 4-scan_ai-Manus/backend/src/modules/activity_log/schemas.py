...
class ActivityLogBase(BaseModel):
    ...
    log_data: Optional[Dict[str, Any]] = Field(
        None, description="بيانات السجل")
...
class SystemLogBase(BaseModel):
    ...
    severity: str = Field(...,
                          description="مستوى الخطورة (critical, error, warning, info, debug)")
...
class UserLogBase(BaseModel):
    ...
    before_state: Optional[Dict[str, Any]] = Field(
        None, description="الحالة قبل التغيير")
    after_state: Optional[Dict[str, Any]] = Field(
        None, description="الحالة بعد التغيير")
...
class ActivityLogFilter(BaseModel):
    ...
    log_type: Optional[str] = Field(
        None, description="نوع السجل (system, user, ai)")
...
class ActivityLogExport(BaseModel):
    ...
    export_format: str = Field(
        "csv", description="صيغة التصدير (csv, xlsx, pdf)")
...
class ActivityLogStatistics(BaseModel):
    ...
    logs_by_module: Dict[str,
                         int] = Field(..., description="السجلات حسب المديول")
    logs_by_status: Dict[str,
                         int] = Field(..., description="السجلات حسب الحالة")
    logs_by_date: Dict[str,
                       int] = Field(..., description="السجلات حسب التاريخ")
