...
from modules.image_search.models import (
    Crop,
    Disease,
    ImageSearchHistory,
    ImageTag,
...
from modules.image_search.schemas import (
    CropImageSearchRequest,
    DiseaseImageSearchRequest,
    ImageCollectionRequest,
    ImageCollectionResponse,
    ImageSearchRequest,
    ImageSearchResponse,
    ImageUploadResponse,
    PestImageSearchRequest,
)
...
async def search_images(
    ...
    try:
        ...
        raise HTTPException(status_code=500,
                            detail=f"فشل البحث عن الصور: {str(e)}") from e
    ...
    try:
        ...
        for img_data in collected_images:
            ...
            plant_image = PlantImage(
                ...
                file_format=img_data["format"].lower() if isinstance(
                    img_data["format"],
                    str) else None,
                ...
                user_id=current_user.id,
                organization_id=current_user.organization_id if hasattr(
                    current_user,
                    "organization_id") else None,
                branch_id=current_user.branch_id if hasattr(
                    current_user,
                    "branch_id") else None,
        ...
        raise HTTPException(status_code=500,
                            detail=f"فشل جمع الصور: {str(e)}") from e
...
async def search_disease_images(
    ...
    try:
        ...
        disease = db.query(Disease).filter(
            Disease.id == request.disease_id).first()
        if not disease:
            raise HTTPException(
                status_code=404,
                detail=f"المرض برقم المعرف {request.disease_id} غير موجود")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"فشل البحث عن صور المرض: {str(e)}") from e
...
async def search_pest_images(
    ...
    try:
        ...
        if not pest:
            raise HTTPException(
                status_code=404,
                detail=f"الآفة برقم المعرف {request.pest_id} غير موجود")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"فشل البحث عن صور الآفة: {str(e)}") from e
...
async def search_crop_images(
    ...
    try:
        ...
        if not crop:
            raise HTTPException(
                status_code=404,
                detail=f"المحصول برقم المعرف {request.crop_id} غير موجود")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"فشل البحث عن صور المحصول: {str(e)}") from e


def _add_disease_to_image(
        db: Session,
        plant_image: PlantImage,
        disease_id: int) -> None:
...
def _create_plant_image(
        file: UploadFile,
        file_path: str,
        title: str,
        description: str,
        current_user) -> PlantImage:
...
async def upload_image(
    ...
    try:
        ...
        # إنشاء سجل الصورة
        plant_image = _create_plant_image(
            file, file_path, title, description, current_user)
        db.add(plant_image)
        db.commit()
        db.refresh(plant_image)
        ...
        raise HTTPException(status_code=500,
                            detail=f"فشل رفع الصورة: {str(e)}") from e
