...
class ImageSearchRequest(BaseModel):
    ...
    search_params: Dict[str, Any] = Field(
        default_factory=dict, description="معلمات البحث الإضافية")
...
class ImageCollectionRequest(BaseModel):
    ...
    keywords: List[str] = Field(...,
                                description="قائمة الكلمات المفتاحية للبحث")
    max_images_per_keyword: int = Field(
        20, description="الحد الأقصى لعدد الصور لكل كلمة مفتاحية", ge=1, le=100)
...
class ImageCollectionResponse(BaseModel):
    ...
    stored_images: List[Dict[str, Any]
                        ] = Field(..., description="قائمة الصور المخزنة")
...
class CropImageSearchRequest(BaseModel):
    ...
    condition: Optional[str] = Field(
        None, description="حالة المحصول (مثل 'صحي'، 'مريض')")
    count: int = Field(10, description=RESULTS_COUNT, ge=1, le=100)
...
class PlantImageCreate(PlantImageBase):
    ...
    crop_ids: Optional[List[int]] = Field(
        None, description="قائمة معرفات المحاصيل")
    tag_ids: Optional[List[int]] = Field(
        None, description="قائمة معرفات الوسوم")
...
class PlantImageResponse(PlantImageBase):
    ...
    crops: Optional[List[CropResponse]] = Field(
        None, description="قائمة المحاصيل")
    tags: Optional[List[ImageTagResponse]] = Field(
        None, description="قائمة الوسوم")
