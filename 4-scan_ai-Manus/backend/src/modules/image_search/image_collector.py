...
class ImageCollector:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.min_image_size = self.config.get(
            ...
            10 * 1024)  # 10KB الحد الأدنى الافتراضي للحجم
        self.min_image_dim = self.config.get(
            "min_image_dimension",
            200)  # الحد الأدنى الافتراضي للعرض/الارتفاع
        self.search_api_params = self.config.get("search_api_params", {})

    def collect_images_by_keywords(
            self, keywords: List[str], max_images_per_keyword: int = 20) -> List[Dict[str, Any]]:
        ...
        if not keywords or not isinstance(keywords, list):
            logger.error(
                "الكلمات المفتاحية مفقودة أو غير صالحة (يجب أن تكون قائمة)")
            return []
        ...
        for query in keywords:
            ...
            try:
                ...
                if not image_urls:
                    logger.warning(
                        "لم يتم العثور على عناوين URL للصور للاستعلام: '%s'", query)
                    continue

                # تنظيف الاستعلام للاستخدام في المسار
                safe_query_folder = "".join(
                    c for c in query if c.isalnum() or c in (
                        '_', '-')).rstrip()
                keyword_path = os.path.join(
                    self.download_path, safe_query_folder)
                ...
                for img_url in image_urls:
                    try:
                        ...
                        logger.debug(
                            "الانتظار %.2fث قبل تنزيل %s", delay, img_url)
                        time.sleep(delay)
                        ...
                        if not content_type.startswith("image"):
                            logger.warning(
                                "تخطي نوع محتوى غير صورة ('%s') للعنوان: %s", content_type, img_url)
                            continue
                        ...
                        if len(img_data) < self.min_image_size:
                            logger.warning(
                                "تخطي الصورة (صغيرة جدًا: %s بايت) من %s", len(img_data), img_url)
                            continue
                        ...
                        try:
                            with Image.open(io.BytesIO(img_data)) as img:
                                ...
                                if width < self.min_image_dim or height < self.min_image_dim:
                                    logger.warning(
                                        "تخطي الصورة (الأبعاد %sx%s صغيرة جدًا) من %s", width, height, img_url)
                                    continue
                                img_format = img.format or "UNKNOWN"  # الحصول على التنسيق
                        except Exception as img_val_e:
                            logger.warning(
                                "لا يمكن التحقق من بيانات الصورة من %s: %s", img_url, img_val_e)
                        ...
                        logger.info(
                            "تم تنزيل الصورة والتحقق منها بنجاح إلى: %s", save_path)
                        collected_image_metadata.append({
                            "file_path": save_path,
                            "source_url": img_url,
                            "query": query,
                        ...
                        logger.warning(
                            "فشل تنزيل الصورة %s: %s", img_url, img_e)
                    except IOError as io_e:
                        logger.error(
                            "فشل حفظ الصورة %s إلى %s: %s", img_url, save_path, io_e)
                    except Exception as gen_e:
                        logger.error(
                            "خطأ غير متوقع أثناء تنزيل/حفظ %s: %s",
                            img_url,
                            gen_e,
                            exc_info=True)

                logger.info(
                    "تم تنزيل %s صور جديدة للاستعلام '%s'.",
                    download_count_for_query,
                    query)

            except Exception as search_e:
                logger.error(
                    "خطأ أثناء عملية البحث/تنزيل الصور للاستعلام '%s': %s",
                    query,
                    search_e,
                    exc_info=True)

        logger.info(
            "اكتمل جمع صور البحث. إجمالي الصور الجديدة التي تم تنزيلها: %s",
            len(collected_image_metadata))
        # إرجاع قائمة قواميس البيانات الوصفية
        return collected_image_metadata

    def collect_images_by_disease(
            self, disease_name: str, max_images: int = 20) -> List[Dict[str, Any]]:
        ...
        return self.collect_images_by_keywords(
            keywords, max_images // len(keywords))

    def collect_images_by_pest(
            self, pest_name: str, max_images: int = 20) -> List[Dict[str, Any]]:
    ...
    def collect_images_by_crop(self,
                               crop_name: str,
                               condition: str = None,
                               max_images: int = 20) -> List[Dict[str,
                                                                  Any]]:
    ...
    def _generate_save_path(self, base_dir: str, url: str) -> str:
        ...
        try:
            ...
            # الحصول على المسار، وإزالة الشرطات المائلة في البداية/النهاية، وفك
            # ترميز URL
            path_part = unquote(parsed_url.path.strip('/'))
            if not path_part:
                # إذا كان المسار فارغًا، استخدم المجال + تجزئة عنوان URL الكامل
                base_name = parsed_url.netloc + "_" + \
                    hashlib.sha1(url.encode(), usedforsecurity=False).hexdigest()[:8]
            else:
            ...
            # تنظيف جذع اسم الملف
            safe_stem = "".join(
                c for c in filename_stem if c.isalnum() or c in (
                    '_', '-')).rstrip()
            ...
            # إضافة جزء من تجزئة عنوان URL لتقليل التصادمات لأسماء الملفات
            # المتشابهة من عناوين URL مختلفة
            url_hash_part = hashlib.sha1(url.encode(), usedforsecurity=False).hexdigest()[:6]
            ...
            logger.warning(
                "خطأ في توليد اسم الملف لعنوان URL %s: %s. استخدام احتياطي عشوائي.", url, e)
            fallback_name = f"image_{random.randint(10000, 99999)}_{hashlib.sha1(url.encode(), usedforsecurity=False).hexdigest()[:6]}.jpg"
