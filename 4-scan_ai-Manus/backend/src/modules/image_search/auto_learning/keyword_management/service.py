...
class KeywordManagementService:
    ...
    def _keyword_to_response(
            self,
            keyword: SearchKeyword) -> SearchKeywordResponse:
        """
    ...
    def _category_to_response(
            self,
            category: KeywordCategory) -> KeywordCategoryResponse:
        """
    ...
    def create_keyword(
            self,
            keyword_data: SearchKeywordCreate) -> SearchKeywordResponse:
        """
        ...
        try:
            ...
            if existing_keyword:
                raise ValueError(
                    f"الكلمة المفتاحية موجودة مسبقاً: {keyword_data.keyword}")
            ...
            new_keyword = SearchKeyword(
                ...
                semantic_relations=self._process_semantic_relations(
                    keyword_data.semantic_relations),
    ...
    def _process_semantic_relations(
            self, relations: Optional[List[SemanticRelation]]) -> Optional[List[Dict[str, Any]]]:
        """
        ...
        for relation in relations:
            ...
            if not related_keyword:
                logger.warning(
                    f"الكلمة المفتاحية المرتبطة غير موجودة: {relation.related_keyword_id}")
                continue
    ...
    def update_keyword(
            self,
            keyword_id: int,
            keyword_data: SearchKeywordUpdate) -> SearchKeywordResponse:
        """
        ...
        try:
            ...
            keyword = self.db.query(SearchKeyword).filter(
                SearchKeyword.id == keyword_id).first()
            ...
            if "semantic_relations" in update_data:
                update_data["semantic_relations"] = self._process_semantic_relations(
                    update_data["semantic_relations"])
    ...
    def get_keyword_by_id(
            self,
            keyword_id: int) -> Optional[SearchKeywordResponse]:
        """
    ...
    def get_keywords(self,
                     ...
                     sort_order: str = "desc") -> Tuple[List[SearchKeywordResponse],
                                                        int]:
        """
        ...
        try:
            ...
            if condition_type:
                query = query.filter(
                    SearchKeyword.condition_type == condition_type)
            ...
            return [self._keyword_to_response(keyword)
                    for keyword in keywords], total_count

        except Exception as e:
            logger.error(
                "حدث خطأ أثناء الحصول على قائمة الكلمات المفتاحية: %s",
                str(e))
            logger.error(traceback.format_exc())
            raise
    ...
    def get_priority_keywords(
            self,
            limit: int = 10) -> List[SearchKeywordResponse]:
        """
        ...
        try:
            ...
            logger.error(
                "حدث خطأ أثناء الحصول على الكلمات المفتاحية ذات الأولوية: %s",
                str(e))
            logger.error(traceback.format_exc())
            raise
    ...
    def update_keyword_last_search(self, keyword_id: int) -> bool:
        ...
        try:
            ...
            self.db.rollback()
            logger.error(
                "حدث خطأ أثناء تحديث آخر وقت بحث للكلمة المفتاحية: %s",
                str(e))
            logger.error(traceback.format_exc())
            raise

    def update_keyword_success_rate(
            self,
            keyword_id: int,
            success: bool,
        ...
        try:
            ...
            keyword.success_count = (
                keyword.success_count or 0) + (1 if success else 0)
            keyword.total_results = (
                keyword.total_results or 0) + results_count

            # حساب معدل النجاح
            if keyword.search_count and keyword.search_count > 0:
                keyword.success_rate = (
                    keyword.success_count / keyword.search_count) * 100
            ...
            self.db.rollback()
            logger.error(
                "حدث خطأ أثناء تحديث معدل نجاح الكلمة المفتاحية: %s",
                str(e))
            logger.error(traceback.format_exc())
            raise
    ...
    def get_keyword_statistics(self) -> KeywordStatistics:
        ...
        try:
            ...
            total_keywords = self.db.query(
                func.count(SearchKeyword.id)).scalar() or 0

            # عدد الكلمات المفتاحية النشطة
            # pylint: disable=not-callable
            active_keywords = self.db.query(
                func.count(
                    SearchKeyword.id)).filter(
                SearchKeyword.is_active).scalar() or 0

            # متوسط معدل النجاح
            avg_success_rate = self.db.query(
                func.avg(SearchKeyword.success_rate)).scalar() or 0

            # إجمالي عدد عمليات البحث
            # pylint: disable=not-callable
            total_searches = self.db.query(
                func.sum(SearchKeyword.search_count)).scalar() or 0

            # إجمالي عدد النتائج
            # pylint: disable=not-callable
            total_results = self.db.query(
                func.sum(SearchKeyword.total_results)).scalar() or 0
            ...
            logger.error(
                "حدث خطأ أثناء الحصول على إحصائيات الكلمات المفتاحية: %s",
                str(e))
            logger.error(traceback.format_exc())
            raise
    ...
    def create_category(
            self,
            category_data: KeywordCategoryCreate) -> KeywordCategoryResponse:
        """
        ...
        try:
            ...
            if existing_category:
                raise ValueError(
                    f"فئة الكلمات المفتاحية موجودة مسبقاً: {category_data.name}")
            ...
            logger.info(
                "تم إنشاء فئة كلمات مفتاحية جديدة: %s",
                new_category.name)
