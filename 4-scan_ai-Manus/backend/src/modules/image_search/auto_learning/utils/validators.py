...
def validate_keyword(keyword_data: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    if 'category' not in keyword_data or not keyword_data['category']:
        ...
        errors['category'] = [
            f"تصنيف الكلمة المفتاحية غير صالح. القيم المسموح بها: {', '.join(CATEGORIES)}"]
    ...
    if 'plant_parts' in keyword_data and keyword_data['plant_parts']:
        if not isinstance(keyword_data['plant_parts'], list):
            ...
            errors['plant_parts'] = [
                f"أجزاء النبات غير صالحة. القيم المسموح بها: {', '.join(PLANT_PARTS)}"]
...
def validate_source(source_data: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    if 'trust_level' not in source_data:
        ...
        errors['trust_level'] = [
            f"مستوى الثقة يجب أن يكون بين {TRUST_LEVEL_MIN} و {TRUST_LEVEL_MAX}"]
...
def validate_search_engine(
        engine_data: Dict[str, Any]) -> Dict[str, List[str]]:
    """
...
def validate_keyword_relation(
        relation_data: Dict[str, Any]) -> Dict[str, List[str]]:
    """
    ...
    if 'source_id' not in relation_data:
        ...
        errors['source_id'] = [
            "معرف الكلمة المفتاحية المصدر يجب أن يكون رقماً صحيحاً"]
    ...
    if 'related_id' not in relation_data:
        ...
        errors['related_id'] = [
            "معرف الكلمة المفتاحية المرتبطة يجب أن يكون رقماً صحيحاً"]
    ...
    if 'relation_type' not in relation_data or not relation_data['relation_type']:
        ...
        errors['relation_type'] = [
            f"نوع العلاقة غير صالح. القيم المسموح بها: {', '.join(RELATION_TYPES)}"]
    ...
    if 'source_id' in relation_data and 'related_id' in relation_data:
        if relation_data['source_id'] == relation_data['related_id']:
            errors['related_id'] = [
                "لا يمكن إنشاء علاقة بين الكلمة المفتاحية ونفسها"]
