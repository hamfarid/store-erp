# File:
# /home/ubuntu/gaara_scan_ai_final_4.2/src/modules/ai_agent/memory_integration.py
"""
...
from src.modules.memory.models import MemoryEntry, MemoryType
...
class AIAgentMemoryIntegration:
    ...
    async def store_agent_memory(
        ...
        try:
            ...
            logger.info(
                "تم تخزين ذاكرة الوكيل %s بنجاح، المعرف: %s",
                agent_id,
                memory_id)
            return memory_id
    ...
    async def retrieve_agent_memories(
        ...
        try:
            ...
            logger.info(
                "تم استرجاع %s ذكريات للوكيل %s",
                len(memories),
                agent_id)
            return memories
        except Exception as e:
            logger.error(
                "فشل في استرجاع ذكريات الوكيل %s: %s",
                agent_id,
                str(e))
            raise
    ...
    async def update_agent_memory(
        ...
        try:
            ...
            if success:
                logger.info(
                    "تم تحديث ذاكرة الوكيل بنجاح، المعرف: %s",
                    memory_id)
            else:
                logger.warning(
                    "لم يتم العثور على ذاكرة بالمعرف: %s", memory_id)
            return success
        except Exception as e:
            logger.error(
                "فشل في تحديث ذاكرة الوكيل، المعرف: %s: %s",
                memory_id,
                str(e))
            raise
    ...
    async def delete_agent_memory(self, memory_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                "فشل في حذف ذاكرة الوكيل، المعرف: %s: %s",
                memory_id,
                str(e))
            raise

    async def get_agent_memory_by_id(
            self, memory_id: str) -> Optional[MemoryEntry]:
        """
        ...
        try:
            ...
            if memory:
                logger.info(
                    "تم استرجاع ذاكرة الوكيل بنجاح، المعرف: %s",
                    memory_id)
            else:
                logger.warning(
                    "لم يتم العثور على ذاكرة بالمعرف: %s", memory_id)
            ...
            logger.error(
                "فشل في استرجاع ذاكرة الوكيل، المعرف: %s: %s",
                memory_id,
                str(e))
            raise
    ...
    async def get_agent_memory_stats(self, agent_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for memory in memories:
                ...
                stats["memory_types"][memory_type] = stats["memory_types"].get(
                    memory_type, 0) + 1
            ...
            if memories:
                stats["average_memory_size"] = sum(
                    len(m.content) for m in memories) / len(memories)
            else:
            ...
            logger.error(
                "فشل في حساب إحصائيات ذاكرة الوكيل %s: %s",
                agent_id,
                str(e))
            raise
