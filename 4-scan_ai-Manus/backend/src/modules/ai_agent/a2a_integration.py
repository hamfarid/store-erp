...
from src.modules.ai_agent.models import AgentStatus, AIAgent
...
class A2AIntegration:
    ...
    async def send_message(self, message: A2AMessage) -> bool:
        ...
        """
        try:
            if not self._check_send_permission(
    ...
    async def _handle_response(self, message: A2AMessage) -> None:
        ...
        try:
            ...
            if not request_id:
                logger.warning(
                    "معرف الطلب غير موجود في البيانات الوصفية للاستجابة")
                return
    ...
    def _apply_receiver_filters(
            self, query, receiver_filter: Optional[Dict[str, Any]]):
        """
    ...
    async def _execute_agent_request(
            self, agent, content: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
    ...
    async def _process_agent_response(
            self, agent, message: A2AMessage) -> None:
        """
        ...
        try:
            # معالجة الاستجابة
            logger.info(
                "تم معالجة استجابة من الوكيل %s: %s",
                agent.id,
                message.content)
        except Exception as e:
            logger.error("فشل معالجة استجابة الوكيل: %s", str(e))

    async def _process_agent_notification(
            self, agent, message: A2AMessage) -> None:
        """
        معالجة إشعار الوكيل
    ...
    async def _process_agent_error(self, agent, message: A2AMessage) -> None:
        ...
        try:
            # معالجة الخطأ
            logger.warning(
                "تم معالجة خطأ من الوكيل %s: %s",
                agent.id,
                message.content)
        except Exception as e:
            logger.error("فشل معالجة خطأ الوكيل: %s", str(e))

    async def _process_agent_broadcast(
            self, agent, message: A2AMessage) -> None:
        """
        معالجة بث الوكيل
        ...
        try:
            # معالجة البث
            logger.info(
                "تم معالجة بث من الوكيل %s: %s",
                agent.id,
                message.content)
        except Exception as e:
            logger.error("فشل معالجة بث الوكيل: %s", str(e))

    async def _notify_admin_of_error(self,
                                     function_name: str,
                                     error_message: str,
                                     context: Dict[str,
                                                   Any]) -> None:
