...
class AIProviderConfig(BaseModel):
    ...
    models: Dict[str, Dict[str, str | int | float | bool]] = Field(
        default_factory=dict, description="النماذج المتاحة وإعداداتها")
...
class AgentConfig(BaseModel):
    ...
    default_temperature: float = Field(
        0.7, description="درجة حرارة النموذج الافتراضية")
    default_max_tokens: int = Field(
        2000, description="الحد الأقصى للرموز الافتراضي")
    default_timeout_seconds: int = Field(
        60, description="مهلة تنفيذ المهمة الافتراضية بالثواني")
    default_memory_retention: int = Field(
        10, description="عدد الرسائل المحتفظ بها في الذاكرة افتراضياً")
    default_system_prompt: str = Field(
        "أنت مساعد ذكي يساعد المستخدمين في نظام Gaara ERP.",
        description="الرسالة النظامية الافتراضية")
    max_conversation_tokens: int = Field(
        8000, description="الحد الأقصى لرموز المحادثة")
    enable_function_calling: bool = Field(
        True, description="تمكين استدعاء الدوال")
    enable_streaming: bool = Field(
        True, description="تمكين البث المباشر للاستجابات")
...
class MonitoringConfig(BaseModel):
    ...
    enable_performance_tracking: bool = Field(
        True, description="تمكين تتبع الأداء")
    ...
    performance_metrics_retention_days: int = Field(
        90, description="مدة الاحتفاظ بمقاييس الأداء بالأيام")
    alert_on_error: bool = Field(
        True, description="إرسال تنبيهات عند حدوث أخطاء")
    alert_on_high_latency: bool = Field(
        True, description="إرسال تنبيهات عند ارتفاع زمن الاستجابة")
    high_latency_threshold_ms: int = Field(
        5000, description="عتبة زمن الاستجابة المرتفع بالمللي ثانية")
    sampling_rate: float = Field(
        0.1, description="معدل أخذ العينات للمراقبة المفصلة")


class IntegrationConfig(BaseModel):
    """تكوين تكامل الوكلاء مع المديولات الأخرى"""
    memory_module_enabled: bool = Field(
        True, description="تمكين التكامل مع مديول الذاكرة")
    a2a_module_enabled: bool = Field(
        True, description="تمكين التكامل مع مديول A2A")
    permissions_module_enabled: bool = Field(
        True, description="تمكين التكامل مع مديول الصلاحيات")
    telegram_bot_enabled: bool = Field(
        False, description="تمكين التكامل مع بوت تيليجرام")
    enable_cross_module_function_calling: bool = Field(
        True, description="تمكين استدعاء دوال المديولات الأخرى")
    enable_agent_collaboration: bool = Field(
        True, description="تمكين التعاون بين الوكلاء")


class AIAgentConfig(BaseModel):
    """التكوين الرئيسي لمديول وكلاء الذكاء الاصطناعي"""
    providers: Dict[str, AIProviderConfig] = Field(
        default_factory=dict, description="مزودو خدمات الذكاء الاصطناعي")
    default_provider: str = Field(
        "openai", description="مزود الخدمة الافتراضي")
    agent: AgentConfig = Field(
        default_factory=AgentConfig,
        description="تكوين الوكيل الافتراضي")
    monitoring: MonitoringConfig = Field(
        default_factory=MonitoringConfig,
        description="تكوين المراقبة")
    integration: IntegrationConfig = Field(
        default_factory=IntegrationConfig,
        description="تكوين التكامل")
    enable_module: bool = Field(True, description="تمكين المديول")
    debug_mode: bool = Field(False, description="وضع التصحيح")
    cache_enabled: bool = Field(
        True, description="تمكين التخزين المؤقت للاستجابات")
    cache_ttl_seconds: int = Field(
        3600, description="مدة صلاحية التخزين المؤقت بالثواني")
    rate_limit_per_minute: int = Field(
        60, description="حد معدل الطلبات في الدقيقة")
    max_parallel_requests: int = Field(
        10, description="الحد الأقصى للطلبات المتوازية")
