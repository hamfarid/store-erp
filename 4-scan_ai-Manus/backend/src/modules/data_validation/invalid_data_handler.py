...
class InvalidDataHandler:
    ...
    def handle_invalid_model_data(self,
                                  data: Dict[str,
                                             Any],
                                  model_name: str,
                                  auto_fix: Optional[bool] = None) -> Tuple[bool,
                                                                            Dict[str,
                                                                                 Any],
                                                                            List[Dict[str,
                                                                                      Any]]]:
        ...
        data_is_valid, errors = self.model_validator.validate_data(
            data, model_name)
        ...
        data_is_valid, remaining_errors = self.model_validator.validate_data(
            data_fixed, model_name)
    ...
    def handle_invalid_database_data(
            self, table_name: str, auto_fix: Optional[bool] = None) -> Tuple[bool, List[Dict[str, Any]]]:
        """
        ...
        schema_valid, schema_errors = self.database_validator.validate_table_schema(
            table_name)
        ...
        if not schema_valid:
            ...
            if self.config.get("notify_admin", True):
                self._notify_errors(
                    "database_schema", table_name, schema_errors)

            # لا يمكن إصلاح مخطط الجدول تلقائيًا
            return False, [
                {"table": table_name, "error": "مخطط الجدول غير صحيح", "details": schema_errors}]

        # التحقق من تكامل البيانات وإصلاحها إذا لزم الأمر
        if auto_fix:
            data_is_valid, action_list = self.database_validator.repair_data_integrity(
                table_name, auto_fix=True)
        else:
            data_is_valid, errors = self.database_validator.validate_data_integrity(
                table_name)
            action_list = [{"table": table_name,
                            "error": "تم اكتشاف أخطاء في تكامل البيانات",
                            "details": errors}]
    ...
    def handle_batch_model_data(self,
        ...
        for index, data in enumerate(data_list):
            data_is_valid, data_fixed, action_list = self.handle_invalid_model_data(
                data, model_name, auto_fix)
    ...
    def handle_all_database_tables(
            self, auto_fix: Optional[bool] = None) -> Dict[str, Tuple[bool, List[Dict[str, Any]]]]:
        """
        ...
        for table_name in self.database_validator.constraints:
            data_is_valid, action_list = self.handle_invalid_database_data(
                table_name, auto_fix)
    ...
    def _handle_missing_field(self,
                              data: Dict[str,
                                         Any],
                              field: str,
                              model_name: str,
                              auto_fix: bool) -> Optional[Tuple[bool,
                                                                Dict[str,
                                                                     Any]]]:
        """
        معالجة حقل مفقود.
        ...
        try:
            if field_type == "string":
                ...
                if isinstance(current_value, str):
                    converted_value = current_value.lower() in [
                        "true", "yes", "1", "y"]
                else:
                ...
                if isinstance(current_value, str):
                    ...
                    converted_value = list(current_value) if hasattr(
                        current_value, "__iter__") else [current_value]
    ...
    def _handle_invalid_value(self,
                              data: Dict[str,
                                         Any],
                              field: str,
                              model_name: str,
        ...
        if field_type == "string":
            ...
            if isinstance(current_value, str):
                if len(current_value) < min_length:
                    corrected_value = current_value + "x" * \
                        (min_length - len(current_value))
            ...
            if isinstance(current_value, list):
                if len(current_value) < min_items:
                    corrected_value = current_value + \
                        [None] * (min_items - len(current_value))
    ...
    def _handle_unknown_field(self,
                              data: Dict[str,
                                         Any],
                              field: str,
                              model_name: str,
    ...
    def _handle_schema_mismatch(self,
                                data: Dict[str,
                                           Any],
                                field: str,
                                table_name: str,
    ...
    def _handle_null_value(self,
        ...
        Returns:
            Optional[Tuple[bool, Dict[str, Any]]]: زوج من القيمة المنطقية (صحيح/خطأ) ومعلومات الإجراء المتخذ.
        """
        # تتم معالجة القيم الفارغة في قاعدة البيانات بواسطة
        # repair_data_integrity
        return None

    def _handle_foreign_key_violation(self,
                                      data: Dict[str,
                                                 Any],
                                      field: str,
                                      table_name: str,
        ...
        Returns:
            Optional[Tuple[bool, Dict[str, Any]]]: زوج من القيمة المنطقية (صحيح/خطأ) ومعلومات الإجراء المتخذ.
        """
        # تتم معالجة انتهاكات المفتاح الأجنبي في قاعدة البيانات بواسطة
        # repair_data_integrity
        return None

    def _log_errors(self, error_type: str, context: str,
    ...
    def _notify_errors(self, error_type: str, context: str,
                       errors: List[Dict[str, Any]]) -> None:
        """
        إشعار بالأخطاء.
        ...
        for handler_name, notification_handler in self.notification_handlers.items():
            try:
                ...
                logger.error(
                    "خطأ في معالج الإشعارات '%s': %s",
                    handler_name,
                    str(e))

    def _notify_log(self, error_type: str, context: str,
                    errors: List[Dict[str, Any]]) -> None:
        """
    ...
    def _notify_admin(self, error_type: str, context: str,
                      errors: List[Dict[str, Any]]) -> None:
        """
        ...
        logger.info(
            "تم إرسال إشعار إلى المسؤول: %s - %s: %d خطأ",
            error_type,
            context,
            len(errors))

    def _notify_email(self, error_type: str, context: str,
                      errors: List[Dict[str, Any]]) -> None:
        """
        ...
        # هنا نقوم بتسجيل الإشعار فقط
        logger.info(
            "تم إرسال إشعار عبر البريد الإلكتروني: %s - %s: %d خطأ",
            error_type,
            context,
            len(errors))

    def _notify_telegram(self, error_type: str, context: str,
                         errors: List[Dict[str, Any]]) -> None:
        """
        ...
        # هنا نقوم بتسجيل الإشعار فقط
        logger.info(
            "تم إرسال إشعار عبر تيليجرام: %s - %s: %d خطأ",
            error_type,
            context,
            len(errors))
