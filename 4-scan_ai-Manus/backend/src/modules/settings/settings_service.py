...
class SettingsService:
    ...
    def __init__(self, config_dir=None):
        ...
        self.general_settings_file = os.path.join(
            self.config_dir, 'general_settings.json')

        # مسار ملف إعدادات الشركة
        self.company_settings_file = os.path.join(
            self.config_dir, 'company_settings.json')

        # مسار ملف إعدادات النظام
        self.system_settings_file = os.path.join(
            self.config_dir, 'system_settings.json')

        # مسار ملف إعدادات الاتصال
        self.connection_settings_file = os.path.join(
            self.config_dir, 'connection_settings.json')

        # تحميل الإعدادات
        self.general_settings = self._load_settings(
            self.general_settings_file, self._get_default_general_settings())
        self.company_settings = self._load_settings(
            self.company_settings_file, self._get_default_company_settings())
        self.system_settings = self._load_settings(
            self.system_settings_file, self._get_default_system_settings())
        self.connection_settings = self._load_settings(
            self.connection_settings_file,
            self._get_default_connection_settings())
    ...
    def export_settings(self, settings_type, file_path=None):
        ...
        try:
            ...
            logger.exception(
                "Error exporting settings to %s: %s",
                file_path,
                str(e))
            return None
    ...
    def import_settings(self, settings_type, file_path):
        ...
        try:
            ...
            logger.exception(
                "Error importing settings from %s: %s",
                file_path,
                str(e))
            return False
