...
class TestMultiAgentRouter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.router.register_connector(  # pylint: disable=no-member
            'openai',
            lambda agent_id, config: MockConnector(
                agent_id, config)  # pylint: disable=no-member
        )
        self.router.register_connector(  # pylint: disable=no-member
            'azure',
            lambda agent_id, config: MockConnector(
    ...
    def test_route_request_with_specific_agent(self):
        ...
        response = self.router.process_request(
            request,
            agent_id='agent2')  # pylint: disable=no-member  # pylint: disable=no-member
    ...
    def test_disabled_agent(self):
        ...
        with self.assertRaises(Exception):
            self.router.process_request(
                request,
                agent_id='agent3')  # pylint: disable=no-member  # pylint: disable=no-member
...
if __name__ == '__main__':
    ...
    if result.wasSuccessful():
        ...
        logger.error("فشلت %s اختبارات", len(
