...
class UsageAnalyzer:
    ...
    def _calculate_interaction_metrics(
        ...
        agent_info = self.db_service.get_agent(
            agent_id) if agent_id is not None else None
        cost_per_token = agent_info.get(
            'cost_per_token', 0) if agent_info is not None else 0
    ...
    def _log_minimal_interaction(
            self, interaction_data: Dict[str, Any], error: Exception) -> int:
        """
    ...
    def log_interaction(self, interaction_data: Dict[str, Any]) -> int:
        ...
        try:
            # حساب مقاييس التفاعل
            interaction_data = self._calculate_interaction_metrics(
                interaction_data)
            ...
            if self.central_ai_service is not None and interaction_data.get(
                    'success', False):
                self._analyze_interaction_async(log_id, interaction_data)

            logger.info(
                "تم تسجيل التفاعل برقم %s للوكيل {interaction_data.get('agent_id')}",
                log_id)
            return log_id
    ...
    def generate_usage_report(self,
        ...
        try:
            ...
            logger.info(
                "تم توليد تقرير الاستخدام: %s تفاعل",
                total_interactions)
            return report
        except Exception as e:
    ...
    def generate_compliance_report(self,
        ...
        try:
            ...
            for _, row in df.iterrows():
                if row.get('analysis'):
                    try:
                        ...
                        logger.warning(
                            "خطأ في تحليل بيانات الامتثال للسجل %s: %s", row['id'], str(e))
            ...
            compliance_rate = (
                compliant_count / total_interactions) * 100 if total_interactions > 0 else 0
            ...
            logger.info(
                "تم توليد تقرير الامتثال: %s تفاعل محلل",
                len(compliance_df))
            return report
        except Exception as e:
    ...
    def analyze_user_behavior(
        ...
        try:
            ...
            logger.info(
                "تم توليد تحليل سلوك المستخدم %s: %s تفاعل",
                user_id,
                total_interactions)
            return report
        except Exception as e:
            logger.error(
                "خطأ أثناء تحليل سلوك المستخدم %s: %s",
                user_id,
                str(e))
            return {
