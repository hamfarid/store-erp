...
def create_agent(agent_data):
    ...
    try:
        ...
        if last_agent_id is not None and isinstance(
                last_agent_id, str) and last_agent_id.startswith('agent'):
            try:
...
def get_user_conversations(user_id):
    ...
    try:
        ...
        logger.error(
            "خطأ أثناء استرجاع محادثات المستخدم %s: {str(e)}",
            user_id)
        return []
    finally:
        session.close()
...
def get_conversation(conversation_id):
    ...
    try:
        ...
        logger.error(
            "خطأ أثناء استرجاع المحادثة %s: {str(e)}",
            conversation_id)
        return None
    finally:
        session.close()
