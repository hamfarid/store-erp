...
class AIManagementPermissionsIntegration:
    ...
    def _register_default_roles(self) -> None:
        ...
        default_roles = [{"name": "ai_admin",
                          ...
                          "permissions": ["ai_model:create",
                                          ...
                                          "ai_model:train",
                                          "ai_model:evaluate",
                                          "ai_model:view_metrics",
                                          "ai_model:manage_versions",
                                          "ai_model:manage_datasets",
                                          "ai_model:manage_providers",
                                          "ai_model:manage_api_keys"],
                          "is_system": True,
                          },
                         {"name": "ai_user",
                          "description": "مستخدم نماذج الذكاء الاصطناعي",
                          "permissions": ["ai_model:read",
                                          "ai_model:view_metrics"],
                          "is_system": True,
                          },
                         {"name": "ai_developer",
        ...
        for role_data in default_roles:
            try:
                ...
                for permission_name in role_data["permissions"]:
                    permission = self.permission_service.get_permission_by_name(
                        permission_name)
                    if permission:
                        ...
                        logger.warning(
                            f"الصلاحية {permission_name} غير موجودة")
            except Exception as e:
                logger.error(
                    f"خطأ في تسجيل الدور {role_data['name']}: {str(e)}")
    ...
    def get_user_models(self, user_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            # التحقق من صلاحية المستخدم العامة
            has_permission = self.permission_service.check_user_permission(
                user_id, "ai_model:read")
            if not has_permission:
                return []

            # الحصول على نماذج الذكاء الاصطناعي المتاحة للمستخدم
            user_permissions = ModelPermission.query.filter_by(
                user_id=user_id).all()
            ...
            model_roles = ModelRole.query.filter(
                ModelRole.role_id.in_(user_role_ids)).all()
            for model_role in model_roles:
            ...
            logger.error(
                "خطأ في الحصول على نماذج الذكاء الاصطناعي المتاحة للمستخدم: %s",
                str(e))
            return []
    ...
    def get_model_roles(self, model_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                "خطأ في الحصول على أدوار نموذج ذكاء اصطناعي: %s",
                str(e))
            return []
    ...
    def get_model_permissions(self, model_id: int) -> List[Dict[str, Any]]:
        ...
        """
        try:
            model_permissions = ModelPermission.query.filter_by(
            ...
            logger.error(
                "خطأ في الحصول على صلاحيات نموذج ذكاء اصطناعي: %s",
                str(e))
            return []
    ...
    def sync_model_permissions(self) -> bool:
        ...
        try:
            ...
            logger.error(
                "خطأ في مزامنة صلاحيات نماذج الذكاء الاصطناعي: %s",
                str(e))
            return False

    def create_audit_log(self, user_id: int, model_id: int,
