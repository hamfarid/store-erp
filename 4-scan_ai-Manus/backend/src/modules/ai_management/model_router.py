...
class AIModelRouter:
            ...
            for model_id, model_config in self.models_config.get(
                ...
                if model_config.get("enabled", False):
                    try:
                        ...
                        logger.error(
                            "خطأ أثناء تهيئة النموذج %s: %s", model_id, str(e))
    ...
    def _handle_request(self, request):
        ...
        try:
            ...
            # محاكاة معالجة الطلب
            logger.info(
                "معالجة الطلب %s باستخدام النموذج {model_id}",
                request_id)
    ...
    def submit_request(self, request_info, callback=None):
        ...
        try:
            ...
            logger.info(
                "تم تقديم الطلب %s للمعالجة باستخدام النموذج {model_id}",
                request_id)
