...
class SecurityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        try:
            ...
            # التحقق من CSRF
            if self.enable_csrf_protection and request.method in [
                    "POST", "PUT", "DELETE", "PATCH"]:
                csrf_token = request.headers.get("X-CSRF-Token")
                if not csrf_token or not self._validate_csrf_token(
                        request, csrf_token):
                    return self._create_error_response(
                        status_code=status.HTTP_403_FORBIDDEN,
            ...
            # التحقق من جسم الطلب
            if self.request_body_validation and request.method in [
                    "POST", "PUT", "PATCH"]:
                try:
                    body = await self._get_request_body(request)
                    if self.enable_sql_injection_protection and self._contains_sql_injection(
                            body):
                        return self._create_error_response(
    ...
    def _check_rate_limit(
        ...
        requests_list[:] = [
            req_time for req_time in requests_list if req_time > window_start]
    ...
    def _record_violation(self, ip: str, violation_type: str) -> None:
        ...
        violations_list[:] = [
            v_time for v_time in violations_list if v_time > hour_ago]
