...
class FeedbackService:
    ...
    def create_feedback(
            ...
            user_id: Optional[int] = None) -> models.Feedback:
    ...
    def get_feedback_by_id(
            self, feedback_id: int, user_id: Optional[int] = None) -> Optional[Dict[str, Any]]:
        """الحصول على تغذية راجعة بواسطة المعرف"""
        feedback = self.db.query(
            models.Feedback).filter(
    ...
    def get_all_feedback(
        ...
        if filter_params.status:
            query = query.filter(
                models.Feedback.status == filter_params.status)
        if filter_params.feedback_type:
            query = query.filter(
                models.Feedback.feedback_type == filter_params.feedback_type)
        if filter_params.priority:
            query = query.filter(
                models.Feedback.priority == filter_params.priority)
        if filter_params.module_name:
            query = query.filter(
                models.Feedback.module_name == filter_params.module_name)
        if filter_params.user_id:
            query = query.filter(
                models.Feedback.user_id == filter_params.user_id)
        if filter_params.start_date:
            query = query.filter(
                models.Feedback.created_at >= filter_params.start_date)
        if filter_params.end_date:
            query = query.filter(
                models.Feedback.created_at <= filter_params.end_date)
        ...
        if filter_params.sort_by == "vote_count":
            ...
            if filter_params.sort_desc:
                query = query.order_by(
                    desc(subquery.c.vote_count.nullsfirst()))
            else:
    ...
    def update_feedback(
        ...
        can_edit = has_permission(
            user_id, config.FeedbackPermissions.EDIT_FEEDBACK)
        can_manage = has_permission(
            user_id, config.FeedbackPermissions.MANAGE_FEEDBACK)

        if not (is_owner or can_edit or can_manage):
            raise HTTPException(
                status_code=403,
                detail="ليس لديك صلاحية لتعديل هذه التغذية الراجعة")
        ...
        if old_status != feedback.status and feedback.user_id and config.FEEDBACK_SETTINGS[
    ...
    def delete_feedback(self, feedback_id: int, user_id: int) -> bool:
        ...
        can_delete = has_permission(
            user_id, config.FeedbackPermissions.DELETE_FEEDBACK)

        if not (is_owner or can_delete):
            raise HTTPException(
                status_code=403,
                detail="ليس لديك صلاحية لحذف هذه التغذية الراجعة")
    ...
    def get_comments(self, feedback_id: int, skip: int = 0,
    ...
    def delete_comment(self, comment_id: int, user_id: int) -> bool:
        """حذف تعليق"""
        comment = self.db.query(
            models.FeedbackComment).filter(
            models.FeedbackComment.id == comment_id).first()
        if not comment:
            return False
        ...
        if not (is_owner or can_manage):
            raise HTTPException(status_code=403,
                                detail="ليس لديك صلاحية لحذف هذا التعليق")
    ...
    async def add_attachment(
        ...
        if not (is_owner or can_edit or can_manage):
            raise HTTPException(
                status_code=403,
                detail="ليس لديك صلاحية لإضافة مرفقات لهذه التغذية الراجعة")

        # التحقق من عدد المرفقات
        attachment_count = self.db.query(
            func.count(
                models.FeedbackAttachment.id)).filter(
            models.FeedbackAttachment.feedback_id == feedback_id).scalar()

        if attachment_count >= config.FEEDBACK_SETTINGS["max_attachments_per_feedback"]:
            raise HTTPException(status_code=400,
                                detail="تم تجاوز الحد الأقصى لعدد المرفقات")
    ...
    def delete_attachment(self, attachment_id: int, user_id: int) -> bool:
        ...
        if not (is_owner or can_edit or can_manage):
            raise HTTPException(
                status_code=403,
                detail="ليس لديك صلاحية لحذف مرفقات هذه التغذية الراجعة")
    ...
    def create_tag(self, tag_data: schemas.TagCreate,
                   user_id: int) -> models.FeedbackTag:
        """إنشاء وسم جديد"""
        # التحقق من الصلاحيات
        if not has_permission(user_id,
            ...
            raise HTTPException(
                status_code=403,
                detail="ليس لديك صلاحية لإنشاء وسوم")
        ...
        if existing_tag:
            raise HTTPException(
                status_code=400,
                detail="يوجد وسم بنفس الاسم بالفعل")
    ...
    def add_tag_to_feedback(
        ...
        if not has_permission(user_id,
            ...
            raise HTTPException(
                status_code=403,
                detail="ليس لديك صلاحية لإضافة وسوم")
        ...
        existing_association = self.db.query(
            models.FeedbackTagAssociation).filter(
            models.FeedbackTagAssociation.feedback_id == feedback_id,
            models.FeedbackTagAssociation.tag_id == tag_id).first()
    ...
    def remove_tag_from_feedback(
        ...
        if not has_permission(user_id,
            ...
            raise HTTPException(
                status_code=403,
                detail="ليس لديك صلاحية لإزالة وسوم")
    ...
    def get_feedback_statistics(self) -> Dict[str, Any]:
        ...
        module_stats = self.db.query(
            ...
            func.count(
                models.Feedback.id)).filter(
            models.Feedback.module_name.isnot(None)).group_by(
                models.Feedback.module_name).all()
    ...
    def cleanup_old_feedback(self) -> Dict[str, int]:
        ...
        now = datetime.now(timezone.utc)
        completed_cutoff = now - \
            timedelta(days=config.FEEDBACK_SETTINGS["days_to_keep_completed_feedback"])
        rejected_cutoff = now - \
            timedelta(days=config.FEEDBACK_SETTINGS["days_to_keep_rejected_feedback"])
    ...
    def _feedback_to_dict(self, feedback: models.Feedback,
        ...
        if include_details:
            ...
            tag_associations = self.db.query(models.FeedbackTagAssociation).filter(
                models.FeedbackTagAssociation.feedback_id == feedback.id).all()
    ...
    def _notify_user_status_change(
            self,
            feedback: models.Feedback,
            old_status: config.FeedbackStatus,
            admin_id: int):
    ...
    def _notify_user_new_comment(
        ...
        try:
            ...
            logger.error(
                "خطأ في إرسال إشعار للمستخدم عن تعليق جديد: %s",
                str(e))
