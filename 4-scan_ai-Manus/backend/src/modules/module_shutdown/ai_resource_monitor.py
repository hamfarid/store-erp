...
class AIResourceMonitor:
    ...
    def _load_config(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"تم تحميل تكوين مراقب موارد الذكاء الصناعي من {self.config_path}")
            return True
        except Exception as e:
            logger.error(
                f"فشل تحميل تكوين مراقب موارد الذكاء الصناعي: {str(e)}")
            return False
    ...
    def _save_config(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"تم حفظ تكوين مراقب موارد الذكاء الصناعي إلى {self.config_path}")
            return True
        except Exception as e:
    ...
    def update_ai_module_resources(
            self, ai_module_id: str, resources: Dict[str, Any]) -> bool:
        """
        ...
        with self.lock:
            ...
            # تحديث الحالة
            resource_data.status = self._calculate_resource_status(
                resource_data)

            logger.debug(f"تم تحديث موارد مديول الذكاء الصناعي {ai_module_id}")
            return True

    def update_ai_module_limits(
            self, ai_module_id: str, limits: Dict[str, Any]) -> bool:
        """
        ...
        with self.lock:
            ...
            logger.info(
                f"تم تحديث حدود موارد مديول الذكاء الصناعي {ai_module_id}")
            return True

    def get_ai_module_resources(
    ...
    def _collect_ai_module_resources(self, ai_module_id: str) -> bool:
        ...
        try:
            ...
            if resource_data.process_id is not None:
                process_resources = self._collect_process_resources(
                    resource_data.process_id)
            ...
            logger.error(
                f"فشل جمع موارد مديول الذكاء الصناعي {ai_module_id}: {str(e)}")
            return False
    ...
    def _add_alert(self, ai_module_id: str, status: str) -> None:
        ...
        logger.info(
            f"تم إضافة تنبيه لمديول الذكاء الصناعي {ai_module_id} بحالة {status}")
    ...
    def _send_alert_notification(self, alert: Dict[str, Any]) -> None:
        ...
        if email:
            try:
                ...
                logger.info(
                    f"إرسال إشعار التنبيه عبر البريد الإلكتروني إلى {email}")
            except Exception as e:
                logger.error(
                    f"فشل إرسال إشعار التنبيه عبر البريد الإلكتروني: {str(e)}")
    ...
    def unregister_alert_callback(
        ...
        with self.lock:
            ...
            if callback in self.alert_callbacks[status]:
                ...
                logger.info(
                    f"تم إلغاء تسجيل دالة استدعاء للتنبيهات بحالة {status}")
                return True

            logger.warning(
                f"دالة الاستدعاء غير مسجلة للتنبيهات بحالة {status}")
            return False
    ...
    def get_alert_history(self,
        ...
        if ai_module_id:
            filtered_alerts = [
                alert for alert in filtered_alerts if alert["ai_module_id"] == ai_module_id]
    ...
    def update_alert_thresholds(
            self, level: str, thresholds: Dict[str, Any]) -> bool:
        """
        تحديث عتبات التنبيه.
    ...
    def set_monitoring_interval(self, interval: int) -> bool:
        ...
        with self.lock:
            ...
            if result:
                logger.info(
                    f"تم تعيين الفاصل الزمني للمراقبة: {interval} ثانية")
    ...
    def enable_gpu_monitoring(self, enable: bool) -> bool:
        ...
        with self.lock:
            ...
            if result:
                logger.info(
                    f"تم {'تفعيل' if enable else 'تعطيل'} مراقبة وحدة معالجة الرسومات")
    ...
    def enable_token_monitoring(self, enable: bool) -> bool:
        ...
        with self.lock:
            ...
            if result:
                logger.info(
                    f"تم {'تفعيل' if enable else 'تعطيل'} مراقبة الرموز")
    ...
    def enable_api_calls_monitoring(self, enable: bool) -> bool:
        ...
        with self.lock:
            ...
            if result:
                logger.info(
                    f"تم {'تفعيل' if enable else 'تعطيل'} مراقبة استدعاءات واجهة برمجة التطبيقات")
    ...
    def update_token_usage(self, ai_module_id: str, tokens: int) -> bool:
        ...
        with self.lock:
            ...
            logger.debug(
                f"تم تحديث استهلاك الرموز لمديول الذكاء الصناعي {ai_module_id}: {tokens}")
            return True
    ...
    def update_api_calls(self, ai_module_id: str, calls: int) -> bool:
        ...
        with self.lock:
            ...
            logger.debug(
                f"تم تحديث استدعاءات واجهة برمجة التطبيقات لمديول الذكاء الصناعي {ai_module_id}: {calls}")
            return True
    ...
    def reset_token_usage(self, ai_module_id: str) -> bool:
        ...
        with self.lock:
            ...
            logger.info(
                f"تم إعادة تعيين استهلاك الرموز لمديول الذكاء الصناعي {ai_module_id}")
            return True
    ...
    def reset_api_calls(self, ai_module_id: str) -> bool:
        ...
        with self.lock:
            ...
            logger.info(
                f"تم إعادة تعيين استدعاءات واجهة برمجة التطبيقات لمديول الذكاء الصناعي {ai_module_id}")
            return True
    ...
    def get_ai_modules_by_status(self, status: str) -> List[str]:
        ...
        """
        return [
            ai_module_id for ai_module_id,
            resource_data in self.ai_resources.items() if resource_data.status == status]
    ...
    def set_ai_module_active(self, ai_module_id: str, active: bool) -> bool:
        ...
        with self.lock:
            ...
            logger.info(
                f"تم تعيين نشاط مديول الذكاء الصناعي {ai_module_id}: {active}")
            return True
