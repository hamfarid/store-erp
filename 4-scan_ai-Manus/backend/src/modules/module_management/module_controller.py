# File:
# /home/ubuntu/ai_web_organized/src/modules/module_management/module_controller.py
"""
from flask import g
...
def start_module(module_name):
    ...
    try:
        ...
        if process.poll() is None:  # العملية لا تزال قيد التشغيل
            ...
            processes["processes"][module_name]["started_at"] = datetime.now(
            ).isoformat()
            save_data(MODULES_PROCESSES_FILE, processes)

            logger.info(
                f"Module {module_name} started successfully with PID {process.pid}")
            return True
        else:
            # قراءة رسائل الخطأ
            stdout, stderr = process.communicate()
            logger.error(
                f"Failed to start module {module_name}: {stderr.decode('utf-8')}")
            return False
    except Exception as e:
        ...
        if module_name in processes["processes"]:
            ...
            if pid:
                try:
                    ...
                    processes["processes"][module_name]["stopped_at"] = datetime.now(
                    ).isoformat()
                    save_data(MODULES_PROCESSES_FILE, processes)
...
def get_module_cpu_usage(module_name):
    ...
    try:
        ...
        logger.error(
            f"Error getting CPU usage for module {module_name}: {str(e)}")
        return 0
...
def get_module_ram_usage(module_name):
    ...
    try:
        ...
        logger.error(
            f"Error getting RAM usage for module {module_name}: {str(e)}")
        return 0
...
def get_module_last_started(module_name):
    ...
    try:
        ...
        logger.error(
            f"Error getting last started time for module {module_name}: {str(e)}")
        return None
...
def get_module_last_stopped(module_name):
    ...
    try:
        ...
        logger.error(
            f"Error getting last stopped time for module {module_name}: {str(e)}")
        return None
