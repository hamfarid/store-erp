# File:
# /home/ubuntu/ai_web_organized/src/modules/disease_diagnosis/db_service.py
"""
from flask import g
...
def init_default_data(session):
    ...
    if os.path.exists(default_path):
        try:
            ...
            logger.error(
                "خطأ في تحميل البيانات الافتراضية من الملف: %s",
                str(e))
    ...
    conditions = {
        "high_humidity": Condition(
            description="رطوبة عالية"),
        "moderate_temp": Condition(
            description="درجات حرارة معتدلة (15-25 درجة مئوية)"),
        "dry_warm": Condition(
            description="طقس جاف ودافئ"),
        "warm_temp": Condition(
            description="درجات حرارة معتدلة إلى مرتفعة (20-30 درجة مئوية)"),
        "dew": Condition(
            description="رطوبة عالية أو ندى"),
        "seed_contamination": Condition(
            description="تلوث البذور بالجراثيم"),
        "cold_wet": Condition(
            description="ظروف باردة ورطبة أثناء الإنبات")}
        ...
        for crop_data in data.get('crops', []):
            ...
            for disease_data in crop_data.get('diseases', []):
                ...
                for prevention_text in disease_data.get('prevention', []):
                    ...
                    disease.preventions.append(
                        preventions_dict[prevention_text])
...
def create_crop(crop_data):
    ...
    try:
        ...
        existing_crop = session.query(Crop).filter(
            Crop.name == crop_data['name']).first()
        if existing_crop:
...
def delete_crop(crop_id):
    ...
    try:
        ...
        diseases_count = session.query(Disease).filter(
            Disease.crop_id == crop_id).count()
        if diseases_count > 0:
            logger.warning(
                "لا يمكن حذف المحصول {crop_id} لأنه مرتبط بـ %s مرض",
                diseases_count)
            return False
...
def create_disease(disease_data):
    ...
    try:
        ...
        if existing_disease:
            logger.warning(
                "المرض {disease_data['name']} موجود بالفعل للمحصول %s",
                crop_id)
            return None
        ...
        if 'symptoms' in disease_data:
            for symptom_text in disease_data['symptoms']:
                symptom = session.query(Symptom).filter(
                    Symptom.description == symptom_text).first()
                if not symptom:
                    symptom = Symptom(description=symptom_text)
                    session.add(symptom)
                    session.flush()
        ...
        if 'conditions' in disease_data:
            for condition_text in disease_data['conditions']:
                condition = session.query(Condition).filter(
                    Condition.description == condition_text).first()
                if not condition:
                    condition = Condition(description=condition_text)
                    session.add(condition)
                    session.flush()
        ...
        if 'treatments' in disease_data:
            for treatment_text in disease_data['treatments']:
                treatment = session.query(Treatment).filter(
                    Treatment.description == treatment_text).first()
                if not treatment:
                    treatment = Treatment(description=treatment_text)
                    session.add(treatment)
                    session.flush()
        ...
        if 'preventions' in disease_data:
            for prevention_text in disease_data['preventions']:
                prevention = session.query(Prevention).filter(
                    Prevention.description == prevention_text).first()
                if not prevention:
                    prevention = Prevention(description=prevention_text)
                    session.add(prevention)
                    session.flush()
...
def update_disease(disease_id, disease_data):
    ...
    try:
        ...
        if 'cropId' in disease_data:
            # التحقق من وجود المحصول
            crop = session.query(Crop).filter(
                Crop.id == disease_data['cropId']).first()
            if crop:
        ...
        if 'images' in disease_data:
            ...
            session.query(DiseaseImage).filter(
                DiseaseImage.disease_id == disease_id).delete()
...
def delete_disease(disease_id):
    ...
    try:
        ...
        diagnoses_count = session.query(Diagnosis).filter(
            Diagnosis.disease_id == disease_id).count()
        if diagnoses_count > 0:
            logger.warning(
                "لا يمكن حذف المرض {disease_id} لأنه مرتبط بـ %s تشخيص",
                diagnoses_count)
            return False
...
def get_diagnosis_by_id(diagnosis_id):
    ...
    try:
        diagnosis = session.query(Diagnosis).filter(
            Diagnosis.id == diagnosis_id).first()
...
def get_diagnoses_by_user(user_id):
    ...
    try:
        diagnoses = session.query(Diagnosis).filter(
            Diagnosis.user_id == user_id).order_by(
            desc(
                Diagnosis.created_at)).all()
        return [diagnosis.to_dict() for diagnosis in diagnoses]
    except SQLAlchemyError as e:
        logger.error(
            "خطأ أثناء استرجاع تشخيصات المستخدم {user_id}: %s",
            str(e))
        return []
    finally:
        session.close()
...
def get_all_diagnoses():
    ...
    session = Session()
    try:
        diagnoses = session.query(Diagnosis).order_by(
            desc(Diagnosis.created_at)).all()
        return [diagnosis.to_dict() for diagnosis in diagnoses]
    except SQLAlchemyError as e:
...
def update_diagnosis(diagnosis_id, diagnosis_data):
    ...
    try:
        ...
        if 'diseaseId' in diagnosis_data:
            # التحقق من وجود المرض
            disease = session.query(Disease).filter(
                Disease.id == diagnosis_data['diseaseId']).first()
            if disease:
...
def search_by_symptoms(symptoms_list, crop_id=None):
    ...
    try:
        ...
        for disease in diseases:
            # حساب درجة التطابق
            disease_symptoms = [symptom.description.lower()
            ...
            if match_count > 0:
                ...
                match_score = match_count / \
                    len(symptoms_list) if symptoms_list else 0
