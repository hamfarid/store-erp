# File:
# /home/ubuntu/ai_web_organized/src/modules/disease_diagnosis/diagnosis_engine_enhanced.py
"""
from flask import g
...
class DiagnosisEngineEnhanced:
    ...
    def diagnose(self, symptoms: List[str], crop_type: str,
        ...
        try:
            ...
            validation_result = self._validate_input_data(
                symptoms, crop_type, image_features, environmental_conditions)
            if not validation_result["valid"]:
                logger.error(
                    f"خطأ في البيانات المدخلة: {validation_result['error']}")
                return {
                    "success": False,
            ...
            probable_diseases = [
                ds for ds in disease_scores if ds["score"] >= self.confidence_threshold]
    ...
    def _validate_input_data(self,
                             symptoms: List[str],
                             crop_type: str,
                             image_features: Optional[Dict[str,
                                                           Any]],
                             environmental_conditions: Optional[Dict[str,
                                                                     Any]]) -> Dict[str,
                                                                                    Any]:
        """
        التحقق من صحة البيانات المدخلة
        ...
        if crop_type not in self.knowledge_base.crops:
            return {
                "valid": False,
                "error": f"المحصول '{crop_type}' غير موجود في قاعدة المعرفة"}
        ...
        if environmental_conditions:
            if not isinstance(environmental_conditions, dict):
                return {
                    "valid": False,
                    "error": "الظروف البيئية يجب أن تكون على شكل قاموس"}

            # التحقق من نطاقات القيم
            if "temperature" in environmental_conditions:
                temp = environmental_conditions["temperature"]
                if not isinstance(
                        temp, (int, float)) or temp < -50 or temp > 60:
                    return {
                        "valid": False,
                        "error": "درجة الحرارة خارج النطاق المقبول (-50 إلى 60 درجة مئوية)"}
    ...
    def _calculate_match_score(self,
        ...
        try:
            ...
            symptom_score, symptom_details = self._calculate_symptom_match(
                disease, symptoms)
            ...
            if image_features and "image_patterns" in disease:
                image_score, image_details = self._calculate_image_match(
            ...
            if environmental_conditions and "conditions" in disease:
                env_score, env_details = self._calculate_environmental_match(
                    disease["conditions"], environmental_conditions)
    ...
    def _calculate_symptom_match(
            self, disease: Dict[str, Any], symptoms: List[str]) -> Tuple[float, Dict[str, Any]]:
        """
        حساب درجة تطابق الأعراض
        ...
        if not disease_symptoms:
            return 0.0, {"matching_symptoms": [],
                         "missing_symptoms": [], "extra_symptoms": symptoms}
        ...
        matched_disease_symptoms = [match["matched_symptom"]
                                    for match in matching_symptoms]
        missing_symptoms = [
            s for s in disease_symptoms if s not in matched_disease_symptoms]
        extra_symptoms = [s for s in symptoms if not any(
            m["input_symptom"] == s for m in matching_symptoms)]
        ...
        weighted_matches = sum(
            symptom_weights.get(
                match["matched_symptom"],
                1.0) for match in matching_symptoms)
        total_weight = sum(self.symptom_weights.get(s, 1.0)
    ...
    def _calculate_text_similarity(self, text1: str, text2: str) -> float:
        ...
        # في التطبيق الحقيقي، يمكن استخدام خوارزميات أكثر تعقيدًا مثل
        # Levenshtein أو Jaccard أو نماذج لغوية
    ...
    def _calculate_image_match(self,
                               disease_patterns: Dict[str,
                                                      Any],
    ...
    def _calculate_environmental_match(self,
                                       disease_conditions: Dict[str,
                                                                Any],
                                       environmental_conditions: Dict[str,
                                                                      Any]) -> Tuple[float,
                                                                                     Dict[str,
                                                                                          Any]]:
        """
        ...
        try:
                ...
                if isinstance(
                    ...
                    if min_temp <= actual_temp <= max_temp:
                        ...
                        if actual_temp < min_temp:
                            temp_match = max(
                                0, 1 - (min_temp - actual_temp) / 10)
                        else:  # actual_temp > max_temp
                            temp_match = max(
                                0, 1 - (actual_temp - max_temp) / 10)
                ...
                if isinstance(
                    ...
                    if min_humidity <= actual_humidity <= max_humidity:
                        ...
                        if actual_humidity < min_humidity:
                            humidity_match = max(
                                0, 1 - (min_humidity - actual_humidity) / 20)
                        else:  # actual_humidity > max_humidity
                            humidity_match = max(
                                0, 1 - (actual_humidity - max_humidity) / 20)
            ...
            for factor in ["rainfall", "soil_ph", "light"]:
                if factor in disease_conditions and factor in environmental_conditions:
                    ...
                    if isinstance(
                            disease_value,
                            dict) and "min" in disease_value and "max" in disease_value:
                        ...
                        if min_value <= actual_value <= max_value:
                            ...
                            if range_width > 0:
                                if actual_value < min_value:
                                    factor_match = max(
                                        0, 1 - (min_value - actual_value) / range_width)
                                else:  # actual_value > max_value
                                    factor_match = max(
                                        0, 1 - (actual_value - max_value) / range_width)
    ...
    def _calculate_image_confidence(
            self, image_features: Dict[str, Any]) -> float:
        """
        ...
        """
        try:
            # في التطبيق الحقيقي، سيتم استخدام نموذج أكثر تعقيدًا لحساب مستوى
            # الثقة
            ...
            if "quality_score" in image_features:
                quality_score = min(
                    1.0, max(
                        0.0, image_features["quality_score"]))
            else:
            ...
            if "clarity_score" in image_features:
                clarity_score = min(
                    1.0, max(
                        0.0, image_features["clarity_score"]))
            else:
            ...
            if "coverage_score" in image_features:
                coverage_score = min(
                    1.0, max(
                        0.0, image_features["coverage_score"]))
            else:
    ...
    def _analyze_environmental_impact(self,
                                      environmental_conditions: Dict[str,
                                                                     Any],
                                      probable_diseases: List[Dict[str,
                                                                   Any]]) -> Dict[str,
                                                                                  Any]:
        """
        ...
        try:
            ...
            for disease_score in probable_diseases:
                ...
                impact_analysis["disease_specific_impacts"].append(
                    disease_impact)
    ...
    def _generate_recommendations(
            self, probable_diseases: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if "scientific_name" in top_disease:
                general_recommendations.append(
                    f"الاسم العلمي: {top_disease['scientific_name']}")

            if "description" in top_disease:
                general_recommendations.append(
                    f"وصف المرض: {top_disease['description']}")
            ...
            if len(probable_diseases) > 1:
                for i in range(1, min(3, len(probable_diseases))):
                    ...
                    other_diseases.append(
                        f"{other_disease['name']} (درجة التطابق: {other_score:.2f})")

                if other_diseases:
                    general_recommendations.append(
                        f"أمراض أخرى محتملة: {', '.join(other_diseases)}")
    ...
    def _load_symptom_weights(self) -> Dict[str, float]:
        ...
        try:
            weights_file = os.path.join(
                self.models_dir, "symptom_weights.json")
            ...
            if os.path.exists(weights_file):
                ...
                logger.info(
                    "لم يتم العثور على ملف أوزان الأعراض. سيتم استخدام الأوزان الافتراضية.")
                return {}
    ...
    def update_model_from_feedback(
            self, diagnosis_id: str, feedback: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            diagnosis_file = os.path.join(
                self.results_dir, f"{diagnosis_id}.json")
            ...
            if "correct_disease" not in feedback:
                logger.error(
                    "التغذية الراجعة غير صالحة: يجب تحديد المرض الصحيح")
                return False

            correct_disease_name = feedback["correct_disease"]

            # الحصول على المرض الصحيح من قاعدة المعرفة
            correct_disease = self.knowledge_base.get_disease_by_name(
                correct_disease_name)

            if not correct_disease:
                logger.error(
                    f"المرض غير موجود في قاعدة المعرفة: {correct_disease_name}")
                return False
            ...
            logger.info(
                f"تم تحديث النموذج بناءً على التغذية الراجعة للتشخيص: {diagnosis_id}")
    ...
    def get_diagnosis_by_id(
            self, diagnosis_id: str) -> Optional[Dict[str, Any]]:
        """
