...
from database_models import Crop, Diagnosis, Disease, create_database, get_session
from file_handler import process_image_for_diagnosis, save_uploaded_image
...
async def get_crops():
    ...
    try:
        crops = session.query(Crop).filter(Crop.is_active).all()
...
async def get_diseases(crop_id: Optional[int] = None):
    ...
    session = get_session(engine)
    try:
        query = session.query(Disease).filter(Disease.is_active)
...
async def diagnose_plant(
    ...
    try:
        ...
        file_result = save_uploaded_image(
            image_content, image.filename, user_id)
        ...
        process_image_for_diagnosis(image_path)
...
async def get_diagnosis_statistics():
    ...
    try:
        ...
        crop_stats = session.query(Crop.name, session.query(Diagnosis).filter(
            Diagnosis.crop_id == Crop.id).count().label('count')).all()

        # التشخيصات حسب الحالة
        status_stats = session.query(Diagnosis.status, session.query(Diagnosis).filter(
            Diagnosis.status == Diagnosis.status).count().label('count')).distinct().all()
...
async def get_user_diagnoses(token: str, limit: int = 20, offset: int = 0):
    ...
    try:
        ...
        diagnoses = session.query(Diagnosis).filter(
            ...
            Diagnosis.created_at.desc()).offset(offset).limit(limit).all()

        total = session.query(Diagnosis).filter(
            Diagnosis.user_id == user_id).count()
