...
class ModelBenchmarkSystem:
    ...
    def run_comprehensive_benchmark(self,
                                    test_images_path: List[str],
                                    ground_truth_labels: List[int],
                                    models_to_test: Optional[List[str]] = None,
                                    num_iterations: int = 3) -> Dict[str,
                                                                     Any]:
        """
    ...
    def calculate_metrics(self,
                          ...
                          ground_truth: List[int],
                          confidences: List[float]) -> Dict[str,
                                                            Any]:
        """
        حساب مقاييس الأداء
    ...
    def aggregate_results(
            self, model_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        if not model_results or all(
                "error" in result for result in model_results):
            return {"error": "فشل في جميع التكرارات"}

        # تصفية النتائج الصحيحة
        valid_results = [
            result for result in model_results if "error" not in result]
        ...
        for key in metrics_keys:
            values = [result["metrics"][key]
                      for result in valid_results if key in result["metrics"]]
            if values:
        ...
        for key in timing_keys:
            values = [result["timing"][key]
                      for result in valid_results if key in result["timing"]]
            if values:
        ...
        for key in resource_keys:
            values = [result["resource_usage"][key]
                      for result in valid_results if key in result["resource_usage"]]
            if values:
        ...
        accuracy_values = [result["metrics"]["accuracy"]
                           for result in valid_results]
        timing_values = [result["timing"]["avg_inference_time"]
                         for result in valid_results]

        aggregated["stability"] = {
            "accuracy_cv": float(
                np.std(accuracy_values) /
                np.mean(accuracy_values)) if accuracy_values else 0,
            "timing_cv": float(
                np.std(timing_values) /
                np.mean(timing_values)) if timing_values else 0,
            "consistency_score": 1.0 -
            float(
                np.std(accuracy_values)) if accuracy_values else 0}
    ...
    def print_recommendations(self) -> None:
        ...
        print(
            f"🏆 أفضل دقة: {best_accuracy_model[0]} ({best_accuracy_model[1]['metrics']['accuracy']['mean']*100:.2f}%)")
        print(
            f"🚀 الأسرع: {fastest_model[0]} ({fastest_model[1]['timing']['fps']['mean']:.2f} FPS)")
        print(
            f"💾 الأكثر كفاءة: {most_efficient_model[0]} ({most_efficient_model[1]['resource_usage']['memory_used']['mean']:.2f} GB)")
    ...
    def create_visualizations(self) -> None:
        ...
        for model_name, results in self.benchmark_results.items():
            if "error" not in results:
                ...
                memory_usage.append(
                    results["resource_usage"]["memory_used"]["mean"])
        ...
        for i, model in enumerate(models):
            axes[1, 1].annotate(model, (fps_values[i], accuracies[i]), xytext=(
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        valid_results = {
            k: v for k,
            v in self.benchmark_results.items() if "error" not in v}
    ...
    def analyze_model_learning(self, model_name: str) -> Dict[str, Any]:
        ...
        if metrics["accuracy"]["mean"] > 0.9:
            ...
            analysis["recommendations"].append(
                "تحسين الدقة عن طريق زيادة بيانات التدريب أو تعديل معلمات النموذج")
        ...
        if timing["fps"]["mean"] > 30:
            ...
            analysis["recommendations"].append(
                "تحسين السرعة عن طريق تقليل حجم النموذج أو استخدام تقنيات التسريع")
        ...
        if results["stability"]["consistency_score"] > 0.95:
            ...
            analysis["recommendations"].append(
                "تحسين الاستقرار عن طريق زيادة تنوع بيانات التدريب")
