...
class ModelComparisonAPI:
    ...
    def __init__(self, processor, benchmark_system, learning_system=None,
        ...
        self.upload_dir = Path(
            "/home/ubuntu/gaara_scan_ai_final_4.2/uploads/test_images")
        self.results_dir = Path(
            "/home/ubuntu/gaara_scan_ai_final_4.2/reports/benchmarks")
        self.attention_dir = Path(
            "/home/ubuntu/gaara_scan_ai_final_4.2/reports/attention")
        self.robustness_dir = Path(
            "/home/ubuntu/gaara_scan_ai_final_4.2/reports/robustness")
    ...
    def run_benchmark(self,
        ...
        try:
            ...
            if self.learning_system:
                learning_results = self.learning_system.analyze_benchmark_results(
                    results)
    ...
    def get_benchmark_details(self, benchmark_id: str) -> Dict[str, Any]:
        ...
        """
        report_path = self.results_dir / \
            f"benchmark_report_{benchmark_id}.json"
    ...
    def predict_with_model(self, image_path: str,
        ...
        """
        try:
            result = self.processor.predict_single_model(
    ...
    def learn_from_results(
        ...
        try:
            return self.learning_system.analyze_benchmark_results(
                benchmark_results)
        except Exception as e:
    ...
    def analyze_attention(
        ...
        try:
            ...
            attention_results = self.attention_analyzer.analyze_attention_patterns(
                model_name, valid_paths)

            # حفظ النتائج
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            report_path = self.attention_dir / \
                f"attention_report_{model_name}_{timestamp}.json"
            ...
            visual_explanations = {}
            for img_id, img_data in attention_results.get(
                    "visual_explanations", {}).items():
            ...
            return {
                ...
                "attention_statistics": attention_results.get(
                    "attention_statistics",
                    {}),
                "focus_regions": attention_results.get(
                    "focus_regions",
                    {}),
    ...
    def compare_attention_patterns(
        ...
        try:
            ...
            for model_name in models:
                self.attention_analyzer.analyze_attention_patterns(
                    model_name, valid_paths)
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            report_path = self.attention_dir / \
                f"attention_comparison_{timestamp}.json"

            comparison_data = {}
            for model_name in models:
                if model_name in self.attention_analyzer.attention_data:
                    stats = self.attention_analyzer.attention_data[model_name].get(
                        "attention_statistics", {})
        ...
        try:
            for file in self.attention_dir.glob("attention_report_*.json"):
                try:
                    ...
                    logger.error(
                        f"خطأ في قراءة ملف تقرير الانتباه {file}: {e}")
            ...
            for file in self.attention_dir.glob("attention_comparison_*.json"):
                try:
                    ...
                    logger.error(
                        f"خطأ في قراءة ملف مقارنة الانتباه {file}: {e}")
    ...
    def test_model_robustness(
        ...
        try:
            ...
            robustness_results = self.robustness_analyzer.test_comprehensive_robustness(
                model_name, valid_paths)

            # حفظ النتائج
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            report_path = self.robustness_dir / \
                f"robustness_report_{model_name}_{timestamp}.json"
    ...
    def compare_models_robustness(
        ...
        try:
            ...
            for model_name in models:
                self.robustness_analyzer.test_comprehensive_robustness(
                    model_name, valid_paths)

            # مقارنة متانة النماذج
            comparison_results = self.robustness_analyzer.compare_models_robustness(
                models)
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            report_path = self.robustness_dir / \
                f"robustness_comparison_{timestamp}.json"
            ...
            return {
                "timestamp": timestamp,
                "models_compared": models,
                "comparison_data": comparison_results.get(
                    "comparison_data",
                    {}),
                "best_models": comparison_results.get(
                    "best_models",
                    {}),
                "comparison_chart_path": comparison_chart_path if os.path.exists(comparison_chart_path) else None,
                "report_path": str(report_path)}
    ...
    def get_robustness_reports(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # تقارير المقارنة
            for file in self.robustness_dir.glob(
                    "robustness_comparison_*.json"):
                try:
                    ...
                    logger.error(
                        f"خطأ في قراءة ملف مقارنة المتانة {file}: {e}")
    ...
    def _get_model_input_shape(self, model, model_type: str) -> List[int]:
        ...
        try:
            if model_type == "huggingface":
                ...
                return model.input_shape[1:] if hasattr(
                    model, 'input_shape') else [224, 224, 3]
            else:
                return [224, 224, 3]  # افتراضي
        except BaseException:
            return [224, 224, 3]  # افتراضي

    def _get_model_output_shape(
            self, model, model_type: str) -> Union[List[int], str]:
        """
        ...
        try:
            if model_type == "huggingface":
                return [len(model.config.id2label)] if hasattr(
                    model.config, 'id2label') else "غير متاح"
            elif model_type in ["tensorflow", "keras"]:
                return model.output_shape[1:] if hasattr(
                    model, 'output_shape') else "غير متاح"
            else:
                return "غير متاح"
        except BaseException:
            return "غير متاح"
    ...
    def _get_model_description(self, model_name: str, model_type: str) -> str:
        ...
        return descriptions.get(model_name,
                                f"نموذج {model_type} لتحديد أمراض النباتات")
