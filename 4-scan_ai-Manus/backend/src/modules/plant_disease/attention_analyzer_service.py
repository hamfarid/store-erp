        ...
        logging.FileHandler(
            os.path.join(
                os.getenv(
                    'LOG_DIR',
                    '/app/logs'),
                'attention_analyzer_service.log'))])
...
REPORT_FOLDER = os.getenv(
    'REPORT_STORAGE_PATH',
    '/app/reports/attention_analysis')
...
def analyze_attention():
    ...
    try:
        ...
        result = attention_analyzer.analyze_attention_patterns(model_name, [
                                                               file_path])
        ...
        return jsonify(
            {'error': f'خطأ في تحليل أنماط الانتباه: {str(e)}'}), 500
...
def compare_attention():
    ...
    try:
        ...
        comparison_result = attention_analyzer.create_attention_comparison(
            models, file_paths)
        ...
        return jsonify(
            {'error': f'خطأ في مقارنة أنماط الانتباه: {str(e)}'}), 500
...
def get_reports():
    ...
    try:
        ...
        return jsonify(
            {'error': f'خطأ في الحصول على قائمة التقارير: {str(e)}'}), 500
