...
class AttentionAnalyzer:
    ...
    def analyze_attention_patterns(
            self, model_name: str, test_images: List[str]) -> Dict[str, Any]:
        """
        ...
        for i, image_path in enumerate(sample_images):
            try:
                ...
                if attention_map is not None:
                    ...
                    # إنشاء تفسير بصري
                    visual_explanation = self.create_visual_explanation(
                        image_path, attention_map)
    ...
    def generate_gradcam_pytorch(self,
    ...
    def generate_gradcam_tensorflow(self,
                                    model,
    ...
    def analyze_focus_regions(
            self, attention_map: np.ndarray) -> Dict[str, Any]:
        """
    ...
    def analyze_focus_distribution(
            self, attention_map: np.ndarray) -> Dict[str, float]:
        """
    ...
    def create_visual_explanation(
            self, image_path: str, attention_map: np.ndarray) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # إنشاء التصور المركب
            explanation_path = self.create_combined_visualization(
    ...
    def calculate_attention_statistics(
            self, focus_regions_data: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    def create_attention_comparison(
            self, models_to_compare: Optional[List[str]] = None) -> None:
        """
        ...
        for model_name in models_to_compare:
            if model_name in self.attention_data:
                stats = self.attention_data[model_name].get(
                    "attention_statistics", {})
    ...
    def plot_attention_comparison(
            self, comparison_data: Dict[str, Dict[str, Any]]) -> None:
        """
    ...
    def print_attention_comparison_table(
            self, comparison_data: Dict[str, Dict[str, Any]]) -> None:
        """
        ...
        if data:
            ...
            focus_values = [float(d.get("نسبة التركيز", "0").split(" ± ")[
                                  0]) for d in data if "نسبة التركيز" in d]
            if focus_values:
                max_focus_idx = np.argmax(focus_values)
                logger.info(
                    f"• الأكثر تركيزاً: {data[max_focus_idx]['النموذج']}")

            # النموذج الأكثر انتشاراً
            spread_values = [float(d.get("انتشار الانتباه", "0").split(" ± ")[
                                   0]) for d in data if "انتشار الانتباه" in d]
            if spread_values:
                max_spread_idx = np.argmax(spread_values)
                logger.info(
                    f"• الأكثر انتشاراً: {data[max_spread_idx]['النموذج']}")

            # النموذج الأكثر تنوعاً (إنتروبيا عالية)
            entropy_values = [float(d.get("إنتروبيا الانتباه", "0").split(" ± ")[
                                    0]) for d in data if "إنتروبيا الانتباه" in d]
            if entropy_values:
                max_entropy_idx = np.argmax(entropy_values)
                logger.info(
                    f"• الأكثر تنوعاً: {data[max_entropy_idx]['النموذج']}")
