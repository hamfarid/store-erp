...
class ModelLearningSystem:
    ...
    def __init__(self, history_dir=None):
        ...
        """
        self.history_dir = history_dir or Path(
            "/home/ubuntu/gaara_scan_ai_final_4.2/reports/learning")
    ...
    def load_learning_history(self):
        ...
        try:
            ...
            if history_file.exists():
                ...
                logger.info(
                    f"تم تحميل سجل التعلم السابق: {len(self.learning_history)} سجل")
            else:
    ...
    def analyze_benchmark_results(
        ...
        try:
            ...
            if not benchmark_results or not isinstance(
            ...
            for model_name, result in benchmark_results.items():
                    ...
                    models_data[model_name].update(
                        {
                            "fps": timing.get(
                                "fps", {}).get(
                                "mean", 0), "avg_inference_time": timing.get(
                                "avg_inference_time", {}).get(
                                "mean", 0), "total_time": timing.get(
                                "total_time", {}).get(
                                "mean", 0)})
                ...
                if "resource_usage" in result:
                    ...
                    models_data[model_name].update(
                        {
                            "memory_used": resources.get(
                                "memory_used", {}).get(
                                "mean", 0), "cpu_usage": resources.get(
                                "cpu_usage", {}).get(
                                "mean", 0), "gpu_memory_used": resources.get(
                                "gpu_memory_used", {}).get(
                                "mean", 0) if "gpu_memory_used" in resources else 0})
    ...
    def analyze_data(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        correlations["accuracy_vs_memory"] = float(
            df["accuracy"].corr(df["memory_used"]))

        # العلاقة بين السرعة واستهلاك الذاكرة
        correlations["fps_vs_memory"] = float(
            df["fps"].corr(df["memory_used"]))
    ...
    def discover_patterns(self, df: pd.DataFrame) -> List[str]:
        ...
        if acc_fps_corr > 0.7:
            patterns.append(
                "هناك علاقة إيجابية قوية بين الدقة والسرعة، مما يشير إلى أن النماذج الأكثر دقة هي أيضاً الأسرع")
        elif acc_fps_corr < -0.7:
            patterns.append(
                "هناك علاقة سلبية قوية بين الدقة والسرعة، مما يشير إلى وجود مقايضة بين الدقة والسرعة")

        # نمط العلاقة بين الدقة واستهلاك الذاكرة
        acc_mem_corr = df["accuracy"].corr(df["memory_used"])
        if acc_mem_corr > 0.7:
            patterns.append(
                "هناك علاقة إيجابية قوية بين الدقة واستهلاك الذاكرة، مما يشير إلى أن النماذج الأكثر دقة تستهلك ذاكرة أكبر")
        elif acc_mem_corr < -0.7:
            patterns.append(
                "هناك علاقة سلبية قوية بين الدقة واستهلاك الذاكرة، مما يشير إلى أن النماذج الأكثر دقة تستهلك ذاكرة أقل")
        ...
        for model_type in unique_types:
            type_indices = [
                idx for idx in df.index if idx.startswith(model_type)]
            if len(type_indices) > 1:
                ...
                if avg_accuracy > overall_avg_accuracy * 1.2:
                    patterns.append(
                        f"نماذج {model_type} تظهر أداءً أفضل من المتوسط من حيث الدقة بنسبة {((avg_accuracy / overall_avg_accuracy) - 1) * 100:.1f}%")

                if avg_fps > overall_avg_fps * 1.2:
                    patterns.append(
                        f"نماذج {model_type} تظهر أداءً أفضل من المتوسط من حيث السرعة بنسبة {((avg_fps / overall_avg_fps) - 1) * 100:.1f}%")
        ...
        if "consistency_score" in df.columns:
            ...
            if high_consistency:
                patterns.append(
                    f"النماذج التالية تظهر استقراراً عالياً في الأداء: {', '.join(high_consistency)}")

            low_consistency = df[df["consistency_score"] < 0.5].index.tolist()
            if low_consistency:
                patterns.append(
                    f"النماذج التالية تظهر تبايناً كبيراً في الأداء: {', '.join(low_consistency)}")
    ...
    def generate_recommendations(self,
        ...
        recommendations.append(
            f"للتطبيقات التي تتطلب دقة عالية، استخدم نموذج {best_accuracy_model} (الدقة: {analysis['best_models']['accuracy']['accuracy']*100:.1f}%)")
        recommendations.append(
            f"للتطبيقات التي تتطلب استجابة فورية، استخدم نموذج {best_fps_model} (السرعة: {analysis['best_models']['fps']['fps']:.1f} FPS)")
        recommendations.append(
            f"للأجهزة ذات الموارد المحدودة، استخدم نموذج {best_memory_model} (استهلاك الذاكرة: {analysis['best_models']['memory']['memory_used']:.2f} GB)")

        # توصيات بناءً على العلاقات
        acc_fps_corr = analysis["correlations"]["accuracy_vs_fps"]
        if acc_fps_corr < -0.5:
            recommendations.append(
                "هناك مقايضة واضحة بين الدقة والسرعة. حدد أولوياتك بناءً على متطلبات التطبيق")
        ...
        for model_type, models in model_types.items():
            if len(models) > 1:
                ...
                if model_type == "mobilenet":
                    recommendations.append(
                        f"نماذج MobileNet مناسبة للأجهزة المحمولة، ونوصي باستخدام {best_model}")
                elif model_type == "vit":
                    recommendations.append(
                        f"نماذج Vision Transformer توفر دقة عالية، ونوصي باستخدام {best_model}")
                elif model_type == "cropnet":
                    recommendations.append(
                        f"نماذج CropNet مخصصة للمحاصيل الزراعية، ونوصي باستخدام {best_model}")

        # توصيات للتحسين
        low_accuracy_models = df[df["accuracy"] < 0.7].index.tolist()
        if low_accuracy_models:
            recommendations.append(
                f"النماذج التالية تحتاج إلى تحسين الدقة: {', '.join(low_accuracy_models)}")

        slow_models = df[df["fps"] < 5].index.tolist()
        if slow_models:
            recommendations.append(
                f"النماذج التالية تحتاج إلى تحسين السرعة: {', '.join(slow_models)}")

        # توصيات للاستخدام المتوازن
        balanced_score = df["accuracy"] * 0.6 + (df["fps"] / df["fps"].max()) * 0.3 - (
            df["memory_used"] / df["memory_used"].max()) * 0.1
        balanced_model = balanced_score.idxmax()

        recommendations.append(
            f"للتوازن الأمثل بين الدقة والسرعة واستهلاك الموارد، نوصي باستخدام نموذج {balanced_model}")

        # توصيات للتعلم المستمر
        recommendations.append(
            "قم بتدريب النماذج على مجموعات بيانات أكبر وأكثر تنوعاً لتحسين الدقة والتعميم")
        recommendations.append(
            "استخدم تقنيات تقليل حجم النموذج مثل التقطير (distillation) وتقليم الأوزان (pruning) لتحسين السرعة واستهلاك الموارد")
