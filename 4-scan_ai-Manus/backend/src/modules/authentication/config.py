...
class JWTConfig(BaseModel):
    ...
    token_url: str = Field(
        "/api/v1/auth/token",
        description="مسار الحصول على الرمز")
...
class SessionConfig(BaseModel):
    ...
    session_expire_minutes: int = Field(
        60, description="مدة صلاحية الجلسة بالدقائق")
    ...
    enable_session_tracking: bool = Field(
        True, description="تمكين تتبع الجلسات")
...
class PasswordConfig(BaseModel):
    ...
    require_uppercase: bool = Field(
        True, description="طلب حرف كبير واحد على الأقل")
    require_lowercase: bool = Field(
        True, description="طلب حرف صغير واحد على الأقل")
    require_digit: bool = Field(True, description="طلب رقم واحد على الأقل")
    require_special_char: bool = Field(
        True, description="طلب حرف خاص واحد على الأقل")
    ...
    lockout_duration_minutes: int = Field(
        15, description="مدة قفل الحساب بالدقائق")
...
class OAuthProviderConfig(BaseModel):
    ...
    client_id_env_var: str = Field(...,
                                   description="متغير البيئة لمعرف العميل")
    client_secret_env_var: str = Field(...,
                                       description="متغير البيئة لسر العميل")
