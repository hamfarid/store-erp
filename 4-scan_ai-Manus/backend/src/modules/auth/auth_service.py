...
class AuthService:
    ...
    def verify_token(token):
        ...
        try:
            ...
            sessions_data = load_data(SESSIONS_FILE)
            session = next(
                (s for s in sessions_data['sessions'] if s['id'] == session_id and s['is_active']),
                None)
            ...
            if session:
                ...
                users_data = load_data(USERS_FILE)
                user = next(
                    (u for u in users_data['users'] if u['id'] == user_id and u['is_active']),
                    None)
    ...
    def get_user_permissions(user_id):
        ...
        user_roles = [ur['role_id']
                      for ur in roles_data['user_roles'] if ur['user_id'] == user_id]

        # الحصول على صلاحيات الأدوار
        user_permissions = []
        for role_id in user_roles:
            role_perms = [rp['permission_id']
                          for rp in permissions_data['role_permissions'] if rp['role_id'] == role_id]
        ...
        for perm_id in user_permissions:
            perm = next(
                (p for p in permissions_data['permissions'] if p['id'] == perm_id),
                None)
            if perm:
    ...
    def get_user_companies(user_id):
        ...
        all_companies = next((cp['company_id'] for cp in permissions_data['company_permissions']
                             if cp['user_id'] == user_id and cp['company_id'] == 'all'), None)

        if all_companies:
            return ['all']

        # الحصول على الشركات المحددة
        companies = [cp['company_id']
                     for cp in permissions_data['company_permissions'] if cp['user_id'] == user_id]
    ...
    def get_user_countries(user_id):
        ...
        all_countries = next((cp['country_id'] for cp in permissions_data['country_permissions']
                             if cp['user_id'] == user_id and cp['country_id'] == 'all'), None)

        if all_countries:
            return ['all']

        # الحصول على الدول المحددة
        countries = [cp['country_id']
                     for cp in permissions_data['country_permissions'] if cp['user_id'] == user_id]
...
def check_permission(user: Dict[str, Any], permission: str) -> bool:
    ...
    # Changed from 'user_id' to 'id' based on the user structure
    user_id = user.get('id')
    if not user_id:
        return False
