...
from src.utils.api_utils import error_response, paginate_response, validate_schema
...
def get_notifications():
    ...
    try:
        ...
        return paginate_response(
            ...
            page=page,
            per_page=per_page,
            # يجب تحسين هذا لاسترجاع العدد الإجمالي من قاعدة البيانات
...
def mark_as_read(notification_id: int):
    ...
    try:
        ...
        updated_notification = service.mark_notification_as_read(
            notification_id)
...
@notifications_blueprint.route('/<int:notification_id>/unread',
                               methods=['PUT'])
@jwt_required()
def mark_as_unread(notification_id: int):
    ...
    try:
        ...
        updated_notification = service.mark_notification_as_unread(
            notification_id)
        return jsonify(updated_notification.dict())
    except Exception as e:
        return error_response(str(e), 500)
...
@notifications_blueprint.route('/<int:notification_id>/archive',
                               methods=['PUT'])
@jwt_required()
...
@notifications_blueprint.route('/<int:notification_id>/unarchive',
                               methods=['PUT'])
@jwt_required()
        ...
        for notification_id in data.notification_ids:
            ...
            if not notification or notification.user_id != user_id:
                return error_response(
                    f"غير مصرح بالوصول إلى الإشعار {notification_id}", 403)
...
@notifications_blueprint.route('/templates/<int:template_id>',
                               methods=['DELETE'])
@jwt_required()
...
def get_webhook_subscriptions():
    ...
    try:
        ...
        service = NotificationService()
        subscriptions = service.get_webhook_subscriptions(
            skip=(page - 1) * per_page, limit=per_page)
...
@notifications_blueprint.route('/webhooks/<int:subscription_id>',
                               methods=['PUT'])
@jwt_required()
def update_webhook_subscription(subscription_id: int):
    ...
    try:
        data = validate_schema(WebhookSubscriptionCreateSchema, request.json)
        service = NotificationService()
        subscription = service.update_webhook_subscription(
