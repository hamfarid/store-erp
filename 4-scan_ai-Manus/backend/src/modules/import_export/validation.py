...
class DataValidator:
        ...
        try:
            ...
            if expected_format == 'csv':
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    file_info['row_count'] = sum(
                        1 for _ in reader) + 1  # +1 للعناوين
        ...
        for row_idx, row in enumerate(data):
            ...
            for field, field_schema in schema.items():
                ...
                if field_type == 'string':
                    if 'minLength' in field_schema and len(
                    ...
                    if 'maxLength' in field_schema and len(
        ...
        if errors:
            ...
            self.error_logger.log_application_error(
                error=ImportValidationError(
                    "أخطاء في التحقق من صحة البيانات المستوردة",
                    errors),
                module_name=MODULE_NAME,
    ...
    def generate_error_report(
            self, errors: List[Dict[str, Any]], report_format: str = 'excel') -> str:
        """
        ...
        if report_format == 'excel':
            output_path = os.path.join(
                output_dir, f'import_errors_{timestamp}.xlsx')
            df = pd.DataFrame(errors)
            df.to_excel(output_path, index=False)
        elif report_format == 'csv':
            output_path = os.path.join(
                output_dir, f'import_errors_{timestamp}.csv')
            df = pd.DataFrame(errors)
            df.to_csv(output_path, index=False)
        elif report_format == 'json':
            output_path = os.path.join(
                output_dir, f'import_errors_{timestamp}.json')
    ...
    def validate_export_request(self,
                                entity_type: str,
                                filters: Dict = None,
                                fields: List[str] = None) -> Tuple[bool,
                                                                   Optional[str]]:
        """
