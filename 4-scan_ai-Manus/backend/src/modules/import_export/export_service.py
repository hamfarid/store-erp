...
class ActivityLogService:
    ...
    def log_activity(
        ...
        try:
            # This is a simplified logging - in a real implementation you'd use
            # the proper schemas
            logger.info(
                "Activity logged: user_id=%s, action=%s, entity_type=%s, status=%s, message=%s",
                user_id,
                action,
                entity_type,
                status,
...
class ExportService:
        ...
        try:
            if export_format == 'excel':
                output_path = os.path.join(
                    self.export_dir, f"{file_name}.xlsx")
                ...
                output_path = os.path.join(
                    self.export_dir, f"{file_name}.json")
            ...
            raise HTTPException(
                status_code=500,
                detail=f"فشل التصدير: {str(e)}") from e
    ...
    def export_database(self,
        ...
        output_path = os.path.join(
            self.export_dir,
            f"database_export_{timestamp}.zip")
    ...
    def export_learning_data(self,
        ...
        output_dir = os.path.join(
            self.export_dir,
            f"learning_data_export_{timestamp}")
        os.makedirs(output_dir, exist_ok=True)

        # إنشاء ملف التصدير
        output_path = os.path.join(
            self.export_dir,
            f"learning_data_export_{timestamp}.zip")
    ...
    def export_images(self,
        ...
        output_dir = os.path.join(
            self.export_dir,
            f"images_export_{timestamp}")
        os.makedirs(output_dir, exist_ok=True)

        # إنشاء ملف التصدير
        output_path = os.path.join(
            self.export_dir,
            f"images_export_{timestamp}.zip")
    ...
    def _get_entity_data(self,
        ...
        try:
            ...
            return self._convert_results_to_dict(
                results, fields, include_related)

        except Exception as e:
            logger.error(
                "خطأ في الحصول على بيانات الكيان %s: %s",
                entity_type,
                str(e))
            return []
    ...
    def _get_entity_query(self, entity_type: str):
        ...
        if entity_type == 'users':
            try:
                from src.modules.user_management.models import (
    ...
    def _convert_results_to_dict(
            self, results, fields: List[str], include_related: bool) -> List[Dict[str, Any]]:
    ...
    def _convert_single_result(
            self, result, fields: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if include_schema:
                schema_path = os.path.join(
                    output_dir, f"{database_name}_schema.sql")
            ...
            if include_data:
                data_path = os.path.join(
                    output_dir, f"{database_name}_data.sql")
            ...
            raise HTTPException(
                status_code=500,
                detail=f"فشل تصدير قاعدة البيانات: {str(e)}") from e
    ...
    def _export_learning_data_task(self,
        ...
        try:
            ...
            with open(readme_path, 'w', encoding='utf-8') as f:
                ...
                f.write(f"تاريخ التصدير: {datetime.now().isoformat()}\n")
                f.write(
                    f"تضمين الأوزان: {'نعم' if include_weights else 'لا'}\n")
                f.write(
                    f"تضمين بيانات التدريب: {'نعم' if include_training_data else 'لا'}\n")
            ...
            raise HTTPException(
                status_code=500,
                detail=f"فشل تصدير بيانات التعلم: {str(e)}") from e
    ...
    def _export_images_task(self,
        ...
        try:
            ...
            raise HTTPException(status_code=500,
                                detail=f"فشل تصدير الصور: {str(e)}") from e
