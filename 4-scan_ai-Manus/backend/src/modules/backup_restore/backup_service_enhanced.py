...
class BackupServiceEnhanced:
        ...
        while True:
            try:
                ...
                try:
                    ...
                    logger.exception(
                        "Error processing task %s: %s", task_id, str(e))
    ...
    def create_backup(
        ...
        task_id = "backup_{}_{}".format(
            int(time.time.time()), os.urandom(4).hex())

        # إنشاء معلومات المهمة
        task_info = {
            'id': task_id,
            'type': 'backup',
            'name': name or "backup_{}".format(
    ...
    def _create_backup_internal(
        ...
        try:
            ...
            backup_name = name or "backup_{}".format(
                datetime.datetime.now().strftime('%Y%m%d_%H%M%S'))
            backup_description = description or "Automatic backup created on {}".format(
                datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            dirs_to_include = include_dirs or list(
                self.config['data_dirs'].values())
            patterns_to_exclude = exclude_patterns or self.config['exclude_patterns']
            should_backup_dbs = backup_dbs if backup_dbs is not None else self.config[
                'backup_db']
            db_list = db_names or list(self.config['db_configs'].keys())
            should_include_volumes = include_volumes if include_volumes is not None else self.config[
                'docker']['use_volumes']
            should_include_bind_mounts = include_bind_mounts if include_bind_mounts is not None else self.config[
                'docker']['use_bind_mounts']
            should_include_txt_md = include_txt_md if include_txt_md is not None else self.config[
                'include_txt_md']

            # إنشاء مجلد مؤقت للنسخة الاحتياطية
            backup_temp_dir = os.path.join(
                self.config['temp_dir'], backup_name)
                ...
                for dir_path in dirs_to_include:
                    if os.path.exists(dir_path):
                        dest_dir = os.path.join(
                            data_dir, os.path.basename(dir_path))
                        logger.info(
                            "Copying directory %s to %s", dir_path, dest_dir)
                        ...
                        logger.warning(
                            "Directory %s does not exist, skipping", dir_path)
                ...
                for db_name in db_list:
                    if db_name in self.config['db_configs']:
                        ...
                        db_backup_path = os.path.join(
                            db_dir, "{}.sql".format(db_name))
                        logger.info(
                            "Backing up database %s to %s",
                            db_name,
                            db_backup_path)
                        self._backup_database(db_backup_path, db_config)
                        db_backup_paths[db_name] = os.path.relpath(
                            db_backup_path, backup_temp_dir)
            ...
            if should_include_volumes:
                ...
                for volume_name in self.config['docker']['volumes']:
                    volume_backup_path = os.path.join(
                        volumes_dir, "{}.tar".format(volume_name))
                    logger.info(
                        "Backing up Docker volume %s to %s",
                        volume_name,
                        volume_backup_path)
                    success = self._backup_docker_volume(
                        volume_name, volume_backup_path)
                    if success:
                        volume_backup_paths[volume_name] = os.path.relpath(
                            volume_backup_path, backup_temp_dir)
            ...
            backup_file_path = os.path.join(
                self.config['backup_dir'], "{}.tar.{}".format(
                    backup_name, self.config['compression']))
            ...
            with tarfile.open(backup_file_path, compression_mode) as tar:
                tar.add(
                    backup_temp_dir,
                    arcname=os.path.basename(backup_temp_dir))
    ...
    def restore_backup(
        ...
        if not backup_name and not backup_file:
            raise ValueError(
                "Either backup_name or backup_file must be provided")
        ...
        if backup_name and not backup_file:
            ...
            if not backup_file:
                raise FileNotFoundError(
                    "Backup file for {} not found".format(backup_name))

        # إنشاء معرف فريد للمهمة
        task_id = "restore_{}_{}".format(
            int(time.time.time()), os.urandom(4).hex())
    ...
    def _restore_backup_internal(
        ...
        try:
            ...
            if not backup_file or not isinstance(
                    backup_file, str) or not os.path.exists(backup_file):
                raise FileNotFoundError(
                    "Backup file {} not found".format(backup_file))

            # إنشاء مجلد مؤقت لاستخراج النسخة الاحتياطية
            extract_dir = os.path.join(
                self.config['temp_dir'], "restore_{}".format(int(time.time())))
            os.makedirs(extract_dir, exist_ok=True)

            # استخراج النسخة الاحتياطية
            logger.info(
                "Extracting backup file %s to %s",
                backup_file,
                extract_dir)
            with tarfile.open(backup_file, 'r:*') as tar:
                # التحقق من الأعضاء الآمنة فقط
                def is_safe_path(path, base_path):
                    abs_path = os.path.abspath(os.path.join(base_path, path))
                    return abs_path.startswith(os.path.abspath(base_path))

                safe_members = [m for m in tar.getmembers() if is_safe_path(m.name, extract_dir)]
                tar.extractall(path=extract_dir, members=safe_members)

            # البحث عن مجلد النسخة الاحتياطية داخل المجلد المستخرج
            backup_dirs = [
                d for d in os.listdir(extract_dir) if os.path.isdir(
                    os.path.join(
                        extract_dir, d))]
            if not backup_dirs:
                raise ValueError(
                    "No backup directory found in the extracted files")
            ...
            if not os.path.exists(config_path):
                raise FileNotFoundError(
                    "Backup configuration file not found in {}".format(backup_dir))

            with open(config_path, 'r', encoding='utf-8') as f:
                backup_config = json.load(f)

            # استعادة المجلدات (Bind Mounts)
            if restore_bind_mounts and backup_config.get(
                    'include_bind_mounts', False):
                data_dir = os.path.join(backup_dir, 'data')
                if os.path.exists(data_dir):
                    # تحديد المجلدات المراد استعادتها
                    dirs_to_restore = dir_names if dir_names else [
                        os.path.basename(d) for d in backup_config.get('include_dirs', [])]
                    ...
                    for dir_name in dirs_to_restore:
                        ...
                        if os.path.exists(src_dir):
                            ...
                            if target_dir:
                                logger.info(
                                    "Restoring directory %s to %s", src_dir, target_dir)

                                # إنشاء نسخة احتياطية من المجلد الحالي إذا كان
                                # موجوداً
                                if os.path.exists(target_dir):
                                    backup_target_dir = "{}_backup_{}".format(
                                        target_dir, int(time.time()))
                                    logger.info(
                                        "Creating backup of existing directory %s to %s",
                                        target_dir,
                                        backup_target_dir)
                                ...
                                os.makedirs(
                                    os.path.dirname(target_dir), exist_ok=True)
            ...
            if restore_dbs and backup_config.get('backup_dbs', False):
                ...
                dbs_to_restore = db_names if db_names else list(
                    db_backup_paths.keys())

                for db_name in dbs_to_restore:
                    if db_name in db_backup_paths and db_name in self.config['db_configs']:
                        db_backup_path = os.path.join(
                            backup_dir, db_backup_paths[db_name])
                        if os.path.exists(db_backup_path):
                            logger.info(
                                "Restoring database %s from %s", db_name, db_backup_path)
                            self._restore_database(
                                db_backup_path, self.config['db_configs'][db_name])

            # استعادة Docker Volumes
            if restore_volumes and backup_config.get('include_volumes', False):
                volume_backup_paths = backup_config.get(
                    'volume_backup_paths', {})

                # تحديد الـ Volumes المراد استعادتها
                volumes_to_restore = volume_names if volume_names else list(
                    volume_backup_paths.keys())

                for volume_name in volumes_to_restore:
                    if volume_name in volume_backup_paths:
                        volume_backup_path = os.path.join(
                            backup_dir, volume_backup_paths[volume_name])
                        if os.path.exists(volume_backup_path):
                            logger.info(
                                "Restoring Docker volume %s from %s",
                                volume_name,
                                volume_backup_path)
                            self._restore_docker_volume(
                                volume_name, volume_backup_path)
            ...
            backup_file_name = os.path.basename(
                backup_file) if backup_file and isinstance(backup_file, str) else ""
            return {
                'success': True,
                'message': "Backup {} restored successfully".format(
                    backup_name or backup_file_name)}
        except Exception as e:
    ...
    def _export_data_internal(
        ...
        try:
            # إنشاء مجلد للتصدير
            export_dir = os.path.join(target_dir, "export_{}".format(
                datetime.datetime.now().strftime('%Y%m%d_%H%M%S')))
            ...
            if include_dbs:
                ...
                dbs_to_export = db_names if db_names else list(
                    self.config['db_configs'].keys())

                for db_name in dbs_to_export:
                    if db_name in self.config['db_configs']:
                        db_config = self.config['db_configs'][db_name]
                        db_export_path = os.path.join(
                            db_export_dir, "{}.sql".format(db_name))
                        logger.info(
                            "Exporting database %s to %s",
                            db_name,
                            db_export_path)
            ...
            if include_volumes:
                ...
                volumes_to_export = volume_names if volume_names else self.config[
                    'docker']['volumes']

                for volume_name in volumes_to_export:
                    volume_export_path = os.path.join(
                        volume_export_dir, "{}.tar".format(volume_name))
                    logger.info(
                        "Exporting Docker volume %s to %s",
                        volume_name,
                        volume_export_path)
                    success = self._backup_docker_volume(
                        volume_name, volume_export_path)
                    if success:
            ...
            if include_dirs:
                ...
                dirs_to_export = dir_names if dir_names else list(
                    self.config['data_dirs'].keys())

                for dir_name in dirs_to_export:
                    if dir_name in self.config['data_dirs']:
                        dir_path = self.config['data_dirs'][dir_name]
                        if os.path.exists(dir_path):
                            dir_export_path = os.path.join(
                                dir_export_dir, dir_name)
                            logger.info(
                                "Exporting directory %s to %s", dir_path, dir_export_path)
    ...
    def import_data(
        ...
        if not os.path.exists(source_dir):
            raise FileNotFoundError(
                "Source directory {} not found".format(source_dir))

        # إنشاء معرف فريد للمهمة
        task_id = "import_{}_{}".format(
            int(time.time.time()), os.urandom(4).hex())
            ...
            if os.path.exists(config_path):
                ...
                if include_dbs:
                    ...
                    if os.path.exists(db_import_dir):
                        ...
                        available_dbs = [os.path.splitext(f)[0] for f in os.listdir(
                            db_import_dir) if f.endswith('.sql')]
                        ...
                        for db_name in dbs_to_import:
                            if db_name in self.config['db_configs'] and db_name in available_dbs:
                                db_import_path = os.path.join(
                                    db_import_dir, "{}.sql".format(db_name))
                                if os.path.exists(db_import_path):
                                    logger.info(
                                        "Importing database %s from %s", db_name, db_import_path)
                                    self._restore_database(
                                        db_import_path, self.config['db_configs'][db_name])
                ...
                if include_dirs:
                    ...
                    if os.path.exists(dir_import_dir):
                        ...
                        available_dirs = [
                            d for d in os.listdir(dir_import_dir) if os.path.isdir(
                                os.path.join(
                                    dir_import_dir, d))]
                        ...
                        for dir_name in dirs_to_import:
                            if dir_name in self.config['data_dirs'] and dir_name in available_dirs:
                                dir_import_path = os.path.join(
                                    dir_import_dir, dir_name)
                                target_dir = self.config['data_dirs'][dir_name]

                                if os.path.exists(dir_import_path):
                                    logger.info(
                                        "Importing directory %s from %s to %s",
                                        dir_name,
                                        dir_import_path,
                                        target_dir)

                                    # إنشاء نسخة احتياطية من المجلد الحالي إذا
                                    # كان موجوداً
                                    if os.path.exists(target_dir):
                                        backup_target_dir = "{}_backup_{}".format(
                                            target_dir, int(time.time()))
                                    ...
                                    shutil.copytree(
                                        dir_import_path, target_dir)
                                    imported_items['directories'].append(
                                        dir_name)
                ...
                raise FileNotFoundError(
                    f"Export configuration file not found in {source_dir}")
        except Exception as e:
    ...
    def _copy_directory(
        ...
        for item in os.listdir(src):
            ...
            should_exclude = any(Path(s).match(pattern)
    ...
    def _backup_docker_volume(self, volume_name, output_file):
        ...
        try:
            ...
            cmd = [
                ...
                '-v',
                f"{os.path.dirname(os.path.abspath(output_file))}:/backup",
                'alpine',
                'tar',
                'cf',
                f"/backup/{os.path.basename(output_file)}",
                '-C',
                '/source',
                '.']
            logger.info("Running command: %s", ' '.join(cmd))
        ...
        if len(backups) > self.config['max_backups']:
            ...
            for backup in backups[:len(backups) - self.config['max_backups']]:
                ...
                try:
                    ...
                    logger.warning(
                        "Error removing old backup %s: %s", file_path, str(e))
    ...
    def _restore_database(self, input_file, db_config):
        ...
        if not os.path.exists(input_file):
            raise FileNotFoundError(
                f"Database backup file {input_file} not found")
        env = os.environ.copy()
        if db_config.get('password'):
            env['PGPASSWORD'] = db_config['password']
        cmd = [
    ...
    def _restore_docker_volume(self, volume_name, input_file):
        ...
        try:
            if not os.path.exists(input_file):
                raise FileNotFoundError(
                    f"Docker volume backup file {input_file} not found")
            ...
            cmd = [
                ...
                '-v',
                f"{os.path.dirname(os.path.abspath(input_file))}:/backup",
                'alpine',
                'sh',
                '-c',
                f"rm -rf /target/* && tar xf /backup/{os.path.basename(input_file)} -C /target"]
            logger.info("Running command: %s", ' '.join(cmd))
            process = subprocess.run(
                cmd,
                stdout=subprocess.PIPE,
    ...
    def get_backups(self):
        ...
        for file_name in os.listdir(self.config['backup_dir']):
            ...
            if os.path.isfile(file_path) and any(
                file_name.endswith(f".tar.{ext}") for ext in [
                    'gz', 'bz2', 'xz']):
                try:
                        ...
                        with tarfile.open(file_path, 'r:*') as tar:
                            ...
                            if config_file:
                                ...
                                if extracted is not None:
                                    ...
                                    logger.warning(
                                        "Could not extract config file from %s: extractfile returned None", file_path)
                    except Exception as e:
                        logger.warning(
                            "Error extracting config from %s: %s", file_path, str(e))
                    backups.append(backup_info)
                except Exception as e:
                    logger.warning(
                        "Error processing backup file %s: %s", file_path, str(e))
