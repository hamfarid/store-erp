# File:
# /home/ubuntu/ai_web_organized/src/modules/backup_restore/backup_service.py
"""
from flask import g
...
class BackupService:
    ...
    def _create_backup_internal(
        ...
        try:
            ...
            for dir_path in dirs_to_include:
                if os.path.exists(dir_path):
                    dest_dir = os.path.join(
                        backup_temp_dir, os.path.basename(dir_path))
                    logger.info(f"Copying directory {dir_path} to {dest_dir}")
                    self._copy_directory(
                        dir_path, dest_dir, patterns_to_exclude)
                else:
                    logger.warning(
            ...
            backup_file_path = os.path.join(
                ...
                f"{backup_name}.tar.{self.config['compression']}")
        ...
        if backup_name and not backup_file:
            for ext in ['gz', 'bz2', 'xz']:
                potential_file = os.path.join(
                    self.config['backup_dir'], f"{backup_name}.tar.{ext}")
            ...
            if not backup_file:
                raise FileNotFoundError(
                    f"Backup file for {backup_name} not found")
    ...
    def _restore_backup_internal(self, backup_name=None, backup_file=None):
        ...
        try:
            ...
            logger.info(
                f"Extracting backup file {backup_file} to {extract_dir}")
            ...
            if not os.path.exists(config_path):
                raise FileNotFoundError(
                    f"Backup configuration file not found in {backup_dir}")
                ...
                if os.path.isdir(dir_path) and dir_name != 'database':
                    ...
                    for include_dir in backup_config['include_dirs']:
                        if os.path.basename(include_dir) == dir_name:
                            ...
                            logger.info(
                                f"Restoring directory {dir_path} to {target_dir}")
                            ...
                            if os.path.exists(target_dir):
                                ...
                                logger.info(
                                    f"Creating backup of existing directory {target_dir} to {backup_target_dir}")
            ...
            if backup_config.get('backup_db') and backup_config.get(
                    'db_backup_path'):
            ...
            logger.info(
                f"Backup {backup_name or os.path.basename(backup_file)} restored successfully")
            return {
                'success': True,
            ...
            if os.path.isfile(file_path) and any(
                ...
                try:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Error extracting config from {file_path}: {str(e)}")

                    backups.append(backup_info)
                except Exception as e:
                    logger.warning(
                        f"Error processing backup file {file_path}: {str(e)}")
        ...
        if len(backups) > self.config['max_backups']:
            ...
            for backup in backups[:len(backups) - self.config['max_backups']]:
                ...
                try:
                    ...
                    logger.warning(
                        f"Error removing old backup {file_path}: {str(e)}")
