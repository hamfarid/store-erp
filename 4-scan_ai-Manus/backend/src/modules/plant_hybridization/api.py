...
async def add_variety(
    ...
    try:
        ...
        if result:
            return {
                "success": True,
                "message": f"تم إضافة الصنف {variety_id} بنجاح"}
...
async def add_trait(
    ...
    try:
        ...
        if result:
            return {
                "success": True,
                "message": f"تم إضافة الصفة {trait_id} بنجاح"}
...
async def add_objective(
    ...
    try:
        ...
        if result:
            return {
                "success": True,
                "message": f"تم إضافة الهدف {objective_id} بنجاح"}
...
async def get_task_status(
    ...
    try:
        ...
        if task_info["params"]["user_id"] != current_user["id"] and not check_permission(
                current_user, PERMISSION_ADMIN):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
...
async def get_user_tasks(
    ...
    try:
        ...
        for task_id, task_info in running_simulations.items():
            # التحقق من ملكية المهمة
            if task_info["params"]["user_id"] == current_user["id"] or check_permission(
                    current_user, PERMISSION_ADMIN):
                user_tasks.append({
                    "task_id": task_id,
                    "status": task_info["status"],
...
async def get_simulation_result(
    ...
    try:
        ...
        if result["params"]["user_id"] != current_user["id"] and not check_permission(
                current_user, PERMISSION_ADMIN):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
...
async def export_simulation_to_csv(
    ...
    try:
        ...
        # التحقق من ملكية المحاكاة
        if simulation["params"]["user_id"] != current_user["id"] and not check_permission(
                current_user, PERMISSION_ADMIN):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=ERROR_SIMULATION_ACCESS_DENIED
