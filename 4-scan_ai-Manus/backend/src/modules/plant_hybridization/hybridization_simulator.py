# File:
# /home/ubuntu/ai_web_organized/src/modules/plant_hybridization/hybridization_simulator.py
"""
...
class HybridizationSimulator:
    ...
    def _load_data(self):
        ...
        try:
            ...
            if os.path.exists(varieties_file):
                ...
                logger.info(
                    f"تم تحميل {len(self.varieties)} صنف من {varieties_file}")
            else:
            ...
            if os.path.exists(traits_file):
                ...
                logger.info(
                    f"تم تحميل {len(self.traits)} صفة من {traits_file}")
            else:
            ...
            if os.path.exists(objectives_file):
                ...
                logger.info(
                    f"تم تحميل {len(self.objectives)} هدف من {objectives_file}")
            else:
    ...
    def _create_default_varieties(self):
        ...
        logger.info(
            f"تم إنشاء بيانات أصناف افتراضية وحفظها في {varieties_file}")
    ...
    def _create_default_objectives(self):
        ...
        logger.info(
            f"تم إنشاء بيانات أهداف افتراضية وحفظها في {objectives_file}")
    ...
    def get_traits(self, trait_ids=None):
        ...
        """
        if trait_ids:
            return {trait_id: self.traits[trait_id]
                    for trait_id in trait_ids if trait_id in self.traits}
        else:
            return self.traits
    ...
    def get_objectives(self, objective_ids=None):
        ...
        """
        if objective_ids:
            return {objective_id: self.objectives[objective_id]
                    for objective_id in objective_ids if objective_id in self.objectives}
        else:
    ...
    def run_simulation(self, simulation_params):
        ...
        try:
            ...
            validation_result = self._validate_simulation_params(
                simulation_params)
            if not validation_result["valid"]:
                logger.error(
                    f"خطأ في معلمات المحاكاة: {validation_result['error']}")
                return {
                    "success": False,
            ...
            if objective_id:
                ...
                common_traits = set(
                    parent1["traits"].keys()) & set(
                    parent2["traits"].keys())
                for trait in common_traits:
                    if trait in self.traits and "optimization" in self.traits[trait]:
                        optimization = self.traits[trait]["optimization"]
                        if optimization == "maximize":
                            objective["target_traits"][trait] = {
                                "min": 0.7, "weight": 1.0 / len(common_traits)}
                        elif optimization == "minimize":
                            objective["target_traits"][trait] = {
                                "max": 0.3, "weight": 1.0 / len(common_traits)}
                        else:  # target
                            avg_value = (
                                parent1["traits"][trait] + parent2["traits"][trait]) / 2
                            objective["target_traits"][trait] = {
                                "target": avg_value, "weight": 1.0 / len(common_traits)}

            # إنشاء الجيل الأول (تهجين الأبوين)
            f1_generation = self._create_f1_generation(
                parent1, parent2, trait_inheritance, population_size)
            ...
            for _ in range(1, num_generations):
                ...
                selected_individuals = self._select_best_individuals(
                    generations[-1], evaluations[-1], selection_rate)

                # إنشاء الجيل التالي
                next_generation = self._create_next_generation(
                    selected_individuals, trait_inheritance, population_size, mutation_rate)

                # تقييم الجيل التالي
                next_evaluation = self._evaluate_generation(
                    next_generation, objective)
            ...
            # اختيار أفضل الهجن
            best_hybrids = self._select_best_hybrids(
                generations[-1], evaluations[-1], 10)
    ...
    def _validate_simulation_params(self, params):
        ...
        if crop_type not in self.varieties:
            return {
                "valid": False,
                "error": f"نوع المحصول غير موجود: {crop_type}"}
        ...
        if parent1_id not in self.varieties[crop_type]["varieties"]:
            return {
                "valid": False,
                "error": f"الصنف الأول غير موجود: {parent1_id}"}

        if parent2_id not in self.varieties[crop_type]["varieties"]:
            return {
                "valid": False,
                "error": f"الصنف الثاني غير موجود: {parent2_id}"}
        ...
        if "objective_id" in params and params["objective_id"]:
            ...
            if objective_id not in self.objectives:
                return {
                    "valid": False,
                    "error": f"الهدف غير موجود: {objective_id}"}

            # التحقق من توافق الهدف مع المحصول
            if self.objectives[objective_id]["crop"] != crop_type:
                return {
                    "valid": False,
                    "error": f"الهدف {objective_id} غير متوافق مع المحصول {crop_type}"}
        ...
        if "num_generations" in params:
            ...
            if not isinstance(
                    num_generations,
                    int) or num_generations < 1 or num_generations > 20:
                return {
                    "valid": False,
                    "error": "عدد الأجيال يجب أن يكون بين 1 و 20"}

        if "population_size" in params:
            population_size = params["population_size"]
            if not isinstance(
                    population_size,
                    int) or population_size < 10 or population_size > 1000:
                return {
                    "valid": False,
                    "error": "حجم المجتمع يجب أن يكون بين 10 و 1000"}

        if "selection_rate" in params:
            selection_rate = params["selection_rate"]
            if not isinstance(selection_rate, (int, float)
                              ) or selection_rate <= 0 or selection_rate >= 1:
                return {
                    "valid": False,
                    "error": "معدل الاختيار يجب أن يكون بين 0 و 1"}

        if "mutation_rate" in params:
            mutation_rate = params["mutation_rate"]
            if not isinstance(mutation_rate, (int, float)
                              ) or mutation_rate < 0 or mutation_rate > 1:
                return {
                    "valid": False,
                    "error": "معدل الطفرة يجب أن يكون بين 0 و 1"}
        ...
        if "custom_objective" in params and params["custom_objective"]:
            ...
            if "target_traits" not in custom_objective:
                return {
                    "valid": False,
                    "error": "الهدف المخصص يجب أن يحتوي على صفات مستهدفة"}

            if not custom_objective["target_traits"]:
                return {
                    "valid": False,
                    "error": "الهدف المخصص يجب أن يحتوي على صفة مستهدفة واحدة على الأقل"}

            # التحقق من وجود الصفات المستهدفة
            for trait_id in custom_objective["target_traits"]:
                if trait_id not in self.traits:
                    return {"valid": False,
                            "error": f"الصفة غير موجودة: {trait_id}"}

        return {"valid": True}

    def _create_f1_generation(
            self,
            parent1,
            parent2,
            trait_inheritance,
            ...
            for trait_id, inheritance_info in trait_inheritance.items():
                ...
                if trait_id in parent1["traits"] and trait_id in parent2["traits"]:
                    ...
                    if inheritance_type == "mendelian":
                        ...
                        individual["traits"][trait_id] = parent1_value if np.random.random(
                        ) < 0.5 else parent2_value
                        ...
                        # إضافة تباين عشوائي
                        variance = (1 - heritability) * \
                            abs(parent1_value - parent2_value)
                        individual["traits"][trait_id] = np.random.normal(
                            mean_value, variance)
                        # تقييد القيمة بين 0 و 1 (للصفات النسبية)
                        individual["traits"][trait_id] = max(
                            0, min(1, individual["traits"][trait_id]))
                    else:
                        # وراثة افتراضية (متوسط الأبوين)
                        individual["traits"][trait_id] = (
                            parent1_value + parent2_value) / 2

                    # توريث العلامات الوراثية
                    if trait_id in parent1.get(
                            "genetic_markers",
                            {}) and trait_id in parent2.get(
                            "genetic_markers",
                            {}):
                        ...
                        all_markers = list(
                            set(parent1_markers + parent2_markers))

                        # اختيار عدد عشوائي من العلامات
                        num_markers = min(
                            len(all_markers), max(
                                len(parent1_markers), len(parent2_markers)))
                        selected_markers = np.random.choice(
                            all_markers, size=num_markers, replace=False).tolist()
    ...
    def _evaluate_generation(self, generation, objective):
        ...
        for individual in generation:
            ...
            fitness, trait_scores = self._calculate_fitness(
    ...
    def _calculate_fitness(self, individual, objective):
        ...
        for trait_id, target_info in target_traits.items():
            ...
            if trait_id in individual["traits"]:
                ...
                if "min" in target_info:
                    ...
                    # حساب الفرق النسبي
                    relative_diff = abs(
                        trait_value - target_value) / target_value

                    if relative_diff <= tolerance:
                        trait_score = 1.0
                    else:
                        trait_score = max(
                            0, 1.0 - (relative_diff - tolerance) / (1.0 - tolerance))
                else:
        ...
        if weighted_scores:
            fitness = sum(weighted_scores) / sum(target_info.get("weight", 1.0)
                                                 for target_info in target_traits.values())
        else:
            fitness = 0.0
    ...
    def _select_best_individuals(
        ...
        sorted_indices = np.argsort(
            [evaluation["fitness"] for evaluation in evaluations])[::-1]

        # تحديد عدد الأفراد المختارين
        num_selected = max(2, int(len(generation) * selection_rate))

        # اختيار أفضل الأفراد
        selected_individuals = [generation[i]
    ...
    def _create_next_generation(
        ...
        for i in range(population_size):
            ...
            parent1 = selected_individuals[np.random.randint(
                0, len(selected_individuals))]
            parent2 = selected_individuals[np.random.randint(
                0, len(selected_individuals))]
    ...
    def get_simulation_result(self, simulation_id):
        ...
        try:
            ...
            logger.warning(
                f"لم يتم العثور على نتيجة المحاكاة: {simulation_id}")
            return None
            ...
            for i, hybrid in enumerate(best_hybrids[:3]):  # أفضل 3 هجن
                ...
                for trait_id, trait_value in hybrid["traits"].items():
                    if trait_id in self.traits:
                        ...
                        # تحويل القيمة النسبية إلى قيمة فعلية إذا كانت الوحدة
                        # محددة
                ...
                for trait_id, trait_value in hybrid["traits"].items():
                    if trait_id in parent1["traits"] and trait_id in parent2["traits"]:
                        ...
                        if trait_id in self.traits and "optimization" in self.traits[trait_id]:
                            ...
                            if optimization == "maximize" and trait_value > max(
                                    parent1_value, parent2_value):
                                improvement = (trait_value - max(parent1_value, parent2_value)) / \
                                    max(parent1_value, parent2_value) * 100
                                ...
                                improvement = (min(parent1_value, parent2_value) - trait_value) / \
                                    min(parent1_value, parent2_value) * 100
                                hybrid_recommendation["improvements"].append({
                                    "trait_id": trait_id,
                                    "trait_name": self.traits[trait_id]["name"],
                                    "improvement": f"-{improvement:.1f}%",
