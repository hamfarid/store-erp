...
TASK_LINE_RE = re.compile(
    r"^(?P<num>\d+)\.\s*\[(?P<status>.|\s)\]\s*\*\*(?P<title>.+?)\*\*\s*—\s*(?P<meta>.+)$")
SECTION_PRIORITY_RE = re.compile(
    r"^##\s*(?P<priority>P[0-3])\s*—\s*(?P<name>.+)$")
SUBSECTION_AREA_RE = re.compile(
    r"^###\s*(?P<area>.+?)\s*\((?P<priority>P[0-3])\)")
...
def parse_tasks(md_text: str) -> List[Dict]:
    ...
    while i < len(lines):
        ...
        if m:
            ...
            while j < len(lines):
                next_line = lines[j]
                if TASK_LINE_RE.match(next_line) or SECTION_PRIORITY_RE.match(
                        next_line) or SUBSECTION_AREA_RE.match(next_line):
                    break
...
def gh_api(token: str, method: str, url: str, **kwargs):
    ...
    resp = requests.request(
        method, url, headers=headers, timeout=(
            10, 30), **kwargs)
...
def find_existing_issue(
    ...
    r = gh_api(
        token,
        "GET",
        f"https://api.github.com/search/issues?q={url_quote(q_open)}")
    ...
    if include_closed:
        ...
        r2 = gh_api(
            token,
            "GET",
            f"https://api.github.com/search/issues?q={url_quote(q_closed)}")
...
def upsert_issue(
    ...
    if dry_run:
        print(
            f"DRY RUN: Would upsert issue for TL-{tl_id}: {title} | labels={labels}")
        return -1

    ensure_labels(token, owner, repo, labels)

    existing = find_existing_issue(
        token, owner, repo, tl_id, include_closed=True)
    if existing:
        ...
        if state == "closed" and not reopen_closed:
            ...
            print(
                f"SKIP: TL-{tl_id} has a closed issue #{issue_number}; use --reopen-closed to reopen.")
...
def main():
    ...
    ap.add_argument(
        "--output",
        help="Optional path to write a pure-JSON results file")
    ap.add_argument(
        "--priority",
        help="Comma-separated priorities to include e.g. P0,P1")
    ap.add_argument(
        "--ids",
        help="Comma-separated list of task IDs to include e.g. 1,2,10")
    ap.add_argument("--id-range", help="ID range to include e.g. 1-40")
    ap.add_argument(
        "--limit",
        type=int,
        help="Process only the first N tasks after filtering/sorting")
    args = ap.parse_args()

    token = os.environ.get("GITHUB_TOKEN") or os.environ.get("GH_TOKEN")
    if not token and not args.dry_run:
        print(
            "Error: GITHUB_TOKEN (or GH_TOKEN) environment variable is required (omit for --dry-run)",
            file=sys.stderr)
        sys.exit(2)
    ...
    if args.ids:
        try:
            id_allow = {int(x.strip())
                        for x in args.ids.split(",") if x.strip()}
        except ValueError:
            print(
                "Warning: --ids contains non-integer values; ignoring.",
                file=sys.stderr)
            id_allow = None
    if args.id_range:
        try:
            ...
            print(
                "Warning: --id-range must be like '1-40'; ignoring.",
                file=sys.stderr)
    ...
    if args.output:
        try:
            ...
            print(
                f"Warning: failed to write output file {args.output}: {e}",
                file=sys.stderr)
