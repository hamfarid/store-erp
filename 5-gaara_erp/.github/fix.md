FILE: .github/fix.md | PURPOSE: Prioritized Fix List (P0–P3) for gaara-store | OWNER: Maintainers | RELATED: GLOBAL_GUIDELINES v2.3, docs/Task_List.md | LAST-AUDITED: 2025-10-25
<!-- cSpell:disable -->

## Fix List (P0–P3) — gaara-store

P0 — Security & Auth (Immediate):
- [ ] Login-fix blitz: JWT Access 15m, Refresh 7d, rotation; revoke on logout; CSRF for forms; lockout; optional MFA
- [ ] Security headers: strict CSP with nonces, HSTS, X-Frame-Options=DENY, X-Content-Type-Options=nosniff, Referrer-Policy; CORS allowlist
- [ ] Secrets: move production secrets to Environment/KMS or Vault; remove from repo/.env; rotate now
- [ ] Unified error envelope for all APIs: `{success:false, code, message, details?, traceId}`

P1 — API & DB:
- [ ] OpenAPI contract and validators for all endpoints; drift tests
- [ ] DB constraints (FK/unique/check) and indexes on hot paths; Alembic migrations (expand→backfill→switch→contract)

P2 — Frontend & UX:
- [ ] React Router v6 protected routes; unified empty/loading/error states; WCAG AA; RTL polish
- [ ] Brand tokens (Gaara/MagSeeds), light/dark themes, micro-interactions

P3 — CI/CD & Resilience:
- [ ] SBOM + supply-chain scans; DAST baseline (OWASP ZAP)
- [ ] Circuit breakers: timeouts, retries, fallbacks; document in /docs/Resilience.md

Notes — corrections to older snippets in this file (supersede previous guidance):
- Use Argon2id (recommended) or bcrypt for password hashing (not MD5/PBKDF2 examples shown below)
- CSRF: use Flask-WTF CSRFProtect for forms; for JSON APIs prefer JWT + double-submit/CSRF token pattern; do not call `csrf.validate()` globally per-request
- SQL injection: use SQLAlchemy ORM/parameters; avoid string interpolation in raw SQL
...
@jwt_required()
def add_product():
    # للواجهات JSON: استخدم JWT، ويمكن (اختياريًا) double-submit token عبر رأس X-CSRF-Token يطابق الكوكي XSRF-TOKEN
