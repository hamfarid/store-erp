...
--[ ] NAME:Global Plan & Baseline Checks DESCRIPTION:Plan all-module remediation phases and run baseline diagnostics: pytest --collect-only (by groups), Django system checks, makemigrations --check --dry-run, and flake8 critical errors (E9,F63,F7,F82). Produce a consolidated findings report and per-module checklist seeds.
-[ ] NAME:Agricultural – Per-Module Checklists & Quick Fixes DESCRIPTION:Generate per-module checklists (imports/exports, required params/argument names, service signature adapters, models integrity, migrations, URLs/views); apply small compatibility fixes; validate with targeted pytest collection. Modules: research, agricultural_experiments, farms, nurseries, plant_diagnosis, production (analytics/dashboards), seed_hybridization (merged), seed_production, variety_trials.
-[ ] NAME:Business – Per-Module Checklists & Quick Fixes DESCRIPTION:Generate per-module checklists (imports/exports, required params/argument names, service signature adapters, models integrity, migrations, URLs/views); apply compatibility fixes; validate with targeted pytest collection. Modules: inventory, sales, purchasing, accounting, production (business), assets, hr, marketing, projects.
-[ ] NAME:Core – Per-Module Checklists & Quick Fixes DESCRIPTION:Generate per-module checklists (imports/exports, required params/argument names, service signature adapters, models integrity, migrations, URLs/views); apply compatibility fixes; validate with targeted pytest collection. Modules: core, permissions, permissions_manager, permissions_common, unified_permissions, users, users_accounts, organization, system_settings, setup (selected submodules).
-[ ] NAME:Investigate/Triage/Understand the problem DESCRIPTION:Confirm current RAG implementation status, permission gating, migrations for pgvector, and identify missing management command for selective embedding regeneration.
-[ ] NAME:Create RAG_PROD_README.md with pgvector rollout steps DESCRIPTION:Document exact production steps: dependency installs, Postgres config + CREATE EXTENSION vector, env vars (RAG_BACKEND=pgvector, RAG_PUBLIC_API=false), migrations, JSON embedding regen, pgvector backfill, and verification queries.
-[ ] NAME:Enable Postgres + pgvector in production (approve installs) DESCRIPTION:Await approval to install Python packages: pgvector and psycopg (or psycopg2-binary). After approval, update environment and verify adapter registration.
-[ ] NAME:Run migrations and backfill on production DESCRIPTION:After DB is Postgres: run manage.py migrate, rag_regen_json_embeddings --table all, then rag_backfill_vectors. Verify indexes exist and that search works with BACKEND=pgvector.
-[ ] NAME:Add optional HNSW index migration (Postgres only, no-op on others) DESCRIPTION:Create rag migration 0004 to attempt creating HNSW indexes for embedding_vec with safe try/except so it won’t fail on older pgvector. Provide reversible drop index in backwards.
-[ ] NAME:Add tests for PUBLIC_API=false permission gating DESCRIPTION:Add tests that set RAG_PUBLIC_API=false and assert RAG endpoints require auth (401/403).
-[ ] NAME:Run tests and fix any issues DESCRIPTION:Execute pytest; address failures quickly and keep suite green.
-[ ] NAME:Investigate/triage failing imports from pytest output and fix them incrementally DESCRIPTION:Fixed import errors by: adding City model; exporting CorrespondenceType/Status/Priority; ProjectTask alias; WorkflowStep alias; added WorkflowTransition manager mapping; aligned companies views/urls to tests; added audit fields and Branch.code with migrations.
-[ ] NAME:Add workflow fields to Project and Task to satisfy workflow integration tests DESCRIPTION:Added Project.workflow/workflow_step and Task.workflow/workflow_step with non-destructive migration (0003_add_workflow_fields).
-[/] NAME:Run targeted pytest for services_modules.projects workflow tests and fix next issues DESCRIPTION:Ran targeted pytest for workflow integration; no failures surfaced in current environment. Awaiting next failing traceback (if any) from your side or will proceed to broader suite targeting next modules (assets/beneficiaries) as you prefer.
-[/] NAME:Heavy pass: run full Django test suite and triage first failure DESCRIPTION:Run pytest for the entire repository with -x -vv to capture the first failure with detailed traceback. Record failing module, error type, and top frames. Acceptance: obtain one failing traceback to drive next fix.
-[/] NAME:Investigate/Triage: enable one-by-one app activation and re-activation plan; ensure failing test suites pass incrementally DESCRIPTION:Define the activation order, confirm MIGRATION_MINIMAL_MODE behavior for tests, and create/apply minimal, non-destructive fixes (migrations/exports/app_label) per app. Acceptance: each targeted test folder passes before moving to the next.
-[ ] NAME:Implement settings split proxy DESCRIPTION:Replace gaara_erp/gaara_erp/settings.py content with a thin delegator: `from .settings import *` so Django uses the new split settings (base/dev/test/prod). Acceptance: `APP_MODE=test pytest -q` uses gaara_erp/gaara_erp/settings/test.py.
-[ ] NAME:One-by-one activation: feasibility_studies re-activation DESCRIPTION:Run tests with minimal app set + feasibility_studies enabled via TEST_EXTRA_APPS, then re-run with full SERVICE_MODULES to confirm re-activation. Acceptance: feasibility_studies tests green in both modes.
-[/] NAME:One-by-one activation: next target app (agricultural_modules.research) DESCRIPTION:Select the next app per your preferred order; enable via TEST_EXTRA_APPS; run tests; apply minimal non-destructive fixes (exports/app_label/migrations) as needed; then re-activate in broader group.
-[ ] NAME:agricultural_modules.research: service signature adapter for tests DESCRIPTION:Provide a thin adapter/wrapper so research services accept project (object) or project_id, and user/user_id/added_by. Map aliases internally. Acceptance: pytest -q gaara_erp/agricultural_modules/research/tests --maxfail=1 passes; Pylint E1125/E1123 cleared in test_services.py.
-[ ] NAME:seed_hybridization.merged: fix __str__ return types DESCRIPTION:Ensure __str__ methods at lines ~79 and ~154 of merged/models.py return str. Add minimal unit tests if not present. Acceptance: Pylint E0307 cleared; related tests green.
-[ ] NAME:seed_hybridization.merged.services: ApiKey import shim DESCRIPTION:Fix unknown import symbol 'ApiKey' by importing correct model or providing a safe fallback shim (e.g., settings-based token for tests). Acceptance: Pylance reportAttributeAccessIssue cleared; services tests unaffected.
-[ ] NAME:seed_hybridization.tests: M2M assertion compatibility DESCRIPTION:Adjust service return type or accessor so tests can use assertIn with iterable (e.g., return QuerySet/list rather than method). Acceptance: test_services.py lines ~163-164 assertions pass without type errors.
-[ ] NAME:seed_hybridization.tests: optional member access safety DESCRIPTION:Guard against None before attribute access (e.g., name, recommendation_score) in services or factory/setup. Acceptance: Pylance reportOptionalMemberAccess cleared; tests pass.
-[ ] NAME:Investigate/Triage: research tests import error (nurseries not in INSTALLED_APPS) DESCRIPTION:Confirm settings/test.py composition and ensure agricultural_modules.nurseries is included or add via TEST_EXTRA_APPS.
-[/] NAME:Run agricultural_modules.research tests and fix next failure(s) DESCRIPTION:Execute pytest for research tests with required dependent apps enabled; address the first failing error (migrations/models/signatures) minimally and re-run until green.
-[ ] NAME:Static code analysis: generate per-module metadata and module dependency graph DESCRIPTION:Add scripts/analyze_repo.py; run it to emit data/*.json, data/all_modules.json, and data/module_graph.json.
-[ ] NAME:Investigate/Triage/Understand the problem DESCRIPTION:Identify current blockers for running agricultural_modules.research tests and confirm root causes (syntax errors, missing exports). Gather minimal evidence to proceed safely.
-[/] NAME:Fix production.services syntax and exports DESCRIPTION:Resolve indentation/syntax issues in agricultural_modules/production/services.py and add a module-level alias get_production_order_by_id. Verify that research tests can import without errors.
-[ ] NAME:Run research integration tests (nurseries, research) DESCRIPTION:With APP_MODE=test and TEST_EXTRA_APPS including research and nurseries, run pytest for research tests to surface the next failing assertion or error.
-[ ] NAME:Add visualization to analyzer (option A) DESCRIPTION:Extend scripts/analyze_repo.py to output data/module_graph.png when networkx/matplotlib are available; otherwise emit DOT and Mermaid files.
-[ ] NAME:Investigate/Triage/Understand the problem DESCRIPTION:Reproduce failing research tests and schema error (companies_company.code), examine settings/test configuration, and inspect analyzer JSON outputs.
-[ ] NAME:Fix DB schema: add companies_company.code via migration DESCRIPTION:Create Django migration core_modules.companies.0005 to add Company.code (nullable, unique, max_length=50) and apply to test DB.
-[ ] NAME:Run agricultural_modules.research tests DESCRIPTION:Run pytest on gaara_erp/agricultural_modules/research/tests with APP_MODE=test and TEST_EXTRA_APPS configured; capture first failure and iterate.
-[ ] NAME:Merge analyzer JSON into Markdown documentation DESCRIPTION:Process data/*.json, produce docs/analysis/MODULES_OVERVIEW.md, MODULE_GRAPH_MERMAID.md, and per-module inventories under docs/analysis/modules/.
-[ ] NAME:Generate class-level workflow diagrams DESCRIPTION:For each module, generate Mermaid class diagrams or flowcharts representing key classes and relationships; add to docs/analysis/modules/*.md.
-[ ] NAME:Plan module-by-module subtasks DESCRIPTION:Create subtasks per app to review documentation, fix issues, and validate tests for each app.
--[ ] NAME:Create module-by-module subtasks for systematic remediation DESCRIPTION:Based on the module overview and dependency analysis, create detailed subtasks for each high-priority module (core, companies, inventory, organization, accounting) to scan, fix issues, validate tests, and document findings.
---[ ] NAME:Scan and fix all model classes systematically DESCRIPTION:Based on the comprehensive module analysis, scan each of the 1,570 modules for missing app_label declarations, duplicate registrations, and other class-level issues.
-[/] NAME:Fix production.services missing model imports DESCRIPTION:Address missing BatchPackaging, BatchStorage, QualityTest, BatchPackagingMaterial imports in agricultural_modules.production.services based on IDE diagnostics.
-[ ] NAME:Investigate PowerShell output capture issue DESCRIPTION:Debug why pytest commands run successfully (exit code 0) but produce no visible output in PowerShell terminal sessions.
-[ ] NAME:Fix duplicate model registration warnings DESCRIPTION:Address RuntimeWarning about models being already registered (production.exportdestination, production.exportapproval, production.importapproval) by ensuring proper app_label declarations and avoiding duplicate imports.
-[/] NAME:Create missing database tables DESCRIPTION:Ensure core_systemsetting and companies_company tables exist with proper schema including the code column.
-[ ] NAME:Fix database access during app initialization DESCRIPTION:Address RuntimeWarning about accessing database during app initialization by moving database operations out of AppConfig.ready() methods.
