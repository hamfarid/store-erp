...
def create_app():
    ...
    try:
        from src.utils.error_handlers import register_error_handlers as ext_error_handlers
        ext_error_handlers(app)
        logger.info("✅ External error handlers registered successfully")
    except ImportError:
        logger.warning("⚠️ External error handlers not available, using built-in")

    # Enable CORS with explicit credentials support
    allowed_origins = [
        "http://localhost:3000",
        "http://127.0.0.1:3000",
        "http://localhost:5502",
    ...
    # Don't use Flask-CORS, handle it manually for full control
    @app.before_request
    ...
    # Register built-in error handlers (defined in this file)
...
def register_blueprints(app):
    ...
    def apply_default_headers(response):
        ...
        csp = (
            ...
            "font-src 'self' data:; "
            "connect-src 'self' http://localhost:5173 http://127.0.0.1:5173 http://localhost:3000 http://127.0.0.1:3000 http://localhost:5505 http://127.0.0.1:5505 http://localhost:5506 http://127.0.0.1:5506;"
        )
    ...
    blueprints_to_register = [
        # System & Status
        ('src.routes.temp_api', 'temp_api_bp'),
        ('src.routes.system_status', 'status_bp'),
        ('src.routes.dashboard', 'dashboard_bp'),

        # Unified routes (v2.0) - أولوية عالية
        ('src.routes.auth_unified', 'auth_unified_bp'),
        ('src.routes.users_unified', 'users_unified_bp'),
        ('src.routes.products_unified', 'products_unified_bp'),
        ('src.routes.partners_unified', 'partners_unified_bp'),
        ('src.routes.invoices_unified', 'invoices_unified_bp'),

        # Legacy routes (v1.x) - للتوافق مع الإصدارات القديمة
        ('src.routes.products', 'products_bp'),
        ('src.routes.customers', 'customers_bp'),
        ('src.routes.suppliers', 'suppliers_bp'),
        ('src.routes.sales', 'sales_bp'),
        ('src.routes.inventory', 'inventory_bp'),
        ('src.routes.reports', 'reports_bp'),
        ('src.routes.auth_routes', 'auth_bp'),
        ('src.routes.categories', 'categories_bp'),
        ('src.routes.warehouses', 'warehouses_bp'),
        ('src.routes.users', 'users_bp'),

        # Critical Missing Blueprints - Frontend Dependencies
        ('src.routes.accounting', 'accounting_bp'),
        ('src.routes.settings', 'settings_bp'),
        ('src.routes.integration_apis', 'integration_bp'),
        ('src.routes.rag', 'rag_bp'),

        # Advanced Features
        ('src.routes.advanced_reports', 'advanced_reports_bp'),
        ('src.routes.financial_reports', 'financial_reports_bp'),
        ('src.routes.financial_reports_advanced', 'financial_reports_advanced_bp'),
        ('src.routes.comprehensive_reports', 'comprehensive_reports_bp'),
        ('src.routes.products_advanced', 'products_advanced_bp'),
        ('src.routes.sales_advanced', 'sales_advanced_bp'),
        ('src.routes.inventory_advanced', 'inventory_advanced_bp'),

        # Management Modules
        ('src.routes.lot_management', 'lot_bp'),
        ('src.routes.batch_management', 'batch_bp'),
        ('src.routes.batch_reports', 'batch_reports_bp'),
        ('src.routes.warehouse_adjustments', 'warehouse_adjustments_bp'),
        ('src.routes.warehouse_transfer', 'warehouse_transfer_bp'),
        ('src.routes.returns_management', 'returns_management_bp'),
        ('src.routes.payment_management', 'payment_management_bp'),
        ('src.routes.payment_debt_management', 'payment_debt_management_bp'),
        ('src.routes.treasury_management', 'treasury_management_bp'),

        # Accounts & Partners
        ('src.routes.customer_supplier_accounts', 'customer_supplier_accounts_bp'),
        ('src.routes.partners', 'partners_bp'),

        # Settings & Configuration
        ('src.routes.company_settings', 'company_settings_bp'),
        ('src.routes.system_settings_advanced', 'system_settings_advanced_bp'),
        ('src.routes.permissions', 'permissions_bp'),

        # Import/Export
        ('src.routes.export', 'export_bp'),
        ('src.routes.excel_import_clean', 'excel_bp'),
        ('src.routes.excel_operations', 'excel_operations_bp'),
        ('src.routes.excel_templates', 'excel_templates_bp'),
        ('src.routes.import_export_advanced', 'import_export_advanced_bp'),

        # Additional Features
        ('src.routes.profit_loss', 'profit_loss_bp'),
        # ('src.routes.profit_loss_system', 'profit_loss_system_bp'),  # disabled to avoid duplicate blueprint name
        ('src.routes.security_system', 'security_bp'),
        ('src.routes.automation', 'automation_bp'),
        ('src.routes.interactive_dashboard', 'interactive_dashboard_bp'),
        ('src.routes.opening_balances_treasury', 'opening_balances_treasury_bp'),
        ('src.routes.user_management_advanced', 'user_management_advanced_bp'),
        # ('src.routes.lot_reports', 'lot_reports_bp'),  # disabled to avoid duplicate blueprint name 'batch_reports'
        # ('src.routes.sales_simple', 'sales_simple_bp'),  # disabled to avoid duplicate blueprint name 'sales'
...
def _register_health(app):
    ...
    try:
        ...
        # Check if api_meta is callable
        if callable(api_meta):
            api_meta_func = api_meta
    except Exception:  # noqa: BLE001
        api_meta_func = None

    # Use simple pass-through decorator if api_meta is not available/callable
    def identity_decorator(f):
        return f
    
    decorator_health = identity_decorator
    decorator_info = identity_decorator
    
    if api_meta_func:
        try:
            decorator_health = api_meta_func(summary='Health Check',
                                        description='Quick health probe for uptime monitoring',
                                        tags=['System'],
                                        response_schema='HealthStatus')
            decorator_info = api_meta_func(summary='System information',
