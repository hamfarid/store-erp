...
def check_accessibility_issues(file_path):
    ...
    try:
        ...
        buttons_without_labels = re.findall(button_pattern,
            content,
            re.IGNORECASE)
        ...
        selects_without_labels = re.findall(select_pattern,
            content,
            re.IGNORECASE)

        if selects_without_labels:
            issues.append(f"عناصر select بدون aria-label أو title: {len(selects_without_labels)}")
        ...
        inputs_without_labels = re.findall(input_pattern,
            content,
            re.IGNORECASE)
...
def check_specific_elements():
    ...
    try:
        ...
        # فحص عناصر محددة
        navbar_togglers = len(re.findall(r'navbar-toggler[^>]*aria-label',
            content))
        btn_closes = len(re.findall(r'btn-close[^>]*aria-label', content))
        selects_with_labels = len(re.findall(r'<select[^>]*aria-label',
            content))
        inputs_with_labels = len(re.findall(r'<input[^>]*(?:aria-label|placeholder)',
            content))
