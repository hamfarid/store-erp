...
if Blueprint is not None:
    ...
    class ExternalHealthResponse(Schema):
        service = fields.String(required=True, metadata={"example": "httpbin"})
        url = fields.String(
            required=True, metadata={"example": "https://httpbin.org/status/200"}
        )
        ok = fields.Boolean(required=True, metadata={"example": True})
        status_code = fields.Integer(required=True, metadata={"example": 200})
        elapsed_ms = fields.Integer(required=True, metadata={"example": 42})
