...
def init_database():
    ...
    if cursor.fetchone()[0] == 0:
        cursor.execute(
            "INSERT INTO warehouses (name, code, address) VALUES ('المستودع الرئيسي', 'MAIN', 'الرياض')"
        )
...
def get_categories():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "categories": [dict(cat) for cat in categories]}
        )
...
def get_warehouses():
    ...
    try:
        ...
        return jsonify({"success": True, "warehouses": [dict(wh) for wh in warehouses]})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def get_products():
    ...
    try:
        ...
        return jsonify({"success": True, "products": [dict(prod) for prod in products]})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def login():
    ...
    try:
        ...
        if user and user["password_hash"] == password:
            ...
            return (
                jsonify({"success": False, "error": "بيانات تسجيل الدخول غير صحيحة"}),
                401,
            )
