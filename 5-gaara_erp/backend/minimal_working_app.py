...
def get_supplier_performance():
    ...
    try:
        performance = {
            ...
            "delivery_performance": {"on_time": 89, "late": 8, "very_late": 3},
            "quality_metrics": {"excellent": 65, "good": 25, "average": 8, "poor": 2},
        }
...
def create_role():
    ...
    try:
        ...
        # إنشاء الدور
        cursor.execute(
            "INSERT INTO roles (name, description, is_active, created_at) VALUES (?, ?, 1, ?)",
        ...
        return jsonify(
            {"success": True, "message": "تم إنشاء الدور بنجاح", "role_id": role_id}
        )
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def check_permission():
    ...
    try:
        ...
        if not user_id or not permission_name:
            return (
                jsonify(
                    {"success": False, "error": "معرف المستخدم واسم الصلاحية مطلوبان"}
                ),
                400,
            )
        ...
        if user_role and user_role[0] == "admin":
            conn.close()
            return jsonify(
                {"success": True, "has_permission": True, "reason": "مدير النظام"}
            )
...
def get_available_permissions():
    ...
    try:
        available_permissions = [
            ...
            {
                "name": "suppliers",
                "display_name": "إدارة الموردين",
                "category": "العلاقات",
            },
            {
                "name": "invoices",
                "display_name": "إدارة الفواتير",
                "category": "المبيعات",
            },
    ...
    try:
        ...
        if backup_settings["include_uploads"]:
            ...
            if uploads_dir.exists():
                shutil.copytree(
                    uploads_dir, backup_path / "uploads", dirs_exist_ok=True
                )
...
def manual_backup():
    ...
    try:
        ...
        if backup_path:
            ...
            return (
                jsonify({"success": False, "error": "فشل في إنشاء النسخة الاحتياطية"}),
                500,
            )
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def restore_backup():
    ...
    try:
        ...
        if not backup_name:
            return (
                jsonify({"success": False, "error": "اسم النسخة الاحتياطية مطلوب"}),
                400,
            )
        ...
        if not backup_path.exists():
            return (
                jsonify({"success": False, "error": "النسخة الاحتياطية غير موجودة"}),
                404,
            )
