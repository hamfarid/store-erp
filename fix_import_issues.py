            ...
            try:
                ...
                if "from models.partners import" in content and "Customer" in content:
                    ...
                    content = re.sub(
                        ...
                        lambda m: f"from models.customer import Customer\nfrom models.partners import {m.group(1) or ''}{m.group(2) or ''}".replace("import ,", "import").replace("import \n", ""),
                        content
                    )
    ...
    if os.path.exists(file_path):
        ...
        for i, line in enumerate(lines):
            if "class Supplier(db.Model):" in line:
                supplier_start = i
            elif supplier_start is not None and line.startswith("class ") and "Supplier" not in line:
                supplier_end = i
                break
